...
| `Teknik-Tenaga-Listrik/` | `teknik_tenaga_listrik` | `teknik_tenaga_listrik-modul-N` (terbit: Modul 1–2) | TTL |
...
Total berkas HTML utama: **58 modul + 8 exam + 6 OBE**. Pemodelan CAD baru
sampai Tahap 1 (Silabus/OBE + RPS). Teknik Tenaga Listrik dibangun modul demi
modul: Modul 1–2 sudah terbit (generator `scripts/ttl-modul/bangun.py`) dan terdaftar
di `_MODUL_COURSES` backend lewat `moduls: [1, 2]`, tetapi belum punya exam, sehingga validator keamanan tetap
...
- **Akun simulasi** NIM `41399999901` ("SIMULASI MAHASISWA") — akun uji
  ...
  - daftar NIM-nya harus sama di dua tempat: `SIM_NIMS` di backend
    ...
    (disuntikkan ke 66 halaman modul/exam + 4 halaman OBE); nama ada di lima `students.json` **dan**
