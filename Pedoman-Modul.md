...
>
> **Diperbarui:** Juli 2026 (v22) — autentikasi dosen kini server-authoritative melalui Firebase custom token dan Secret Manager; backend, rules, bank soal, serta kunci jawaban berada di repositori privat. Catatan versi lama tetap dipertahankan sebagai riwayat dan harus dibaca bersama §19 serta §40.31.
>
...
> **v18.3 (Mei 2026) — Phase 3 rollout Optoauto UAS:** Apply pattern v18 ke Optoauto UAS. EXAM_CONFIG tambah entry `optoauto-uas`. Seed file baru `functions/seed/uas-optoauto-answers.js` (45 soal, MC tidak shuffle — UAS source explicitly disables). Client UAS.html migration. Latent bugs ditemukan + di-fix sebagai side-effect: (1) **15 comp entries pakai field `question:` bukan `text:`** → renderer baca `data.text` → comp text render undefined sebelumnya (mirror UAS Getaran v18.1 bug); (2) **15 comp entries TIDAK punya field `hint:`** → renderer interpolate `${data.hint}` literal "undefined" di pre tag → subagent ADD hint field untuk semua 15 soal dengan content compliant (imports + parameters + high-level TODO, no formula/algorithm leak). Hint compliance + Phase 3 migration dalam 1 PR (#239), tidak perlu follow-up seperti #238 untuk Optoauto UTS. PR #239.

> **v18.2 (Mei 2026) — Phase 3 rollout Optoauto UTS:** Apply pattern v18 ke Optimalisasi-dan-Automasi UTS. EXAM_CONFIG tambah entry `optoauto-uts` (paths `visitors/optoauto/uts` + `settings/optoauto/uts/schedule`). Seed file baru `functions/seed/uts-optoauto-answers.js` (45 soal, schema identik). Client UTS.html: strip semua answer keys, integrate `_callCheckExamAnswer` dengan EXAM_ID='optoauto-uts'. Fix pre-existing bug `_isHardComp` regex `/^c(\d+)$/` → `/^ch\d+$/` (Optoauto pakai prefix `ch<N>` untuk Hard dan `ce<N>` untuk Easy, beda dgn Getaran `c1..c15`). Seed Comp pakai field `expected:` (sama dgn UAS Getaran, server `evaluateAnswer` sudah handle keduanya sejak PR #235). PR #236.

> **v18.1 (Mei 2026) — Phase 3 rollout UAS Getaran:** Apply pattern v18 ke UAS Getaran Mekanik. EXAM_CONFIG tambah entry `getaran-mekanik-uas` (paths `visitors/getaran_mekanik/uas` + `settings/getaran_mekanik/uas/schedule`). Seed file baru `functions/seed/uas-getaran-answers.js` (45 soal, schema identik UTS, MC tidak shuffle, Comp Hard single-value tanpa `expectedSteps`). Client UAS.html: strip semua answer keys, integrate `_callCheckExamAnswer` dengan EXAM_ID='getaran-mekanik-uas', reset STEP 0 panggil callable. Bonus fix latent bug di UAS source: rename `question:` → `text:` di 15 comp entries (renderer baca `data.text` tapi entries pakai `question:` → comp text rendered undefined sebelumnya), wrap `${data.hint}` dengan conditional fallback. PR #234.
...
19. [Autentikasi Admin Berbasis Firebase](#19-autentikasi-admin-berbasis-firebase)
...
| **Dosen** | Pilih peran Dosen + **Password Admin** | Firebase custom token dengan claim `admin: true` (lihat §19) |
...
5. Client memanggil `createAdminSession({password})` melalui HTTPS dan menukar custom token dengan sesi Firebase Auth
6. Jika sesi ber-claim admin berhasil → identitas UI dosen disimpan, FAB muncul
...
if (isDosen) {
  ...
  const ok = await _verifyAdminPw(pwInput); // memanggil server + signInWithCustomToken
  if (!ok) {
    document.getElementById('visitorOverlay').classList.remove('hidden');
...
**Catatan:** Dosen tidak perlu setup PIN. Password hanya dikirim melalui HTTPS ke callable autentikasi; browser tidak menyimpan password maupun hash-nya.
...
- Bisa melakukan **reset** setelah sesi Firebase admin tervalidasi
- Bisa **set jadwal** setelah sesi Firebase admin tervalidasi
- Tidak perlu PIN
...
Tombol **"🕐 Atur Jadwal 🔒"** (di login overlay) **selalu terlihat untuk siapa pun** — namun penulisannya ke RTDB memerlukan token Firebase dengan claim admin.
...
2. **Layer Logic** (server auth): Atur Jadwal & Reset memerlukan sesi Firebase admin yang masih berlaku
3. **Layer Firebase Rules**: write sensitif memerlukan `auth.token.admin === true`; operasi besar melalui callable juga menjalankan `_requireAdmin(request)`
...
## 19. Autentikasi Admin Berbasis Firebase

### 19.1 Konsep

Semua tindakan dosen memakai sesi Firebase Authentication dengan custom claim
`admin: true`. Browser tidak menyimpan atau membandingkan hash password.
Password dikirim melalui HTTPS hanya ke callable `createAdminSession`, lalu
server membandingkan SHA-256-nya secara constant-time terhadap
`ADMIN_PASSWORD_HASH` di Firebase Secret Manager.

Jika cocok, server menerbitkan custom token Firebase. Callable administratif
memanggil `_requireAdmin(request)` dan menolak token tanpa claim admin atau
yang `auth_time`-nya sudah lebih dari dua jam. Lima kegagalan login dari
alamat yang sama memicu lockout 15 menit.

### 19.2 Aturan Implementasi

- Jangan menaruh password atau hash password admin di HTML, JavaScript publik,
  dokumentasi, workflow, maupun konfigurasi Git.
- Jangan menerima hash password melalui payload callable; hash publik dapat
  dipakai ulang seperti bearer credential.
...
### 19.3 Rotasi Password

1. Pilih password unik dan panjang; jangan menaruhnya di command history.
2. Hitung SHA-256 secara lokal.
3. Simpan hash sebagai GitHub Actions secret `ADMIN_PASSWORD_HASH_VALUE`
   pada repositori backend privat.
4. Jalankan workflow backend dengan `set_admin_secret=true` untuk menulis
   `ADMIN_PASSWORD_HASH` ke Firebase Secret Manager.
5. Deploy Functions dan uji bahwa password lama ditolak serta password baru
   menghasilkan sesi admin.

### 19.4 Alur Login Dosen

1. Pilih peran Dosen dan masukkan password.
2. Client memanggil `createAdminSession` melalui HTTPS.
3. Server menerapkan rate-limit dan validasi Secret Manager.
4. Client menukar custom token menjadi sesi Firebase Auth.
5. Login, pengaturan jadwal, reset, OBE, dan alat Admin memakai ID token yang
   otomatis dilampirkan Firebase SDK.
6. Logout atau perpindahan ke mahasiswa menghapus sesi Firebase Auth.
...
### 20.6 Rotasi Password Darurat

Jika dosen lupa password admin:
1. Pilih password baru yang panjang dan unik.
2. Hitung SHA-256 secara lokal (lihat §19.3); jangan kirim password mentah melalui chat.
3. Ganti GitHub Actions secret `ADMIN_PASSWORD_HASH_VALUE` di repositori backend privat.
4. Jalankan workflow backend dengan `set_admin_secret=true`, lalu deploy Functions.
5. Uji bahwa password lama ditolak dan password baru menghasilkan sesi admin.
...
**Password Admin** — satu kredensial lintas mata kuliah dan modul, tetapi nilainya tidak didokumentasikan di repositori. Hash-nya hanya berada di Firebase Secret Manager (lihat §19.3).
...
BUKAN hardcode — frontend berada di repo publik; HMAC key mentah yang bocor = siapa pun bisa forge kode. Hash password admin juga tidak boleh dipublikasikan karena dapat dipakai untuk serangan offline atau berubah menjadi kredensial replay jika desain client keliru.
Setup sekali: GitHub secret `EXPORT_CODE_SECRET_VALUE` → workflow input
`set_export_secret=true` (butuh role IAM Secret Manager Admin). Detail deployment berada di README/DEPLOY repositori backend privat.
...
| `resetExamQuestion` | dosen (admin pw hash) | Clear attempt, marker, pilihan/kode, dan scoreDelta untuk **soal UTS/UAS spesifik**. **NIM diisi** = 1 mahasiswa; **NIM kosong** = semua mahasiswa. UI di `Admin/reset-soal.html`; mendukung seluruh 6 exam dan ID khusus `ce/ch` pada Optoauto UTS. |
...
- **§19 Autentikasi Admin Berbasis Firebase** — callable OBE administratif memerlukan ID token dengan claim admin
...
### 40.6 Tabel Bernomor, Riset Referensi via WebSearch, dan Keandalan Simulator Pagination (BARU di v21, Jul 2026)

Revisi lanjutan Modul 1 Opto ke-3: dosen minta "tabel harus ada nomor+caption di
ATAS tabel (kebalikan gambar), disitasi di teks", gambar sedikit diperkecil
teksnya + audit overlap/terpotong, dan "pastikan jumlah daftar pustaka sudah
memenuhi aturan".

1. **Tabel bernomor — caption DI ATAS tabel, kebalikan konvensi gambar.**
   Helper `make_table(headers, rows, caption=...)` — kalau `caption` diisi,
   tulis paragraf `Tabel N. <caption>` (italic, size 10, `keep_with_next=True`
   supaya tidak terpisah page break dari tabelnya) TEPAT SEBELUM tabel
   dibuat, pakai counter terpisah `_TABEL_COUNTER` (independen dari
   `_GAMBAR_COUNTER`). Sama seperti gambar, body text SEBELUM tabel WAJIB
   menyitasi nomornya ("Tabel N merangkum...", "dirangkum pada Tabel N").
   Ingat konvensi APA: **judul tabel di ATAS, judul gambar di BAWAH** — jangan
   disamakan.
2. **Riset Daftar Pustaka: `WebSearch` bekerja, `WebFetch` langsung ke
   `doi.org`/`mdpi.com`/`pmc.ncbi.nlm.nih.gov` DIBLOK proxy (403)** di
   environment ini. `WebSearch` tetap mengembalikan metadata sitasi yang
   valid (judul, penulis, jurnal, volume, halaman, DOI) walau fetch langsung
   ke halaman yang sama gagal — pakai `WebSearch` utk riset, cross-check
   nama penulis dgn query kedua yang lebih spesifik kalau hasil pertama
   ambigu, lalu tulis sitasi lengkap format APA dari metadata yang
   terverifikasi (jangan mengarang DOI/volume/halaman).
3. **Aturan "≤10 tahun" berlaku KETAT ke SELURUH Daftar Pustaka, bukan cuma
   minimal-5-jurnal-nya** — dibaca ulang §40.1: "referensi ≤10 tahun
   terakhir, minimal 5 jurnal internasional + link". Artinya SEMUA entri
   harus dalam 10 tahun terakhir; paper metode fondasional yang lazim
   disitasi di akademia walau tua (mis. Box & Cox 1964, Cleveland STL 1990)
   **tetap melanggar aturan ini kalau usianya >10 tahun**, meski secara
   akademis "standar" untuk disitasi. **Solusi**: cari paper MODERN yang
   me-review/menerapkan/mengembangkan metode yang sama (mis. STL lama →
   ganti dgn "STD: A Seasonal-Trend-Dispersion Decomposition" Dudek 2023,
   IEEE TKDE, yang memperluas STL) — bukan menghapus sitasi metodenya, tapi
   mengganti sumbernya dgn versi kontemporer yang membahas metode yang sama.
   Modul 1 Opto akhirnya pakai 7 referensi 2021–2024, 5 di antaranya jurnal
   internasional ber-DOI (Sensors ×2, IEEE TKDE, Eng, Applied Sciences).
4. **⚠️ KOREKSI PENTING thd §40.5 — simulator pagination heuristik TERBUKTI
   TIDAK AKURAT** memprediksi lokasi *maupun* besar celah dibanding
   rendering Word sesungguhnya. Setelah simulator melaporkan "0 GAP BESAR
   selain halaman terakhir" dan file dikirim, dosen mengirim screenshot
   rendering Word asli yang menunjukkan celah kosong besar di 3 lokasi yang
   **sama sekali tidak diprediksi simulator** (termasuk 1 lokasi yang
   simulator anggap aman dgn margin nyaman). Investigasi: geometri halaman
   (886pt usable height) dan ukuran gambar ter-embed (dicek langsung dari
   XML `wp:extent`) keduanya **terverifikasi BENAR**, tapi celah nyata masih
   jauh lebih besar dari prediksi — akar masalah pastinya tidak berhasil
   didiagnosis (LibreOffice headless dicoba ulang, tetap gagal total,
   sehingga tidak ada cara verifikasi render sungguhan di sandbox ini).
   **Implikasi praktis ke depan**: JANGAN percaya angka lokasi/besar celah
   dari `simulate_pages.py` sbg kebenaran akhir — pakai hanya sbg sinyal
   internal kasar. Kalau dosen melaporkan (dgn screenshot) celah di lokasi
   tertentu, **percaya lokasi itu apa adanya** (bukan lokasi yang
   diprediksi simulator) dan tambal PERSIS di sana dgn paragraf substantif
   yang **cukup panjang (generous, bukan pas-pasan hasil kalkulasi)** —
   krn paragraf body text biasa (`keep_with_next=False`) bisa mengalir
   terpotong rapi di batas halaman manapun, menambah teks lebih banyak
   dari perkiraan JAUH lebih aman daripada kurang (worst case cuma
   melimpah rapi ke halaman berikut, bukan menyisakan ruang kosong).
5. **Audit overlap/clipping per-gambar sebelum & sesudah perkecil teks** —
   pola bug yang ditemukan & pola fix-nya (checklist ulang tiap kali font
   ilustrasi diubah): (a) **legend menimpa data** → pindah via
   `ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.16), ...)`
   (taruh di luar area plot, bukan di dalam `loc='upper right'` dst yang
   bisa tabrakan dgn garis sinyal); (b) **label persen di dalam wedge donut
   chart terpotong** kalau 2-baris (nama kategori + poin) — sederhanakan
   `autopct` jadi 1 baris angka saja, pindahkan detail (nama+poin) ke
   `labels=` eksternal (`labeldistance>1`); (c) **diagram kotak/panah
   (roadmap-style)** boros ruang kosong internal kalau `figsize` & ukuran
   box tidak match — perkecil `figsize` & `bw`/`bh` box bersamaan,
   hapus baris kode dead/duplikat sisa iterasi sebelumnya (mis. `ax.annotate`
   panah dgn koordinat lama yang lupa dihapus saat re-layout).
6. **PPT: gambar konten dipakai ULANG dari sumber Word (`figs/`) apa
   adanya** utk 7 dari 9 ilustrasi — `add_picture_fit()` di `build_ppt.py`
   sudah otomatis scale-to-fit + center menjaga aspect ratio ke box gambar
   PPT (± 3.76×4.24 in), jadi TIDAK PERLU generate ulang dgn `figsize`
   khusus PPT utk gambar landscape (cukup copy file PNG dari `figs/` ke
   `figs_ppt/`). Hanya 2 ilustrasi diagram-kotak (roadmap & analogi) yang
   punya varian `_vertical` khusus (`fig_roadmap_vertical`,
   `fig_analogi_vertical`, aspect portrait ~0.75) krn box gambar PPT
   portrait — variasi ini py FUNGSI TERPISAH dgn font size **hardcoded**
   (bukan referensi konstanta `SZ_*` global), jadi kalau `SZ_TITLE` dst di
   pangkal file diubah, INGAT update juga angka hardcoded di 2 fungsi
   `_vertical` ini scr manual (tidak otomatis ikut berubah). Daftar Pustaka
   Word & PPT WAJIB disinkron manual (7 referensi identik, tidak ada
   pengecekan otomatis lintas file) tiap kali salah satunya diperbarui.

### 40.7 LibreOffice Headless AKHIRNYA JALAN + Gap-Fixing 13 Modul (2-15) via Render+Ukur Nyata (BARU di v21, Jul 2026)

Revisi lanjutan ke-4 (dosen: "banyak gambar melebihi margin, masih banyak
space kosong. Perbaiki semuanya" → setelah fix margin, "masih ada space yg
kosong" → dosen lalu menantang premis "Apakah LibreOffice tidak bisa
di-instal dahulu?"). Ini membongkar asumsi lama di §40.4/§40.5/§40.6 bahwa
LibreOffice headless "nonfungsional total" di sandbox — **klaim itu SALAH,
akar masalahnya cuma package yang belum lengkap**:

1. **⚠️ KOREKSI KRITIS thd §40.4/§40.5/§40.6**: `soffice --headless
   --convert-to pdf` gagal total ("source file could not be loaded") krn
   paket `libreoffice-core`/`-common` SAJA terpasang, **`libreoffice-writer`
   tidak ada** — bukan limitasi sandbox permanen. Fix: `apt-get install -y
   libreoffice-writer poppler-utils` (poppler utk `pdftoppm`/`pdfinfo`/
   `pdftotext`, dipakai Read tool merender halaman PDF + ekstraksi teks).
   Setelah restart-repair ini, render+verifikasi visual sungguhan JADI
   MUNGKIN untuk pertama kalinya di seluruh proyek modul Word — pelajaran:
   **jangan terima "environment ini tidak mendukung X" sbg kesimpulan final
   tanpa diagnosis akar penyebab** (`dpkg -l | grep libreoffice`), terutama
   kalau dosen mempertanyakannya.
2. **Tooling verifikasi baru menggantikan `simulate_pages.py` (§40.5, sudah
   terbukti tidak akurat di §40.6 poin 4)**: (a) render tiap docx ke PDF via
   `soffice --headless --convert-to pdf` (profil user (`-env:UserInstallation`)
   HARUS unik/fresh per invocation kalau proses sebelumnya crash/lock, pakai
   folder `prof<N>` incrementing); (b) `analyze_gaps.py` — rasterize tiap
   halaman PDF via `pdftoppm -gray` 100 DPI, hitung baris piksel gelap
   (<200) per baris, cari baris konten TERAKHIR sebelum margin bawah, ukur
   jarak kosong; flag `>3.5cm` kecuali halaman terakhir dokumen (objektif,
   proxy akurat utk inspeksi visual manual, tervalidasi silang); (c)
   `find_culprits2.py` — `pdftotext -layout -f P -l P+1` utk ekstrak teks
   halaman ber-gap + halaman berikutnya, cari pola `(Gambar|Tabel) N\.` di
   awal halaman berikutnya utk identifikasi blok mana yg "lompat" & bikin
   gap. Verifikasi ukuran gambar aktual pakai `Document.inline_shapes`
   (python-docx) — BUKAN regex `wp:extent` mentah pada `document.xml` (regex
   itu ikut menangkap shape non-gambar dari template, spt textbox
   dekoratif, hasil ukur jadi salah/kecil palsu).
3. **Akar mekanis gap**: blok gambar+caption (`keep_with_next` menyatukan
   keduanya) atau tabel (`cantSplit` per baris + `keepNext` berantai, §40.4)
   adalah unit ATOMIK — kalau tidak muat di sisa ruang halaman, Word
   melompatkan SELURUH blok ke halaman baru, TIDAK reflow konten setelahnya
   utk mengisi sisa ruang kosong yg ditinggalkan. Ini murni masalah
   bin-packing per-halaman: mengecilkan 1 gambar mengubah titik lompat
   berikutnya, tapi TIDAK monoton — kadang membaik, kadang gap-nya cuma
   pindah ke halaman lain atau malah lebih besar (whack-a-mole, sesuai
   peringatan §40.5 poin 4). Cap tinggi GLOBAL (`MAX_IMG_H_CM`) terbukti
   tidak reliable turunkan total gap (uji 8,5cm vs 10,0cm: net nyaris sama,
   sebagian modul membaik sebagian memburuk) — **strategi yang benar-benar
   efektif: override `max_h_cm` PER-INSTANCE** pada `gambar()` (parameter
   opsional, hanya berlaku ke 1 panggilan spesifik), ditarget dari gap yang
   TERUKUR (bukan tebakan), lalu rebuild+render+re-ukur ulang tiap iterasi
   — target `max_h_cm ≲ gap_terukur − overhead_caption(≈1.0–1.5cm) − buffer`.
4. **Donut "Distribusi Poin Tugas"** (muncul di SETIAP modul) semula layout
   VERTIKAL (donut di atas, legend di bawah) → sangat tinggi (~15–21cm),
   sumber gap terbesar & paling sering di hampir semua modul. Redesign jadi
   HORIZONTAL (`gridspec_kw={'width_ratios':[1,1.5],'wspace':0.02}`, donut
   kiri + legend kanan) → tinggi turun jadi ~7,3cm di lebar kolom penuh —
   perbaikan tunggal ini menghilangkan gap di paling banyak modul sekaligus
   (pola: kalau 1 elemen berulang di SEMUA modul jadi sumber gap, benahi
   template elemen itu dulu sebelum override per-instance satu-satu).
5. **Tabel dipadatkan global** (`make_table()`): tambah `w:tcMar` eksplisit
   tipis (top/bottom 40 dxa ≈ 0,07cm, left/right 100 dxa) + `line_spacing =
   1.0` per paragraf sel (sebelumnya inherit default, sedikit lebih longgar)
   — mengecilkan tinggi SEMUA tabel sedikit, tanpa ubah font/konten,
   menghilangkan/mengecilkan gap tabel di beberapa modul (M11, M13 jadi 0
   gap; M14 turun dari 2 gap jadi 1). Tabel TETAP tak bisa dikecilkan
   sejauh gambar krn atomik & isinya data tetap — sisa gap ber-sumber tabel
   diterima sbg residual (lihat poin 6).
6. **Hasil akhir 13 modul (Pertemuan 2–15), diukur nyata via render**: total
   panjang gap (>3,5cm, jumlah semua halaman semua modul) turun dari ±169cm
   (baseline cap 10,0cm tanpa override) → ±57cm (setelah donut redesign +
   override per-instance + tabel dipadatkan), **penurunan ±66%**. 4 dari 13
   modul (M6, M10, M11, M13) 0 gap sama sekali; sisanya masing-masing 1–2
   gap residual di rentang 3,6–7,9cm (M14 Tabel 1 = gap terbesar tersisa,
   tabel 7-baris tak bisa dipadatkan lebih jauh tanpa restrukturisasi
   konten). **Keputusan sadar utk berhenti iterasi** di titik ini (bukan
   ngoyo sampai 0 gap mutlak): tiap ronde tambahan menunjukkan
   diminishing-return + risiko whack-a-mole makin tinggi (pernah 1 fix
   tabel di M2 justru MENGGESER reflow & memunculkan 2 gap baru yg lebih
   besar dari yg diperbaiki) — gap residual 3,6–7,9cm (±1/8–1/3 halaman)
   dianggap variasi wajar dokumen Word ber-banyak-gambar, sepadan dgn
   effort/reward yang menurun tajam. Verifikasi akhir: 0 margin-overflow
   (semua `inline_shapes` ≤ 15,5cm lebar), 0 em-dash, penomoran Gambar/Tabel
...
5. **Status: sudah digeneralisasi ke SELURUH 14 modul Opto (Modul 1-14)**,
   setelah pilot Modul 1 disetujui dosen ("Terapkan juga ke Modul 2-14
   Opto"). Digeneralisasi via 1 script Python (`patch_modules.py`, pola
   read→`src.replace(OLD, NEW)`→write, konsisten dgn §"Operasi Umum" batch
   edit) yg menerapkan 4 anchor-replace IDENTIK (CSS, tombol HTML, hook
   `_applyRoleVisibility()`, fungsi `exportModulPdf()`+konstanta URL PDF
   per-modul) ke Modul-2..14.html sekaligus — feasible krn KE-14 file
   modul Opto strukturnya seragam persis (navbar, `_applyRoleVisibility`,
   `LOCAL_IDENTITY` pattern identik antar modul, hanya `MODULE_ID` &
   konten yg beda). **Gotcha nomor Pertemuan vs nama file** (§ awal
   dokumen ini, "post-UTS shift"): `Modul-8.html` → `MODULE_ID =
   'pertemuan-9'`, `Modul-9.html` → `'pertemuan-10'`, dst (offset +1
   mulai Modul-8) — path PDF & nama file docx/pdf TETAP ikut nomor
   `Modul-N.html` (bukan nomor Pertemuan), jadi mapping PDF tidak perlu
   disesuaikan gotcha ini, HANYA `LOCAL_IDENTITY`/`MODULE_ID` yg beda
   (tapi itu sudah otomatis benar krn hook ditulis generic, baca variabel
   `me`/`MODULE_ID` yg sudah ada di tiap file, bukan di-hardcode).
   Diverifikasi: (a) `node --check` seluruh blok `<script>` ke-14 file
   lolos; (b) render 13 docx→PDF via LibreOffice (command sama §40.7),
   commit sejajar `.docx` masing-masing; (c) spot-check 4 modul (termasuk
   Modul-9 sbg kasus MODULE_ID-mismatch-dgn-nomor-file) via Playwright +
   stub Firebase (pola §40.9 poin 4) — disabled→login→enabled→klik→
   download PDF dgn ukuran file PERSIS cocok, semua lolos. **Belum**
   diperluas ke course lain (Getaran Mekanik, Math4) krn template Word/PPT
   utk course tsb belum dibuat (§40 baru cover Opto) — begitu modul Word
   course lain selesai, pola §40.9 ini reusable apa adanya (ganti
   `SLUGS`/path folder saja di `patch_modules.py`).

### 40.10 Modul Word Getaran Mekanik + Matematika 4 (28 Modul via Workflow Multi-Agent) + Export PDF Digeneralisasi ke 3 Course (BARU di v21, Jul 2026)

Permintaan dosen: "Lanjut buat modul word untuk getaran dan math4. Jika
sudah selesai, terapkan tombol export." Kode MK diberikan eksplisit:
Getaran `W132500020`, Matematika 4 `W132500014`. Ini memperluas cakupan
modul Word BOP (§40) dari HANYA Optoauto (14 modul) menjadi SELURUH 3
mata kuliah aktif (14+14+14 = 42 modul), skala 2x lipat kerja Opto
dalam satu batch.

1. **Skala besar → dosen setuju pakai Workflow multi-agent** (ditawarkan
   eksplisit via `AskUserQuestion` sblm mulai, krn 28 modul serial spt
   Opto akan memakan berapa sesi; dosen pilih opsi workflow). 28 agent
   paralel (14 Getaran + 14 Math4, 2 fase), tiap agent membangun SATU
   modul end-to-end (baca HTML sumber → desain 7-9 gambar matplotlib
   spesifik topik → tulis `build_word_mN.py` → build → audit terprogram
   → render PDF → analisa gap → perbaiki via tambah teks (BUKAN kecilkan
   gambar, eksplisit di-larang di prompt agent mengacu §40.8) → laporkan
   hasil terstruktur). **Setup sblm launch** (dikerjakan sendiri, bukan
   via agent): generalisasi `word_lib.py` (`set_cover()` tambah param
   `course_name` utk teks header footer, sebelumnya hardcode
   "Optimalisasi & Otomasi" — SATU-SATUNYA string Opto-spesifik di
   seluruh library, sisanya sudah generic sejak awal), buat folder
   `Modul-Word/` baru di kedua course, siapkan konstanta Sub-CPMK per
   modul (dipetakan manual dari `Asesmen-<Course>.json` `sub_cpmk[]` ke
   14 topik Pertemuan berdasar kedekatan tema — TIDAK ADA tabel mapping
   eksplisit Pertemuan→Sub-CPMK di source manapun, jadi mapping berbasis
   judgment kesamaan topik, low-stakes krn cuma dipakai di teks cover).
2. **Session limit mid-run**: 11 dari 14 modul Math4 gagal serentak
   dengan pesan "You've hit your session limit" (bukan error per-agent,
   limit akun keseluruhan). **Fix**: `Workflow({scriptPath,
   resumeFromRunId})` setelah limit reset (dicek via `date -u` vs waktu
   reset yg tertulis di pesan error) — 17 agent yg SUDAH sukses (14
   Getaran + 3 Math4) replay dari cache instan, 11 yg gagal jalan ulang
   fresh, seluruh 28 akhirnya sukses (0 error) di run kedua.
3. **Checkpoint incremental selama workflow berjalan** (bukan tunggu
   semua 28 selesai baru commit sekali): tiap kali dicek progress (via
   polling `journal.jsonl` utk event `"type":"result"`), commit+push HANYA
   modul yg SUDAH confirmed via journal — file di disk yg belum punya
   entry `"result"` di journal TIDAK di-commit (bisa masih partial/mid-
   write dari agent yg belum lapor, atau sisa attempt gagal sblm resume).
   Pola ini menghasilkan ~8 commit checkpoint terpisah sepanjang run
   (bukan 1 commit raksasa di akhir) — lebih aman thd kegagalan
   pertengahan (spt session limit di atas) krn kerja yg sudah selesai
   tidak hilang kalau harus resume/lanjut sesi baru.
4. **Hasil build**: 28/28 modul sukses (0 error), lolos audit terprogram
   penuh (0 em-dash, penomoran Gambar/Tabel sinkron, 0 margin-overflow,
   OOXML valid) di seluruh 28 file. 21/28 modul 0 gap kosong sama sekali
   dari agent masing-masing; 7 modul residual gap 4-20,7cm dilaporkan
   agent sbg "batas maksimal 3 ronde perbaikan tercapai". **Fase
   verifikasi final** (dikerjakan manual, bukan via agent, stlh workflow
   selesai): tinjau ulang seluruh residual, prioritas modul dgn gap
   terbesar — Math4 Modul 10 (Transformasi Laplace, gap 20,7cm dari 2
   blok Gambar yg lompat) ditambal jadi 4,9cm, Getaran Modul 3 (Getaran
   Bebas Teredam, 8,6cm dari h1-section + Gambar yg lompat) ditambal jadi
   0cm, Getaran Modul 12 (Indikator Time-Domain, 7,4cm dari Tabel yg
   lompat) ditambal jadi 0cm — pakai persis teknik §40.8 (tambah paragraf
   substantif tepat sebelum blok yg lompat, ukur ulang via
   `analyze_gaps.py`, TIDAK sentuh ukuran gambar). 4 modul sisa (gap
   3,5-4,9cm) diterima sbg residual wajar sesuai preseden §40.8. **Total
   akhir 28 modul: 24/28 nol gap, sisa 4 modul cuma ±20,5cm gabungan**
   (rata-rata <0,75cm/modul, konsisten dgn kualitas batch Opto).
5. **Export PDF digeneralisasi ke SEMUA 42 modul HTML (14 Opto + 14
   Getaran + 14 Math4)** — pola §40.9 diterapkan apa adanya via
   `patch_getaran_math4.py` (script batch-replace terpisah, bukan extend
   `patch_modules.py` Opto, krn folder/path course beda). **Gotcha
   `LOCAL_IDENTITY` per-course BEDA nama & pola**: Opto & Getaran pakai
   `<prefix>_identity_${MODULE_ID}` dgn prefix literal berbeda per file
   (`optoauto_identity_`, `getaran_mekanik_identity_`), sedangkan Math4
   pakai pola `${COURSE_ID}_identity_${MODULE_ID}` (satu variabel
   `COURSE_ID` dievaluasi runtime) — PENTING saat menulis test/QA manual
   (jangan asumsikan satu pola localStorage key berlaku ke semua course,
   selalu `grep -a -o "const LOCAL_IDENTITY = [^;]*;"` per file dulu utk
   pastikan pola sebenarnya sblm menyusun skenario simulasi login).
   Verifikasi: `node --check` seluruh 28 file baru + 14 file Opto lama
   (42 total) lolos; spot-check 6 modul (3 Getaran + 3 Math4, mencakup
   modul dgn gap terbesar spt Math4 Modul 10) via Playwright + stub
   Firebase — disabled→login (localStorage key benar per-course)→
   enabled→klik→download PDF ukuran file PERSIS cocok, semua lolos.

### 40.11 Fix Overlap Header/Footer Halaman 1 (Cover) — Berlaku ke SEMUA 42 Modul (BARU di v21, Jul 2026)

Laporan dosen: "Footer di modul word halaman 1 overlapping. Buat jenis
footer yg ada di halaman 2 dan seterusnya tidak aktif di halaman 1."

1. **Akar masalah**: template (`Template modul - kurikulum 2025.docx`)
   punya `sectPr` dgn `different_first_page_header_footer = False`,
   artinya SATU footer (badge tahun ajaran "2025 - 2026" dlm floating
   textbox, plus running header "Dedik Romahadi | <Course> | <no hal>")
   dipakai di SEMUA halaman termasuk halaman 1 (cover). Cover SUDAH py
   bar info sendiri di bagian bawah (Fakultas/Program Studi/BOP/
   myclass.mercubuana.ac.id) — footer badge tahun ajaran nge-overlap
   PERSIS di atas teks "Fakultas" itu (screenshot render menunjukkan
   "2025 - 2028kultas : Teknik", "2026" bertabrakan dgn "Fa" dari
   "Fakultas"). Header running di atas cover juga py masalah serupa
   (nomor halaman "1" nampang di atas elemen visual cover, tidak lazim
   utk cover page).
2. **Fix (OOXML `titlePg`, python-docx `different_first_page_header_footer`
   + `first_page_footer`)**: di `word_lib.init()`, tepat setelah
   `Document(dst_path)` dibuka, set
   `doc.sections[0].different_first_page_header_footer = True` lalu
   `doc.sections[0].first_page_footer.is_linked_to_previous = False`
   (python-docx otomatis isi first-page footer dgn 1 paragraf kosong,
   cukup utk suppress konten lama). **Efek samping yg diharapkan**:
   `first_page_header` ikut kosong OTOMATIS tanpa disentuh eksplisit,
   krn section tidak py `w:headerReference type="first"` — itu perilaku
   standar OOXML saat `titlePg` aktif tanpa referensi first-page
   didefinisikan, Word/LibreOffice fallback ke blank. Hasilnya cover
   (halaman 1) bersih total dari header/footer running, sedangkan
   halaman 2 dst TIDAK terpengaruh sama sekali (`sec.header`/`sec.footer`
   yg dipakai `set_cover()` utk isi teks running header tetap itu-itu
   juga, terpisah dari `first_page_header`/`first_page_footer`).
3. **Cara terapkan ke 42 modul yg SUDAH jadi**: patch struktural
   LANGSUNG ke `.docx` final (buka via python-docx, set 2 properti di
   atas, save ulang) — BUKAN rebuild dari source, krn build script
   Modul 1 Opto yg asli sudah tidak ada di scratchpad sesi ini (modul
   itu dibangun sesi sebelum sesi ini dimulai). Patch struktural lebih
   aman krn tidak menyentuh konten yg sudah diverifikasi susah payah
   (gap-fixing dst) — hanya `sectPr`, nol risiko keliru konten. Fix JUGA
   ditambahkan ke `word_lib.init()` (ketiga copy: `opto_build`,
   `getaran_build`, `math4_build`) supaya modul BARU yg dibangun ke
   depan otomatis kena fix ini, tidak perlu patch manual lagi.
4. **Verifikasi**: render ulang seluruh 42 PDF via LibreOffice, screenshot
   halaman 1 vs halaman 2 utk 2 modul sample (1 Getaran, 1 Math4) —
   overlap hilang total di halaman 1, halaman 2 identik dgn sebelumnya.
   Jumlah halaman tiap PDF (`pdfinfo | grep Pages`) dicek SAMA PERSIS dgn
   angka sblm fix di seluruh 42 file (krn fix ini murni properti
   section, nol perubahan konten body) — mengonfirmasi tidak ada regresi
   pagination/gap yg butuh verifikasi ulang. Audit terprogram (em-dash,
   penomoran Gambar/Tabel, OOXML valid) tetap PASS di semua 42 file.

### 40.12 Audit Sitasi "Gambar N"/"Tabel N" di Prosa — Cross-check thd Caption (BARU di v21, Jul 2026)

Laporan dosen: "Saya cek sitasi nomor gambar pada modul 1 word getaran
beberapa tidak match. Cek semuanya dan perbaiki yg salah."

1. **Akar masalah — kelas bug baru, terpisah dari caption**: `gambar()`/
   `make_table()` di `word_lib.py` pakai counter global
   (`_GAMBAR_COUNTER`/`_TABEL_COUNTER`) yg auto-increment, jadi **caption
   itu sendiri SELALU benar dan berurutan by construction** (mis. "Gambar
   3. Klasifikasi getaran..."). Bug-nya ada di kalimat prosa yg ditulis
   MANUAL terpisah dari pemanggilan `gambar()` — mis. "Gambar 5
   merangkum keempat kombinasi..." padahal yg dibahas ada di caption
   Gambar 3. Ini realistis terjadi krn tiap modul ditulis independen oleh
   agent yg bisa salah hitung urutan gambar saat menulis kalimat rujukan,
   terlepas dari nomor caption aktual yg baru ke-generate belakangan.
2. **Verifikasi manual dosen** (Getaran Modul 1) menemukan pola **rotasi
   3-siklus** di antara Gambar 3/4/5: paragraf yg mendeskripsikan topik
   Gambar 3 menyebut "Gambar 5", paragraf topik Gambar 4 menyebut
   "Gambar 3", paragraf topik Gambar 5 menyebut "Gambar 4" (2×, termasuk
   1 forward-reference di seksi Tugas) — total 4 mismatch di 1 file.
3. **Audit terprogram seluruh 42 modul via Workflow 42-agent paralel**
   (1 agent per modul, FIND-ONLY — dilarang edit file): tiap agent
   ekstraksi paragraf via python-docx, kumpulkan semua caption
   ("Gambar N."/"Tabel N." di awal paragraf) + semua sitasi prosa
   ("Gambar N"/"Tabel N" di paragraf non-caption), lalu bandingkan
   TOPIK tiap sitasi thd topik caption bernomor sama DAN caption
   tetangga (N±1, sesuai pola rotasi/off-by-one yg sudah terkonfirmasi).
   Forward-reference (menyitasi gambar sebelum captionnya muncul di
   teks, pola umum di paragraf pembuka/Tugas) TIDAK otomatis dianggap
   salah — tetap diverifikasi via kecocokan topik. Hasil: **10 mismatch
   terkonfirmasi high-confidence di 3 dari 42 modul** (39 modul lain
   BERSIH, termasuk 13 modul Opto lain yg sudah lolos verifikasi
   sebelumnya) — semua bertipe Gambar, nol mismatch Tabel:
   - **Getaran Modul 1** (4 mismatch, match persis temuan manual dosen):
     paragraf idx 32 (Gambar 5→3), idx 43 (Gambar 3→4), idx 50
     (Gambar 4→5), idx 91 (Gambar 4→5).
   - **Opto Modul 3** (3 mismatch, pola off-by-one N→N+1 di sekitar
     segmen spectrogram/heatmap): idx 66 (Gambar 5→6), idx 68
     (Gambar 6→7), idx 71 (Gambar 6→7).
   - **Math4 Modul 8** (3 mismatch, semua merujuk kurva Dynamic
     Magnification Factor tapi menyitasi nomor skema diagram statis):
     idx 76 (Gambar 5→6), idx 90 (Gambar 5→6), idx 100 (Gambar 5→6).
4. **Fix**: MEKANIS, bukan via agent — begitu `paragraph_index` +
   `wrong_text` + `correct_number` diketahui pasti dari hasil audit,
   fix diterapkan langsung via python-docx run-level text replacement
   (skrip Python, bukan spawn agent lagi krn tidak perlu judgment lebih
   lanjut). Tiap mismatch dicek dulu: `paragraph.text.count(wrong_text)
   == 1` DAN persis 1 run yg mengandung teks itu (semua 10 kasus lolos
   cek ini tanpa kecuali — teks sitasi tidak pernah kepotong lintas-run
   di modul-modul ini), baru `run.text` diganti in-place supaya
   formatting run lain di paragraf yg sama (bullet marker, bold label
   "Cell 2:", dst.) tidak tersentuh.
5. **Verifikasi**: `len(doc.inline_shapes)`/`len(doc.tables)` dan jumlah
   paragraf caption ("Gambar N."/"Tabel N.") dicek SAMA PERSIS
   sblm/sesudah fix (murni ganti 1 angka dalam teks prosa, nol dampak ke
   caption/gambar/tabel itu sendiri). PDF ke-3 modul di-render ulang via
   LibreOffice — jumlah halaman IDENTIK persis dgn baseline sblm fix
   (22/18/24 halaman), mengonfirmasi fix murni tekstual tanpa reflow.
   Cross-check tambahan: sitasi "mereproduksi Gambar 3" (Math4 M8, idx
   99, DEKAT tp beda dari 3 mismatch di modul yg sama) diverifikasi
   manual thd caption Gambar 3 ("Dekomposisi solusi total y=yh+yp") —
   cocok topiknya (Cell 3 vs Cell 1 sama-sama solve PD yg sama), BUKAN
   mismatch — dipertahankan apa adanya, ilustrasi kenapa audit
   per-paragraf presisi (bukan blanket search-replace nomor) penting.
6. **Cakupan tak-berulang**: 39 modul lain (termasuk semua 13 modul Opto
   non-M3, semua modul Getaran/Math4 lain) dikonfirmasi BERSIH — tidak
   perlu tindakan lanjutan, tapi build modul BARU ke depan tetap rawan
   kelas bug ini krn sifatnya (prosa manual independen dari counter
   caption) — cross-check sitasi vs caption WAJIB jadi bagian checklist
   audit modul Word baru (bersama em-dash count, OOXML valid, gap
   pagination di §40.7–§40.8).

### 40.13 Audit Sub-CPMK Modul Word thd RPS Resmi — 3 Fix Mekanis + 2 Temuan Butuh Keputusan Dosen (BARU di v21, Jul 2026)

Laporan dosen: "Pastikan semua sub cpmk yang ada di modul Word sudah
sesuai dengan RPS-nya." (RPS 3 mata kuliah baru di-upload ke
`<Course>/Attributes/RPS <Nama MK>.pdf`.)

1. **Ground truth ganda ditemukan**: setiap course punya DUA sumber
   katalog Sub-CPMK — (a) RPS PDF resmi (baru di-upload dosen) dan (b)
   field `sub_cpmk` di `Attributes/Asesmen-<Course>.json` (SSOT bobot
   grading yg SUDAH dipakai live oleh sistem OBE/exam). Untuk **Getaran**
   dan **Math4**, kedua sumber ini IDENTIK (deskripsi + pengelompokan
   CPMK sama persis) — audit langsung apple-to-apple. Untuk **Opto**,
   KEDUANYA BERBEDA pengelompokan meski memakai 14 kalimat deskripsi
   yang SAMA PERSIS: RPS PDF mengelompokkan CPMK1–4 (4/4/4/2), sedangkan
   `Asesmen-Optimalisasi-dan-Otomasi.json` mengelompokkan CPMK1–5
   (3/3/3/3/2) — kalimat RPS-1.4 versi PDF menjadi "2.1" di JSON, dst.,
   bergeser satu slot per batas CPMK sampai 2 kalimat terakhir RPS
   ("CPMK4" versi PDF) menjadi "CPMK5" fiktif di JSON. **Modul Word Opto
   dibangun mengikuti JSON (SSOT grading), BUKAN RPS PDF** — temuan ini
   TIDAK diputuskan sepihak (bisa berarti RPS PDF perlu dikoreksi, atau
   JSON+grading+modul perlu direstrukturisasi ke grouping RPS PDF —
   dampaknya ke exam scoring, jadi WAJIB keputusan dosen, lihat poin 4).
2. **Metodologi**: ekstraksi baris "•  Sub-CPMK X.Y: ..." dari 40 baris
   pertama tiap docx (cover) via python-docx, dicocokkan terhadap
   katalog resmi (kode, `cpmk_induk`, deskripsi) via reverse-lookup teks
   (bukan asumsi posisi/urutan modul, krn kurikulum riil SUDAH
   direstrukturisasi vs urutan minggu RPS — "post-UTS shift" style,
   sama seperti pola di §40.10). Dicek 3 lapis: (a) parenthetical
   "(CPMK N)" harus cocok dgn digit pertama kode Sub-CPMK X.Y yg
   disitasi (cek mekanis, tanpa syarat); (b) kode yg disitasi harus
   BENAR-BENAR ADA di katalog resmi (bukan fabrikasi); (c) isi
   deskripsi modul harus secara substansi cocok dgn makna resmi kode yg
   disitasi (butuh judgment konten).
3. **3 fix mekanis diterapkan langsung** (tidak butuh judgment konten,
   aman di-auto-fix spt pola §40.12):
   - **Getaran Modul-3** (`Getaran-Bebas-Teredam.docx`, idx~10): sitasi
     "Sub-CPMK 3.1 ... (CPMK 1)" — SALAH, Sub-CPMK 3.1 adalah anak
     CPMK 3 di katalog (bukan CPMK 1). Fix: "(CPMK 1)" → "(CPMK 3)".
   - **Opto Modul-1** (`Pengantar-Time-Series-Data.docx`, idx~10):
     berlabel "Sub-CPMK 1.1" tapi teks deskripsinya adalah kalimat
     Sub-CPMK 1.3 ("Mampu mempertimbangkan aspek ekonomi...") — SAMA
     PERSIS dgn deskripsi Modul-3 (duplikat, sementara kalimat 1.1 yg
     asli — "Mampu mengidentifikasi komponen dan kebutuhan teknis
     sistem otomasi" — tidak dipakai modul manapun). Fix: ganti klausa
     dasar ke teks 1.1 yg benar, elaborasi teknis (sampling
     rate/sensor/storage) dipertahankan krn masih akurat utk framing
     1.1. Setelah fix, seluruh 14 modul Opto membentuk bijeksi bersih
     1:1 thd 14 entri katalog JSON tanpa gap/duplikat.
   - **Math4 Modul-13** (`Metode-Jumlahan-Pecahan-Parsial.docx`,
     idx~11): baris Sub-CPMK 4.2 kehilangan suffix "(CPMK 4)" (13 modul
     lain semua konsisten menyertakannya, termasuk Modul-12 yg
     memakai Sub-CPMK 4.2 yang sama). Fix: tambah "(CPMK 4)" di akhir
     kalimat.
   - Verifikasi: PDF ke-3 modul di-render ulang, page count identik
     persis sblm/sesudah (25/21/24 halaman) — fix murni tekstual.
4. **2 temuan BUTUH keputusan dosen, TIDAK di-auto-fix** (berbeda kelas
   dari poin 3 — bukan typo mekanis, tapi ketidakcocokan konten/dokumen
   yg hanya pemilik RPS yang bisa memutuskan resolusinya):
   - **Opto**: ketidaksesuaian pengelompokan RPS PDF vs Asesmen JSON
     (poin 1 di atas) — perlu keputusan mana yang otoritatif ke depan.
   - **Getaran** — 3 modul dgn isi konten yang secara substansi TIDAK
     cocok dgn makna resmi kode Sub-CPMK yang disitasi (kode+CPMK-induk
     sendiri sudah benar, tapi topik riil modul beda dari definisi
     resmi):
     - **Modul-6** (`Eksitasi-Base-Motion-dan-Rotating-Unbalance`)
       berlabel Sub-CPMK 2.1 ("Getaran Bebas Tanpa Redaman di sistem
       Rotasi" = getaran bebas TANPA gaya luar), tapi isinya tentang
       eksitasi base motion + rotating unbalance — itu getaran PAKSA
       (ada gaya eksitasi eksternal), bukan getaran bebas.
     - **Modul-10** (`Vibration-Absorber-dan-Isolasi-Getaran`) berlabel
       Sub-CPMK 2.2 ("Metode Rayleigh Energi" = teknik analitik
       estimasi frekuensi natural via energi), tapi isinya tentang
       desain vibration absorber & isolasi getaran — topik desain
       komponen pengendali getaran, bukan teknik estimasi Rayleigh.
     - **Modul-12** (`Indikator-Getaran-Time-Domain-RMS-Crest-Kurtosis`)
       berlabel Sub-CPMK 6.2 ("komputasi domain waktu MENJADI domain
       frekuensi"), tapi isinya murni indikator statistik time-domain
       (RMS/Peak/Crest/Kurtosis) — deskripsi modul sendiri bahkan
       menyebut ini "fondasi SEBELUM transformasi ke domain frekuensi
       PADA PERTEMUAN BERIKUTNYA", jadi secara internal kontradiktif
       dgn label 6.2-nya sendiri.
   - Ketiganya TIDAK di-reassign sepihak krn menyangkut keputusan
     desain kurikulum (apakah 3 modul ini sengaja dikonsolidasikan ke
     slot Sub-CPMK terdekat sbg bagian dari restrukturisasi urutan
     materi, atau memang perlu kode Sub-CPMK yang berbeda) — di luar
     wewenang audit mekanis, harus dikonfirmasi ke dosen pemilik RPS.
5. **Math4 & sisa 11 modul Opto & sisa 13 modul Getaran**: dikonfirmasi
   BERSIH (kode ada di katalog, CPMK-induk benar, isi cocok topik —
   termasuk pola sah re-use 1 kode Sub-CPMK utk >1 modul berurutan yg
   memang SUDAH ada persis di RPS Math4 sendiri, mis. Sub-CPMK 2.2/2.3/
   3.1/4.1/4.2 masing dipakai 2 modul berurutan, BUKAN bug).

### 40.14 Keputusan Dosen: Opto Direnumber ke Grouping RPS PDF (BARU di v21, Jul 2026)

Jawaban dosen atas temuan §40.13 poin 1: "Untuk opto, sesuaikan jumlah
dan nomor CPMK dengan RPS (PDF). Buat modul word mengacu ke RPS (PDF)."
— RPS PDF (CPMK1–4, grouping 4/4/4/2) dipilih sbg otoritatif utk modul
Word Opto, BUKAN grouping Asesmen JSON (CPMK1–5, 3/3/3/3/2). Catatan:
instruksi ini scoped ke modul Word saja — `Asesmen-Optimalisasi-dan-
Otomasi.json` (dipakai live utk exam grading/OBE) TIDAK disentuh sesi
ini, jadi utk sementara modul Word dan sistem grading py grouping CPMK
yang BERBEDA utk Opto (lihat §40.13 poin 1 utk detail perbedaannya) —
kalau ke depan grading/JSON juga perlu diselaraskan ke RPS PDF, itu
perubahan terpisah yang lebih besar (nyentuh scoring code) dan harus
diminta eksplisit.

1. **Reverse-mapping via teks**: krn RPS PDF dan Asesmen JSON memakai
   14 kalimat deskripsi Sub-CPMK yang SAMA PERSIS (cuma beda pengelom-
   pokan kode/CPMK-induk), remapping dilakukan by CONTENT MATCH per
   kalimat (bukan asumsi posisi), menghasilkan tabel 1:1 pasti:
   `Sub-CPMK 2.1(JSON)="dampak sosial&lingkungan" = Sub-CPMK 1.4(RPS)`,
   dst. — kode 1.1/1.2/1.3 (3 kalimat pertama CPMK1) kebetulan SAMA di
   kedua katalog, sisanya (M4–M14) semua bergeser.
2. **Fix diterapkan ke 14 modul Opto** (`renumber_opto_subcpmk.py`):
   ganti label "Sub-CPMK X.Y:" (cover bullet) dan suffix "(CPMK N)" ke
   kode RPS PDF yang benar; klausa dasar + elaborasi teknis (identik di
   kedua katalog) TIDAK disentuh. M1/M2/M3 tidak berubah kode (RPS &
   JSON sepakat utk 3 kalimat pertama), M4–M14 semua berubah kode.
3. **GOTCHA ditemukan saat verifikasi**: cover page py **preview box**
   terpisah (elemen `w:txbxContent`/text-box, DILUAR jangkauan
   `doc.paragraphs`/`doc.tables` python-docx biasa) yang MENDUPLIKASI
   ringkasan "Sub-CPMK X.Y – <klausa dasar>" (format beda dari cover
   bullet: pakai en-dash "–" bukan titik dua ":", TANPA suffix "(CPMK
   N)"). Fix awal (hanya via `doc.paragraphs`) MELEWATKAN elemen ini
   sama sekali — termasuk retroaktif utk fix Modul-1 di §40.13 (preview
   box Modul-1 masih tampilkan teks SALAH lama "aspek ekonomi..." walau
   cover bullet-nya sudah benar). Fix: akses langsung via
   `doc.element.body.iter(qn('w:t'))` (raw XML text nodes, bkn API
   paragraph-level) utk temukan+ganti SEMUA node teks yg cocok exact-
   match — tiap modul ternyata py 2 node duplikat identik (kemungkinan
   `mc:AlternateContent` Choice/Fallback ganda). **Pelajaran utk modul
   Word manapun ke depan yg py preview box**: SETIAP kali Sub-CPMK
   kode/teks diedit, WAJIB cross-check `doc.element.body.iter(qn('w:t'))`
   utk sitasi kode di LUAR `doc.paragraphs`, jangan asumsikan cover
   bullet adalah satu-satunya tempat kode itu muncul.
4. **Verifikasi akhir**: script audit re-run scan SELURUH `w:t` node di
   tiap docx (bukan cuma 40 paragraf pertama) → tiap modul kini py
   TEPAT 1 kode Sub-CPMK unik yg konsisten di seluruh dokumen (cover
   bullet + preview box + sitasi badan teks bila ada). Bijeksi 1:1
   penuh thd 14 entri RPS PDF terkonfirmasi (`1.1..1.4, 2.1..2.4,
   3.1..3.4, 4.1..4.2`, nol gap nol duplikat). PDF ke-14 modul di-render
   ulang, page count identik persis sblm/sesudah (21/19/18/16/16/13/16/
   15/16/15/14/15/14/15 halaman) — fix murni tekstual, nol dampak body.

### 40.15 Keputusan Dosen: Getaran Modul-6/10/12 Direassign ke Sub-CPMK yang Digabung Modul Terkait (BARU di v21, Jul 2026)

Jawaban dosen atas 3 temuan konten Getaran di §40.13 poin 4: "Perbaiki
getaran Modul-6/10/12 (word) sesuai dengan RPS (PDF)." Berbeda dari
kasus Opto (§40.14, murni bookkeeping krn RPS & JSON pakai kalimat yg
SAMA cuma beda kelompok), di sini TIDAK ada regrouping tersembunyi utk
ditemukan — 3 modul ini genuinely membahas topik yang tak match kode
Sub-CPMK manapun di katalog RPS secara sempurna, dan seluruh 14 kode
RPS SUDAH terpakai penuh oleh 14 modul (11 lainnya sudah dikonfirmasi
benar di §40.13). Perbaikan yg valid HANYA ada 2 opsi: (a) reuse kode
Sub-CPMK milik modul lain yg topiknya berdekatan (menciptakan 2 modul
per kode, pola yg sudah terbukti sah di RPS Math4 sendiri — lihat
§40.13 poin 5), atau (b) pertahankan kode asli + reword deskripsi.
Krn ini keputusan konten/kurikulum sungguhan (bukan typo mekanis),
3 opsi diajukan ke dosen via `AskUserQuestion` per modul dgn rekomendasi
eksplisit; dosen approve seluruh 3 rekomendasi tanpa modifikasi.

1. **Modul-6** (`Eksitasi-Base-Motion-dan-Rotating-Unbalance`):
   **Sub-CPMK 2.1 → 3.2** (gabung dgn Modul-5, "Resonansi/DMF/
   Transmissibility"). Rasional: transmissibility SECARA MATEMATIS
   diturunkan justru dari base motion & rotating unbalance di hampir
   semua textbook getaran (termasuk Rao yg jadi Pustaka Utama RPS) —
   Modul-5 dan Modul-6 adalah dua sisi topik yang sama (respons thd
   eksitasi harmonik), bukan topik terpisah. Klausa dasar diganti ke
   teks resmi 3.2 ("Memahami dan menerapkan Respons getaran bebas
   tanpa redaman dengan eksitasi"), elaborasi teknis (base motion,
   rotating unbalance, transmissibility, faktor pembesaran) DIPERTAHAN-
   KAN sbg penerapan konkret dari respons-thd-eksitasi tsb.
2. **Modul-10** (`Vibration-Absorber-dan-Isolasi-Getaran`): **Sub-CPMK
   2.2 → 4.2** (gabung dgn Modul-9, "Analisis Modal 2-DoF"). Rasional:
   vibration absorber (Tuned Mass Damper) SECARA STRUKTURAL adalah
   sistem 2-DoF (massa utama + massa auxiliary) yang didesain lewat
   analisis frekuensi pribadi & mode shape — persis kompetensi 4.2.
   Klausa dasar diganti ke teks resmi 4.2 ("Menerapkan Respons getaran
   bebas pada sistem 2 DoF"), elaborasi diarahkan ke tuning absorber
   sbg aplikasi desain dari analisis 2-DoF tsb (isolasi getaran tetap
   disebutkan sbg topik pendamping, bukan fokus utama klausa).
3. **Modul-12** (`Indikator-Getaran-Time-Domain-RMS-Crest-Kurtosis`):
   **Sub-CPMK 6.2 → 5.1** (gabung dgn Modul-14, "Komponen Pengukuran &
   Jenis Getaran"). Rasional: indikator RESMI 5.1 di tabel mingguan RPS
   (bukan cuma frasa katalog terse) EKSPLISIT menyebut "data RMS dan
   nilai Peak" sbg bagian materi minggu itu — match langsung dgn isi
   Modul-12, jauh lebih kuat drpd 6.2 (yg justru literally kontradiktif
   krn Modul-12 SEBELUM transformasi domain frekuensi, bukan sesudah).
   Klausa dasar diganti ke teks resmi 5.1, elaborasi diarahkan ke
   RMS/Peak/Crest/Kurtosis sbg "data pengukuran inti" + kaitan ISO 10816.
4. **GOTCHA sama spt §40.14 poin 3 ditemukan ulang**: preview box
   Getaran ternyata pakai VARIAN teks beda dari Opto — Opto preview box
   isinya klausa dasar CUSTOM modul (`sub_cpmk_desc` custom text yg
   sama dgn cover bullet), sedangkan Getaran preview box isinya teks
   RESMI RPS TERSE (`"Sub-CPMK X.Y – <official RPS phrase>"`, BUKAN
   elaborasi custom modul) — jadi string exact-match yg dicari utk fix
   preview box HARUS diverifikasi ulang per-course, tidak boleh
   asumsi format sama dgn course lain walau gotcha (elemen
   `w:txbxContent` di luar `doc.paragraphs`) itu sendiri identik.
5. **Verifikasi**: scan penuh `doc.element.body.iter(qn('w:t'))` per
   modul konfirmasi TEPAT 1 kode Sub-CPMK per dokumen (cover bullet +
   preview box, kedua node duplikat preview box konsisten). PDF ke-3
   modul di-render ulang, page count identik persis sblm/sesudah
   (25/27/22 halaman) — fix murni tekstual, nol dampak body/pagination.
6. **Batasan scope**: fix ini HANYA menyentuh 3 modul yg diminta —
   11 modul Getaran lain (termasuk Modul-4/5/9/14 yg sekarang berbagi
   kode dgn Modul-6/10/12) TIDAK diubah teksnya, cuma jadi tujuan reuse
   kode. Konsisten dgn precedent Math4 §40.13 poin 5 (reuse kode lintas
   modul berurutan adalah pola SAH di RPS, bukan bug), tapi berbeda dari
   Math4 krn Getaran modul yg berbagi kode di sini TIDAK berurutan
   (M5&M6 berurutan ✓, tapi M9&M10 dan M14&M12 TIDAK berurutan) — dosen
   sadar & approve trade-off ini scr eksplisit via konfirmasi opsi.

### 40.16 Keputusan Dosen: Sistem Grading Opto Direnumber ke Grouping RPS PDF — Blast Radius Jauh Lebih Besar dari Modul Word (BARU di v21, Jul 2026)

Follow-up dosen atas §40.14 (yg eksplisit scoped ke modul Word saja):
"Update Asesmen JSON grading to match RPS PDF grouping too." Berbeda
dari §40.14 (nol risiko, murni dokumen statis), permintaan ini
menyentuh **SSOT grading LIVE** yg dipakai sistem OBE/exam scoring utk
mahasiswa sungguhan — audit awal sebelum edit menemukan bahwa
"Asesmen JSON" bukan satu-satunya tempat, melainkan **6 lokasi
independen** yg semuanya menyimpan salinan/turunan katalog Sub-CPMK
CPMK1-5 lama dan HARUS diupdate serentak (kalau cuma sebagian, sistem
JADI LEBIH RUSAK drpd sebelum disentuh — mismatch antar lokasi):

1. `Optimalisasi-dan-Automasi/Attributes/Asesmen-Optimalisasi-dan-
   Otomasi.json` — field `sub_cpmk` (katalog 14 entri), `ringkasan_cpmk`
   (breakdown per CPMK, 5→4 grup), `metadata.catatan` + `asesmen.
   deskripsi_komponen.uts/uas` (prosa yg sitasi kode Sub-CPMK spesifik).
2. `functions/index.js` → `OBE_EXAM_CONFIG["optoauto-uts"/"optoauto-
   uas"]` — **SATU-SATUNYA sumber yg BENAR-BENAR dibaca** `_examQPoints()`
   utk hitung poin per soal real-time saat mahasiswa submit UTS/UAS
   (komentar kode sendiri: "SUMBER UTAMA poin per soal").
3. `functions/index.js` → `OBE_MAP`/`OBE_TUGAS_COLS`/`_defaultObeMapping()`
   — fallback SERVER-SIDE utk `computeObeScores` kalau client (Dokumen
   OBE HTML) tidak kirim mapping custom; **hardcoded Opto-only**
   (komentar: "Fallback default (Opto)"), TERNYATA sudah py Q-range
   yg BEDA dari OBE_EXAM_CONFIG sblm sesi ini (pre-existing drift,
   bukan bug baru) — kode direnumber tapi Q-range dipertahankan apa
   adanya (bukan scope utk diperbaiki).
4. `functions/index.js` → `EXAM_CONFIG["optoauto-uts"/"optoauto-uas"]`
   → field `bobot`/`mapping` — **DEAD CODE, terkonfirmasi via komentar
   kode sendiri** ("tidak pernah dibaca siapa pun; `_examQPoints` pakai
   `OBE_EXAM_CONFIG`") — tetap direnumber demi konsistensi dokumentasi
   (bukan krn fungsional, mencegah kebingungan pembaca masa depan yg
   tidak tahu blok ini vestigial).
5. `Optimalisasi-dan-Automasi/OBE/Dokumen-OBE.html` — **7 struktur data**:
   `SUBCPMK` (katalog), `CPMK` (5→4 entri, teks CPMK-4 lama TERNYATA
   sudah salah/tidak match RPS sama sekali — teks CPMK-5 lama yg
   verbatim = RPS CPMK4 asli), `CPL` (persentase 43/21/22/14
   DIPERTAHANKAN apa adanya — bobot institusional per-CPL, TIDAK
   direkomputasi dari sub-weight), `T2` (bobot per komponen), `T3`
   (bobot per CPMK, 6→5 kolom), `T4` (bobot CPMK→CPL, jadi mapping
   1:1 bersih persis RPS stlh renumber — sebelumnya CPMK-1 & CPMK-2
   SAMA-SAMA nyumbang ke kolom CPL3 yg PERTAMA, indikasi bug serupa
   sudah ada sblm sesi ini), `FORMS` (kolom Tugas/UTS/UAS + bobot%),
   `DEFAULT_MAPPING` (localStorage-overridable client default) — plus
   HTML table header s3 (5→4 kolom CPMK) & JS render slice
   `d.slice(0,5)/d[5]` → `d.slice(0,4)/d[4]`.
6. `Optimalisasi-dan-Automasi/Exam/UTS.html` + `UAS.html` — info panel
   statis "Cara hitung nilai & bobot Sub-CPMK" (kolom kode saja
   diupdate; kolom Q-range sudah TERBUKTI stale/tidak match
   OBE_EXAM_CONFIG SEJAK SEBELUM sesi ini — dibiarkan, di luar scope).
7. **CPL/CPMK mapping RPS dikonfirmasi bersih 1:1**: RPS PDF py tabel
   "Peta CPL dengan CPMK" eksplisit CPL3→CPMK1, CPL4→CPMK2, CPL5→CPMK3,
   CPL6→CPMK4 (4 CPL, 4 CPMK, tanpa split) — kontras dgn T4 lama yg
   split CPL3 antara CPMK-1(20%)+CPMK-2(23%). Setelah remap, tiap CPMK
   baru otomatis 100% ke SATU CPL (CPMK1=27%→CPL3, CPMK2=30%→CPL4,
   CPMK3=29%→CPL5, CPMK4=14%→CPL6) — TAPI perhatikan: 27/30/29/14
   (jumlah sub-weight internal course) ≠ 43/21/22/14 (bobot CPL
   institusional resmi, dipertahankan). Ini BUKAN inkonsistensi baru —
   bobot CPL adalah kontribusi course ini ke keseluruhan kurikulum
   (lintas banyak mata kuliah), independen dari breakdown internal
   sub-CPMK course ini sendiri.
8. **Remap table** (identik dgn §40.14, sama persis reverse-lookup
   berbasis konten, bukan posisi): 1.1/1.2/1.3 tetap; 2.1→1.4;
   2.2→2.1; 2.3→2.2; 3.1→2.3; 3.2→2.4; 3.3→3.1; 4.1→3.2; 4.2→3.3;
   4.3→3.4; 5.1→4.1; 5.2→4.2. Verifikasi arithmetic: Σ semua bobot
   tetap 100 di tiap struktur (sub_cpmk, T2, T3, T4, FORMS×3) sblm/
   sesudah remap — dicek via Node.js eval script, bukan asumsi.
9. **KRITIS — DUA HAL YANG TIDAK BISA DISELESAIKAN DARI SESI INI,
   WAJIB TINDAK LANJUT MANUAL DOSEN**:
   - **Deploy Cloud Functions**: perubahan `functions/index.js`
     (`OBE_EXAM_CONFIG` khususnya) TIDAK berlaku ke sistem live sampai
     dosen jalankan `firebase deploy --only functions` (lihat panduan deploy
     repositori backend privat) — environment sesi ini TIDAK punya Firebase
     CLI/kredensial, jadi TIDAK BISA deploy sendiri. Sebelum deploy,
     repo vs live production BERBEDA (repo sudah CPMK1-4, live masih
     jalan CPMK1-5 lama) — ini status SEMENTARA yg WAJIB disadari,
     bukan cacat.
   - **Data OBE/exam yang SUDAH ada**: kalau ada mahasiswa Opto yang
     SUDAH mengerjakan UTS/UAS term ini SEBELUM deploy, attempt lama
     tersimpan di Firestore `examAttempts/optoauto-*` tetap valid
     (poin per SOAL tidak berubah, cuma LABEL Sub-CPMK grouping-nya
     yang berubah) — TAPI kalau ada Rekap OBE yg SUDAH di-publish ke
     Firestore (`publishObeNilai`) dengan kode Sub-CPMK LAMA sblm
     deploy, WAJIB `recomputeAllObeScores`/republish ulang SETELAH
     deploy supaya label Sub-CPMK di rekap konsisten dgn kode baru —
...
### 40.18 RPS Opto Dibangun Ulang Total sbg .docx Editable, Sinkron ke Konten Riil 14 Modul (BARU di v21, Jul 2026)

Follow-up §40.17. Dosen hapus `RPS Optimalisasi dan Otomasi.pdf` dari
repo lalu upload sendiri versi `.docx` — tapi file upload itu ternyata
cuma hasil convert PDF-ke-Word apa adanya (5 tabel, teks generik
"Pengantar Optimalisasi dan Otomasi"/"Perancangan Sistem Otomasi" dll.
yang TIDAK sinkron dengan topik riil 14 modul: Time Series, Signal
Processing, LP/NLP/ANOVA, ISO 10816, ML). Instruksi dosen: "Sesuaikan
isinya dengan 6 screenshot dan dokumen lain. Pastikan isi RPS semuanya
sinkron dengan modul, penilaian, dll." — RPS dibangun ulang dari nol
via `python-docx` (bukan edit/convert file upload dosen), menggantikan
pendekatan hybrid redact+splice PDF di §40.17 poin 3 sepenuhnya.

1. **Konten per-minggu diriset ulang dari modul asli** — bukan
   direka-reka: agent riset baca 14 modul Word (Abstrak, sub-topik,
   Tools, Daftar Pustaka via python-docx + raw XML utk cover text-box)
   lalu dipetakan 1:1 ke `WEEK` dict (minggu→sub-CPMK/CPMK/BK/modul).
   Tabel mingguan detail (kolom Sub-CPMK/Indikator/Kriteria&Bentuk/
   Tatap-Muka/Daring/Materi[Pustaka]/Bobot) pakai `subtopik` + `ref`
   riil tiap modul di kolom Materi, bukan lagi "Chapter N [1]" generik.
2. **Bobot per-minggu di-derive dari Asesmen JSON, bukan diperkirakan**
   — draft awal `BOBOT_MINGGU` (hardcode manual) ternyata TIDAK match
   SSOT (jumlah 92% bukan 100%); fix: hitung `praktikum+tugas+uts+uas`
   per `sub_cpmk` langsung dari
   `Asesmen-Optimalisasi-dan-Otomasi.json` → hasil match persis
   `ringkasan_cpmk` (20/23/21/22/14, Σ=100%). **Pelajaran**: constant
   yang "kelihatan masuk akal" tetap WAJIB diverifikasi terprogram thd
   SSOT, jangan percaya draft manual walau kelihatan konsisten secara
   internal.
3. **Bug kedua yang sama jenisnya ditemukan & fix sebelum commit**:
   `UTS_CODES`/`UAS_CODES` hardcode di script (7+7 split) TIDAK match
   `bobot.uts`/`bobot.uas` riil di JSON (yang sebenarnya 5+6 split —
   Sub-CPMK 2.1/5.1/5.2 murni Tugas, TIDAK dinilai via UTS **atau**
   UAS). Ini persis skema yang sudah benar di `UTS.html`/`UAS.html`
   (§40.16/§40.17) — script RPS sempat drift dari kedua file itu.
   Fix: derive ulang dari JSON, tambah cabang "Penugasan terstruktur
   (Tugas)" di kolom Kriteria&Bentuk utk 3 minggu yang bukan UTS/UAS
   (minggu 4, 14, 15) — sebelumnya semua minggu dipaksa label
   "Tes tertulis (UTS/UAS)" walau sebenarnya tidak diujikan.
4. **Struktur dokumen penuh** (14 halaman): cover → SAP (CPL/BK summary
   + tabel 16-minggu Belajar-Terbimbing/Penugasan-Terstruktur/Mandiri)
   → RPS header/Otorisasi (nama pengesah digeser vertikal ke tengah
   baris via paragraf kosong, bukan lagi redaksi PDF spt §40.17 poin
   sebelumnya krn sekarang dibangun native) → CPL/CPMK/Sub-CPMK/Peta-
   CPL-CPMK (skema CPMK1-5, 3/3/3/3/2, identik §40.17) → Komponen
   Penilaian (checkmark UTS/UAS per Sub-CPMK) + Deskripsi MK + Bahan
   Kajian + Materi Pembelajaran + Pustaka (2 utama + 4 pendukung, dipilih
   dari frekuensi sitasi lintas 14 modul) → tabel mingguan detail (16
   baris) → Catatan (11 definisi, section terpisah dgn nomor halaman
   restart dari "1", pakai `w:fldChar`/`PAGE` field asli bukan teks
   statis shg auto-update kalau dosen edit manual di Word).
5. **Gotcha python-docx yang berulang dari sesi sebelumnya** (dicatat
   ulang krn re-applied di script baru ini): merge cell HARUS cuma isi
   teks di cell pertama grup (sisanya kosong sblm merge, else teks
   dobel); lebar kolom WAJIB di-set di 2 tempat (`table.columns[i]
   .width` DAN per-cell `cell.width`) utk nested table, else kolom bisa
   overflow tak kelihatan di render LibreOffice; `Table.add_row()`
   bukan `add_rows()`.
6. File lama `RPS Optimalisasi dan Otomasi.docx` (hasil convert PDF
   dosen) DIGANTIKAN (bukan ditambah) oleh versi baru di path yang
   sama; PDF baru (`.pdf`, exported via LibreOffice headless dari
   `.docx` yang sama) ditambahkan kembali di path yang sama dgn nama
   file lama sblm dihapus dosen — `.docx` sekarang jadi source of
   truth yang editable, `.pdf` cuma turunan render.
7. Setiap tahap (cover, SAP, otorisasi, CPL/CPMK/Komponen-Penilaian,
   tabel mingguan, Catatan+footer) dites checkpoint-per-checkpoint
   (`_stageN_*.docx` → convert PDF → render PNG → baca visual) sebelum
   lanjut ke tahap berikutnya — bukan ditulis sekaligus lalu didebug
...
### 40.21 docs/ — Situs Pratinjau PDF via GitHub Pages (BARU di v21, Jul 2026)
...
- `raw.githubusercontent.com/...` — dicek via `curl -I`, GitHub set
  `Content-Type: application/octet-stream` (memaksa download) DAN
  `X-Frame-Options: deny` (tidak bisa di-iframe). Terkonfirmasi tidak
  bisa dipakai utk preview sama sekali, ini enforced server-side GitHub
  bukan sesuatu yang bisa di-workaround dari nama file/extension.
- `github.com/<owner>/<repo>/blob/main/...` — preview inline JALAN,
  tapi selalu dibungkus UI penuh github.com (header, sidebar, repo
  nav) — persis yang mau dihindari dosen.

Solusi (SAAT ITU): **GitHub Pages**, folder khusus `docs/` di root repo.

> ⛔ **KOREKSI KRITIS — JANGAN IKUTI RESEP `/docs` DI BAWAH INI.**
> Lihat §40.21b. Mengubah Pages Source ke "Deploy from a branch → /docs"
> **MERUSAK SELURUH SITUS LMS** (404 di 42 modul + login mahasiswa mati).
> Bagian di bawah dipertahankan apa adanya sbg catatan historis; resep
> setup-nya sudah TIDAK BERLAKU.

- `docs/index.html` — landing page single-file, self-contained (no
  CDN/external asset), dark/light via `prefers-color-scheme`, list 3
  course × 3 link (Modul/Exam/RPS), tiap link `target="_blank"` ke
  `docs/pdf/<file>.pdf` — GitHub Pages serve PDF dgn `Content-Type:
  application/pdf` tanpa `Content-Disposition:attachment` dan tanpa
  `X-Frame-Options` restriktif (beda dgn raw.githubusercontent.com),
  jadi browser buka native PDF viewer inline (preview), bukan download.
- `docs/pdf/*.pdf` — **salinan duplikat** dari `Unduhan-Gabungan/*.pdf`
  (bukan symlink — GitHub Pages build tidak reliable ikut symlink).
  Konsekuensi: 2× storage utk 9 file yg sama (~16MB dobel) DAN 2×
  risiko staleness (lihat §40.20 poin 4) — kalau salah satu source
  diregenerate, WAJIB sync ulang KEDUA folder (`Unduhan-Gabungan/` dan
  `docs/pdf/`), jangan lupa salah satu.
- ~~**Setup manual WAJIB dosen**: Settings → Pages → Source: "Deploy
  from a branch" → `main` → `/docs`~~ ⛔ **SALAH — JANGAN DILAKUKAN.**
  Lihat §40.21b.

### 40.21b ⛔ KOREKSI KRITIS §40.21 — Pages SUDAH aktif via GitHub Actions, root-scoped (Jul 2026)

Resep `/docs` di §40.21 dibuat **tanpa mengecek dulu apakah repo sudah
punya GitHub Pages**. Ternyata sudah — dan itu **hosting LIVE seluruh
LMS**. Menerapkan resep tsb membuat dosen melaporkan "modul tidak bisa
diakses, 404 file not found" untuk SEMUA modul.

**Fakta yang terlewat (WAJIB dicek sebelum menyentuh konfigurasi Pages):**

1. `.github/workflows/deploy-slides.yml` sudah ada sejak Jun 2026 —
   deploy ke Pages via **GitHub Actions**, dan mem-publish **SELURUH
   root repo** (`rsync -a --exclude=... . _site/`), bukan cuma folder
   Slides. Jadi Pages Source repo ini = **"GitHub Actions"**, bukan
   "Deploy from a branch".
2. Pages inilah **frontend hosting LMS yang sesungguhnya** — bukan
   sekadar tempat naruh slide:
   - Tiap `Modul-N.html` punya link "versi Preview" ke
     `https://dedik-romahadi.github.io/.../Modul/Modul-N.html`.
   - **`STUDENTS_JSON_URL` (roster mahasiswa untuk LOGIN) di-fetch dari
     Pages** — lihat §3.1/§7. Kalau path itu 404, login mahasiswa mati
     total, bukan cuma preview PDF yang hilang.
   - `firebase.json` **tidak punya key `hosting`** sama sekali —
     Firebase di repo ini HANYA backend (RTDB + Firestore + Functions).
     Jangan berasumsi frontend-nya di Firebase Hosting.
3. Mengubah Source ke "Deploy from a branch → /docs" membuat Pages
   hanya menyajikan isi `docs/`; SEMUA path lain (42 modul, 6 exam, 3
   OBE, `Attributes/students.json`, Banner, Slides) langsung 404.

**Fix:** Settings → Pages → Source kembalikan ke **"GitHub Actions"**,
lalu jalankan workflow "Deploy Slides ke GitHub Pages" (kini otomatis,
lihat §40.25).

**Konsekuensi lanjutan — folder `docs/` SUDAH DIHAPUS (Jul 2026):**
karena root repo memang disajikan, `docs/` jadi redundan — 9 PDF di
dalamnya cuma duplikat `Unduhan-Gabungan/*.pdf` (§40.20) yang sudah
bisa diakses langsung di `.../Unduhan-Gabungan/<file>.pdf` dgn
perilaku inline-preview persis sama. Atas persetujuan dosen, seluruh
folder dihapus (10 file) supaya tidak ada beban sinkronisasi ganda
(dulu: tiap regenerate WAJIB sync ke 2 tempat, gampang lupa salah
satu). **Link kanonik sekarang `.../Unduhan-Gabungan/<file>.pdf`** —
link lama `.../docs/pdf/...` sudah mati, jangan dipakai/dibagikan lagi.

**PELAJARAN (berlaku umum, jangan diulang):** sebelum mengusulkan
perubahan konfigurasi hosting/deploy apa pun — `ls .github/workflows/`
DULU, dan `grep -rn "github.io" .` untuk melihat apakah domain itu
sudah dipakai sebagai dependensi runtime. Jangan menyimpulkan "belum
ada Pages" hanya karena tidak menemukannya di dokumentasi; cek
artefak nyata di repo.

- URL hasil (Pages root-scoped): `https://dedik-romahadi.github.io/Mechanical-Engineering-Courses/<path-apa-pun-di-repo>`
  — mis. `.../Unduhan-Gabungan/RPS-Getaran-Mekanik.pdf`,
  `.../PDD-UKTPT/Video/index.html`, `.../PDD-UKTPT/TOEFL.pdf`.

### 40.22 TTD di 6 Exam Word (UTS/UAS × 3 course) — Tabel VERIFIKASI SOAL UJIAN (BARU di v21, Jul 2026)

Dosen minta TTD ditambahkan ke SEMUA exam (bukan cuma RPS), dengan
teks placeholder "Tanda Tangan" dihapus dulu sebelum gambar
ditempel. Setiap 1 dari 6 file
`<Course>/Exam/UTS_*.docx`/`UAS_*.docx` punya tabel identik 3 baris
"VERIFIKASI SOAL UJIAN" (2 kolom: **Dosen Pembuat Soal** = Dedik
Romahadi, ST., M.Sc; **Ketua Program Studi** = Dr. Eng. Imam
Hidayat, ST., MT) — signer SAMA persis dengan 2 dari 3 pihak
Otorisasi RPS (§40.19), jadi PNG tanda tangan yang sama
(`sig_dosen_pengemban.png`, `sig_ketua_prodi.png`) dipakai ulang,
tidak perlu ekstraksi baru.

1. **Cari paragraf via exact-text-match `'Tanda Tangan'`, BUKAN index
   baris/tabel hardcoded** — lebih robust krn semua 6 file py struktur
   XML yang identik (2 hit per file, diverifikasi via `assert hits==2`)
   walau nomor tabel/baris bisa saja beda kalau ada dosen edit lain.
2. **GOTCHA row-height overflow → nama signer hilang dari render**
   (paling mahal ditemukan sesi ini): tabel ini py row auto-size SANGAT
   ketat (awalnya cuma pas utk 1 baris teks kecil "Tanda Tangan").
   Tempel gambar tanda tangan (lebih tinggi dari teks placeholder)
   membuat total tinggi cell melebihi sisa ruang halaman 1 → LibreOffice
   PDF export MEMECAH baris tabel itu sendiri (row split, `cantSplit`
   tidak di-set), baris nama penanda tangan "yatim" muncul sendirian di
   ATAS halaman 2, sementara gambar tanda tangan tertinggal di halaman
   1 — kelihatan seperti "nama hilang" kalau cuma preview halaman 1.
   **Naikkan `w:trHeight`/`hRule=atLeast` TIDAK cukup** (sudah dicoba,
   tetap overflow — batasnya adalah sisa ruang di HALAMAN, bukan tinggi
   minimum baris). Fix yang benar: hapus SEMUA paragraf kosong
   (padding) di cell yang sama drpd gambar+nama, mengecilkan total
   tinggi konten shg pas kembali di sisa ruang halaman 1 — page count
   PDF WAJIB diverifikasi identik sblm/sesudah per file (bukan cuma
   `validate.py`, krn ini murni masalah layout bukan XML-invalid) via
   `fitz.open(pdf); len(doc)`.
3. **Slack halaman 1 BEDA per file** (tergantung panjang konten
   sebelumnya spt CPMK table) — lebar TTD maksimum yang masih muat
   TANPA nambah halaman diverifikasi empiris per file (binary search
   lebar), bukan diasumsikan seragam. `UAS_Getaran_Mekanik_-_Dedik.docx`
   py slack PALING SEDIKIT dari 6 file (lebar >3,4cm sudah overflow
   walau 4 paragraf kosong sudah dihapus) — jadi lebar final dipilih dari
   file TERKETAT (3,0cm, sama utk kedua TTD sesuai request dosen
   "ukurannya sama"), diterapkan seragam ke 6 file walau 5 file lain
   py slack lebih longgar.
4. Setelah edit, PDF individual 6 file diregenerate (LibreOffice headless)
   IN-PLACE, lalu turunan combined `Exam-Gabungan-<Course>.pdf` WAJIB
   diregenerate ulang juga (staleness trap yang sama spt §40.20 poin 4,
   kali ini sumbernya exam bukan modul). *Catatan: saat itu ada 2 lokasi
   (`Unduhan-Gabungan/` + `docs/pdf/`) yang harus disync bersamaan;
   sejak `docs/` dihapus (§40.21b) tinggal `Unduhan-Gabungan/` saja.*
...
### 40.23 Fix Tombol Copy Kode Python (`cpC()`) — 42 Modul (BARU di v21, Jul 2026)

Dosen laporkan "tombol copy tidak berfungsi pada mahasiswa" utk kode
Python (bagian Implementasi Python DAN tab Setup di Modul 1). Root
cause: fungsi `cpC(btn)` (dipakai semua tombol `📋 Copy` kode di
seluruh 42 modul, byte-identik di tiap file — bukan shared JS file,
tiap `Modul-N.html` standalone) py 2 bug independen:

1. **`btn.closest('.code-wrap')` gagal utk tombol tab Setup** — sel
   kode "Implementasi Python" dibungkus `<div class="code-wrap">`
   (cocok), tapi kotak terminal di tab Setup Modul-1 (3 course, class
   `sp-cbh-copy`) py struktur BEDA: `<div class="sp-cb"><div
   class="sp-cbh">{tombol}</div><div class="sp-cbd"><pre>...</pre></div></div>`
   — tidak py ancestor `.code-wrap` sama sekali. `.closest()`
   return `null`, lalu `.querySelector('pre')` di atas `null` THROW
   (`Cannot read properties of null (reading 'querySelector')`) —
   tombol diam total, tanpa pesan error apa pun ke user. Diverifikasi
   via Playwright headless (file://, testing DOM+JS terisolasi tanpa
   load Firebase) — direproduksi persis dgn kode LAMA, hilang dgn kode
   BARU. Fix: `btn.closest('.code-wrap, .sp-cb')`.
2. **Tanpa fallback clipboard sama sekali** — beda dgn `copyForumHtml()`
   (§40.19 dst., py 3 lapis fallback: Clipboard API → `execCommand`
   → popup manual), `cpC()` LAMA langsung panggil
   `navigator.clipboard.writeText(...)` tanpa cek `window.isSecureContext`
   atau keberadaan `navigator.clipboard` — di browser/WebView yg
   memblokir/tidak expose Clipboard API (umum di in-app browser mobile,
   kebijakan proxy sekolah, dll.), pemanggilan ini bisa throw
   SYNCHRONOUS (bukan promise-reject) shg TIDAK tertangkap `.catch()` —
   gagal senyap, mismatch dgn yg dosen alami sendiri (browser desktop
   normal, jadi tidak pernah reproduce). Fix: replikasi pola fallback
   3-lapis yg sama spt Forum copy (helper baru `_cpCFallback()`, textarea
   sementara + `execCommand('copy')`).
3. Batch-fix via Python string-replace exact match (bukan regex) ke 42
   file skaligus — diverifikasi SEMUA 42 file py fungsi `cpC()` byte-
   identik persis sblm patch (guard `content.count(OLD)==1` per file,
   0 mismatch), sehingga 1 replace pattern aman diterapkan universal.
   6 file Exam (UTS/UAS × 3 course) TIDAK py `cpC` sama sekali (dicek
   dulu via grep) — tidak disentuh, tidak relevan.
4. Verifikasi `node --check` tiap blok `<script>` inline ke-42 file
   (syntax valid) + Playwright functional test (klik kedua varian
   tombol, 0 `pageerror`, teks berubah jadi "✅ Copied!") sebelum commit.

### 40.24 Video Cadangan via GitHub Release — `PDD-UKTPT/Video/index.html` (BARU di v21, Jul 2026)

Dosen minta preview/cadangan video berbasis GitHub kalau YouTube
bermasalah, sumber video ±300MB (terlalu besar utk chat upload maupun
`git push` biasa — batas keras GitHub 100MB/file). Solusi: **GitHub
Release asset** (bukan commit ke git tree, bukan Git LFS) — upload
manual via web UI (drag-drop), limit jauh lebih longgar (2GB/file),
TIDAK numpuk ke riwayat git/clone size, dan TIDAK kena kuota bandwidth
ketat Git LFS (1GB/bulan gratis — akan habis cepat kalau ditonton
berkali-kali). Asset final: tag `video-pdd-uktpt`, file
`PDD-UKTPT.PENGAJARAN..._.0306029106.mp4` (282MB), diverifikasi via
`get_release_by_tag` API (bukan tebak nama dari teks terpotong di UI
screenshot dosen) — nama file asli py underscore/titik pengganti
spasi otomatis dari GitHub saat upload.

1. **Verifikasi range-request support** (`Accept-Ranges: bytes` pada
   response header, dicek via `curl -I` ke redirect target
   `objects.githubusercontent.com`) — WAJIB sblm klaim "bisa diputar
   langsung tanpa unduh penuh", krn ini beda dari raw.githubusercontent.com
   (§40.21) yg force-download. GOTCHA: signed S3 URL (presigned,
   expire 30 menit) gagal divalidasi ulang lewat sandbox proxy sesi ini
   (401 saat retry GET Range) — bukan bukti fitur tidak jalan, cuma
   proxy sesi mengganggu signature; `Accept-Ranges: bytes` di response
   PERTAMA (sebelum retry) sudah cukup sbg bukti, dikombinasikan dgn
   pengetahuan umum S3-backed GitHub Release assets memang support
   range request.
2. Halaman `PDD-UKTPT/Video/index.html` (BARU): toggle 2 sumber via
   tombol (▶ YouTube default, 🐙 Cadangan GitHub) — video GitHub
   TIDAK di-preload (baru inject `<video src=...>` saat tombol
   diklik) demi cegah fetch 282MB sia-sia ke pengunjung yg cuma mau
   nonton via YouTube. Diverifikasi via Playwright (klik toggle 2 arah,
   cek src berubah benar, 0 `pageerror`).
3. **GOTCHA `.gitignore`**: repo py rule `PDD-UKTPT/Video/` (directory-
   ignore) + `*.mp4/*.mov/*.mkv` global (guard rail existing thd video
   mentah masuk git) — file HTML baru di folder yg sama ikut ke-ignore
   total (git tidak descend ke direktori yg di-exclude, negasi
   `!path/file` di dalamnya TIDAK berlaku kalau direktori itu sendiri
   yg di-exclude, bukan isinya). Fix: ubah pattern direktori jadi
   `PDD-UKTPT/Video/*` (exclude ISI, bukan direktori itu sendiri) +
   baris `!PDD-UKTPT/Video/index.html` sesudahnya — video mentah tetap
   ke-block, index.html lolos. Diverifikasi via `git status` (harus
   muncul sbg `??`, bukan senyap hilang).
4. **Follow-up dosen: "lama menunggu videonya jalan"** — root cause
   BUKAN kecepatan koneksi/GitHub, tapi **posisi `moov atom`** (index
   metadata MP4) di file sumber — file hasil rekam (Zoom/OBS/dll,
   umumnya) menulis moov atom di AKHIR file, shg browser streaming
   HARUS fetch porsi besar dari 282MB dulu sblm bisa mulai play, walau
   server sudah support range-request (§40.24 poin 1) — 2 hal berbeda
   (byte-range serving vs LOKASI metadata yg diminta duluan). Fix:
   dosen jalankan `ffmpeg -i in.mp4 -c copy -movflags +faststart out.mp4`
   sendiri di lokal (remux murni — copy stream tanpa re-encode, cepat +
   zero quality loss, cuma reposisi moov atom ke depan file utk
   "progressive download"/web-optimized MP4), lalu replace asset
   Release manual via web UI. Asset final: `PDD-UKTPT-Serdos-2026-
   faststart.mp4` (296.867.383 bytes) — diverifikasi ulang via
   `get_release_by_tag` API (bukan asumsi nama file dari deskripsi
   dosen), URL di `index.html` disesuaikan.
5. **Follow-up dosen: hapus toggle YouTube, tampilkan cadangan GitHub
   langsung** — setelah faststart fix, halaman disederhanakan drastis:
   iframe YouTube + tombol switcher + 2 blok note dihapus semua,
   tersisa 1 `<video controls preload="metadata" src=...>` langsung ke
   asset GitHub. Diverifikasi via Playwright: `iframe` count = 0,
   `video` count = 1, src sesuai asset terbaru, 0 `pageerror`.

### 40.25 Deploy Pages Otomatis (`push` ke main) — Ganti Trigger Manual (BARU di v21, Jul 2026)

`deploy-slides.yml` sejak dibuat (Jun 2026) hanya py trigger
`workflow_dispatch:` — komentarnya: "Deploy manual saja … hindari
deploy tiap push selama masih tahap editing". Akibatnya SETIAP
perubahan (merge PR maupun upload file via web UI) tidak pernah tayang
sampai dosen membuka tab Actions dan klik Run workflow manual —
gampang terlupa, dan sempat bikin beberapa perbaikan "sudah di-merge
tapi kok belum berubah" berturut-turut.

Fix: tambah trigger `push: branches: [main]`, `workflow_dispatch:`
...
2. **Merge via GitHub App token TETAP memicu `push` workflow** —
   sempat diragukan krn ada aturan GitHub "event dari `GITHUB_TOKEN`
   tidak memicu workflow baru". Aturan itu khusus `GITHUB_TOKEN` DI
   DALAM workflow, bukan installation token GitHub App. Diverifikasi
   empiris: merge PR #669 langsung memunculkan run #27 dgn
   `event: push` (25 run sebelumnya semuanya `workflow_dispatch`).
3. Tidak dipakai *path filter* (mis. skip kalau cuma `Pedoman-Modul.md`
   berubah) — sengaja, supaya tidak ada kasus "kok tidak ke-deploy?"
   yang membingungkan. Build ~3 menit; repo publik ⇒ menit Actions
   gratis tak terbatas, jadi build ekstra tidak berbiaya.
4. `concurrency: {group: pages, cancel-in-progress: true}` sudah ada
   sejak awal — push beruntun tidak menumpuk, run lama otomatis
   dibatalkan.
5. **Konsekuensi ke dokumentasi**: instruksi lama "jalankan workflow
   manual setelah merge" di §40.21/§40.24 dst. sudah TIDAK berlaku.
   Jangan lagi menutup pekerjaan dgn "silakan re-run workflow" kecuali
   memang mau re-deploy tanpa commit (mis. setelah mengubah setting
   Pages di Settings, yang tidak menghasilkan commit apa pun).

### 40.26 Fix `EXAM_QID_POINTS` UAS Salah Sumber (`EXAM_CONFIG` dead-code, bukan `OBE_EXAM_CONFIG` live) + Bug Anti-Cheat Opto + Panel Statis 3 Course (BARU di v21, Jul 2026)

Audit menyeluruh 3 halaman `UAS.html` (diminta dosen "pastikan halaman
UAS sudah benar dan siap digunakan", dikerjakan via Workflow multi-agent
dgn verifikasi skeptis 2×) menemukan konstanta client-side
`EXAM_QID_POINTS` (dipakai `getQPoints()`/`SECTION_TOTALS` utk tampilan
poin per soal & breakdown TF/PG/Komp E/H real-time saat mahasiswa
mengerjakan) di-generate dari `EXAM_CONFIG` (dead-code, field
`bobot`/`mapping`-nya tidak pernah dibaca server — lihat komentar
`functions/index.js` baris 91-96) alih-alih `OBE_EXAM_CONFIG` (sumber
LIVE yg dibaca `_examQPoints()`, jadi acuan poin final yg sungguh
tersimpan ke Firestore/RTDB). Akibat: nilai poin per soal yg tampil ke
mahasiswa selama ujian **tidak sinkron** dgn poin sungguhan yg dihitung
server saat submit — bisa beda cukup jauh krn kedua object py grouping
Sub-CPMK yg berbeda (sisa dari sejarah renumbering §40.14-§40.17).

1. **Math4 paling parah**: `EXAM_QID_POINTS` lama HANYA py 42 dari 45
   entri (`c1`/`c2`/`c3` hilang total) — 3 soal computation easy pertama
   tampil TANPA poin sama sekali di layar mahasiswa (fallback ke default
   `getQPoints(qId, isHard?4:2)` shg tampilan salah, bukan crash).
2. **Getaran & Opto**: 45/45 entri ada tapi NILAI-nya salah (proporsi
   antar-soal ikut grouping `EXAM_CONFIG` lama, bukan `OBE_EXAM_CONFIG`
   final CPMK1-5/CPMK3-6 yg berlaku skrg).
3. Fix: regenerate `EXAM_QID_POINTS` ketiga file persis via formula
   `_examQPoints()` (`subTotal = bobot_sub/Σbobot × 100`, lalu dibagi
   proporsional bobot tipe soal TF=1:MC=1:CompEasy=2:CompHard=4 dalam
   sub yg sama) dari `OBE_EXAM_CONFIG` + `OBE_ORDER` (`functions/index.js`
   baris 132-197, 1953-1968) — bukan re-derivasi manual, supaya
   dijamin identik dgn kalkulasi server. Diverifikasi: 45/45 key hadir
   tanpa duplikat & Σ = 100 per exam (Getaran 100.0004, Math4 99.9993,
   Opto 100.0006 — deviasi murni pembulatan 4 desimal, konsisten dgn
   gaya penulisan literal yg sudah ada). `SECTION_TOTALS` otomatis ikut
   benar krn di-derive dari `EXAM_QID_POINTS` (tidak perlu sentuh
   logikanya).
4. **Bug anti-cheat Opto (independen, ditemukan sesi sama)**: friction
   layer di `Optimalisasi-dan-Automasi/Exam/UAS.html` baris ~8006 (blok
   `_frictionLayer` deteksi identitas login utk deterrent anti-AI) pakai
   key localStorage hardcoded `'optoauto_identity_uts'` (harusnya
   `'optoauto_identity_uas'` — file ini UAS, bukan UTS; `LOCAL_IDENTITY`
   yg benar di baris 5895 sudah derive `_uas` via `${MODULE_ID}`, tapi
   salinan hardcoded di friction layer luput ikut-update). Akibat:
   `isStudent()` di friction layer selalu gagal cari identitas
   mahasiswa (key salah → `localStorage.getItem()` return `null`) shg
   deterrent tidak pernah aktif utk mahasiswa asli yg sedang login sah
   di UAS. Fix: ganti literal ke `'optoauto_identity_uas'`. `UTS.html`
   baris 8381 sudah benar (`_uts`), tidak disentuh.
5. **Panel statis "Cara hitung nilai & bobot Sub-CPMK"** (info `<details>`
   di score-bar, 3 course) py kolom "Soal" (Q-range) yg tidak sinkron
   dgn `OBE_EXAM_CONFIG.mapping` sebenarnya — sisa dari mapping lama
   sblm renumbering. Getaran labelnya sendiri (3.3/4.1/4.2/5.1/6.1/6.2/6.3)
   & bobot % sudah benar, cuma range Q yg salah; Math4 & Opto malah
   urutan barisnya sendiri tidak match grouping live. Diperbaiki semua
   3 (termasuk Opto, membalik keputusan sebelumnya di §40.16 poin 4 yg
   sempat terima "biarkan panel Opto apa adanya" — dosen approve
   eksplisit sesi ini utk perbaiki semuanya sekaligus):
   - Getaran: 3.3→Q1-5, 4.1→Q6-11, 4.2→Q12-17, 5.1→Q18-23, 6.1→Q24-30,
     6.2→Q31-40, 6.3→Q41-45.
   - Math4: 3.1→Q41-45(20%), 4.1→Q1-13(2%), 4.2→Q14-30(3%), 5.1→Q31-40(5%).
   - Opto: 3.1→Q31-40(4%), 3.2→Q1-7(3%), 3.3→Q8-14(3%), 4.1→Q15-22(3%),
     4.2→Q23-30(3%), 4.3→Q41-45(4%).
6. **Cosmetic/dokumentasi (tidak butuh redeploy Functions)**: turut
   dirapikan `EXAM_CONFIG["math4-uas"].mapping` di `functions/index.js`
   yg sebelumnya juga bolong (`4.1`/`4.2` tidak mencakup Q31-33) — field
   ini dead-code (poin 3 di atas), jadi perbaikan murni utk konsistensi
   dokumentasi, sama seperti presedan Opto di §40.16 poin 4.
7. Validasi: `node --check` pada seluruh `<script>` inline ketiga file
   (extract via regex → concat → check, pola established) lolos tanpa
   error; `node --check functions/index.js` lolos. Tidak ada perubahan
   ke logika kalkulasi server (`_examQPoints`, `checkExamAnswer`) — bug
   murni di sisi tampilan client (`EXAM_QID_POINTS` konstanta statis),
   jadi TIDAK perlu `firebase deploy --only functions`.

### 40.27 Fix Lebih Dalam: Mapping Sub-CPMK↔Soal UAS Salah (bukan cuma poin tampilan) — ⚠️ BUTUH `firebase deploy --only functions` (BARU di v21, Jul 2026)

Follow-up dosen "pastikan cakupan soal-soal di halaman UAS juga sudah
sesuai" (setelah §40.26 memperbaiki SUMBER poin, giliran memverifikasi
KONTEN soal). Audit 2-tahap via Workflow-style multi-agent (tahap 1:
cocokkan teks tiap 45 soal ke topik RPS resmi per Sub-CPMK; tahap 2:
bangun mapping koreksi dari tag `modul:` riil tiap objek soal — field
ini sudah ada di data soal sejak awal, cuma tidak pernah dipakai utk
grouping Sub-CPMK) menemukan bug JAUH lebih dalam dari §40.26: bukan
cuma SUMBER poin yang salah, tapi PENGELOMPOKAN Sub-CPMK↔nomor-soal itu
sendiri.

**Root cause**: array soal (`UAS_TF`/`UAS_MC`/`UAS_COMP_EZ`/`UAS_COMP_HARD`)
disusun kronologis per Modul (Modul 8→14) di dalam TIAP kategori
tipe-soal, tapi `OBE_EXAM_CONFIG[...].mapping` memotong nomor-soal
gabungan (Q1-45) sbg rentang datar kontigu (mis. "Q1-13"→Sub-CPMK X).
Krn tiap kategori tipe-soal sendiri berputar lewat semua 7 Modul
pasca-UTS, rentang kontigu manapun otomatis mencampur beberapa Modul —
hasilnya cuma **~22% soal per course** (Getaran & Math4, independen,
angka sama persis) yang benar-benar menguji topik Sub-CPMK yang
di-assign ke soal tersebut. **Dampak nyata**: nilai OBE per Sub-CPMK yg
dilaporkan ke mahasiswa (Rekap Dokumen-OBE) selama ini tercampur —
performa soal Modul 14 (mis. Fourier di Math4) bisa masuk skor
Sub-CPMK yang sebenarnya diperuntukkan Modul 8/9.

**Fix diterapkan (Getaran & Math4, dgn persetujuan eksplisit dosen)**:
1. Agent mengekstrak SEMUA 45 pasangan `id`+`modul:` per course
   (bukan sampel), konfirmasi linkage Pertemuan→Sub-CPMK resmi via RPS
   PDF, susun mapping baru dari situ. Sanity check: 45/45 Q-number
   tercakup tepat 1×, tanpa duplikat/gap, utk kedua course.
2. `OBE_EXAM_CONFIG['getaran-mekanik-uas'/'math4-uas'].mapping`
   (functions/index.js — LIVE, dibaca `_examQPoints`) diganti total
   dgn mapping baru; `EXAM_CONFIG` dead-code disamakan jg utk
   konsistensi dokumentasi (presedan §40.16 poin 4).
3. Client `EXAM_QID_POINTS` diregenerate ulang dari mapping baru
   (formula `_examQPoints` yg sama persis dgn §40.26) — 45/45 key,
   Σ=100 tiap exam.
4. Panel statis "Cara hitung nilai" diganti dari kolom "Soal: Qx-y"
   (kontigu, sudah tidak berlaku krn mapping baru non-kontigu) ke
   kolom "Soal: Pertemuan N" (atau rentang Pertemuan, mis. Math4
   Sub-CPMK 3.1 = Pertemuan 9-11) — diverifikasi tiap grup Sub-CPMK
   memang berasal dari Modul yang sama/berdekatan sblm dipakai sbg teks
   tampilan.
5. **Math4 kasus khusus**: Sub-CPMK 3.1 (bobot 20, tertinggi) tetap
   di-mapping ke soal-soal `modul:8` (Pertemuan 9) sesuai linkage
   RESMI RPS (RPS eksplisit label Minggu 9 = Sub-CPMK 3.1), MESKI
   ternyata Modul-8.html sendiri secara konten mengajarkan Undetermined
   Coefficients/Variation of Parameters (materi pra-UTS, bukan topik
   3.1 "reduksi/invers operator") — problem authoring-konten terpisah
   (drift RPS vs modul riil), TIDAK diselesaikan sesi ini, cuma
   didokumentasikan sbg temuan terbuka utk dosen.

**Opto BELUM termasuk fix ini** — audit menemukan masalah lebih rumit
yg tidak bisa diselesaikan lewat remapping saja: Sub-CPMK 3.1 (bobot 4,
topik resmi RPS: FFT/PSD/bearing-fault-frequency, Minggu 7) **tidak
punya satu pun soal yang cocok** di antara 45 soal bank (bank cuma
mencakup modul 8-14, sementara materi 3.1 ada di Minggu 7) — bukan bug
mapping, tapi memang belum pernah ditulis soalnya. Ditambah 12 soal
(`modul:13`/`modul:14`, Logistic Regression & Random Forest/SVM) yg per
RPS resminya Sub-CPMK 5.1/5.2 berstatus "Bentuk: Tugas" (bukan UAS) —
jadi ke-12 soal itu tidak punya rumah Sub-CPMK UAS yang sah. Menunggu
keputusan dosen (opsi didiskusikan: biarkan 3.1 kosong dgn bobot
diredistribusi vs tulis soal baru vs exclude 12 soal "Tugas-leak" dari
mapping OBE).

**⚠️ PENTING — beda dari §40.26**: `OBE_EXAM_CONFIG` adalah kode LIVE
(bukan `EXAM_CONFIG` dead-code) — perubahan mapping ini **BUTUH**
`firebase deploy --only functions` supaya berlaku efektif di grading
production. Sesi ini TIDAK punya akses Firebase CLI/kredensial (kendala
berulang, sudah dicatat sejak §40.16) — dosen/admin lain WAJIB jalankan
deploy manual. Sampai deploy dijalankan, repo (mapping baru) dan Cloud
Functions live (mapping lama) BERBEDA — attempt exam Getaran/Math4 yang
terjadi di rentang waktu ini akan tetap dinilai pakai mapping LAMA.
Setelah deploy, pertimbangkan `recomputeAllObeScores` utk attempt yg
sudah masuk sebelum deploy (skor OBE per Sub-CPMK-nya perlu dihitung
ulang dgn mapping baru).

### 40.28 Opto UAS Selesai: 5 Soal BARU Sub-CPMK 3.1 (FFT/PSD/Bearing Fault) + Exclude 12 Soal "Tugas-leak" — ⚠️ BUTUH `firebase deploy --only functions` + `seed-firestore.js` (BARU di v21, Jul 2026)

Follow-up §40.27 utk Optimalisasi & Automasi. Dosen diberi 2 pertanyaan
via `AskUserQuestion` (Sub-CPMK 3.1 yang 0 soal cocok, dan 12 soal
modul13/14 yang per RPS harusnya "Tugas" bukan UAS) — keputusan:
**(A) tulis soal baru utk 3.1 sekarang** (bukan redistribusi bobot/tunda),
**(B) keluarkan 12 soal Tugas-leak dari mapping OBE** (soal tetap
tampil, tapi tidak dihitung ke breakdown Sub-CPMK manapun).

1. **5 soal BARU ditulis utk Sub-CPMK 3.1** (topik resmi RPS Minggu 7:
   FFT/DFT, magnitude spectrum & PSD/periodogram vs Welch, frekuensi
   karakteristik bearing BPFO/BPFI/BSF/FTF, envelope analysis via
   Hilbert transform) — grounded langsung dari isi riil
   `Optimalisasi-dan-Automasi/Modul/Modul-7.html` (Pertemuan 7, bukan
   direka dari RPS wording saja). Ditulis dgn REPURPOSE 5 dari 12 slot
   soal "Tugas-leak" (bukan menambah total soal ke 46+, supaya TIDAK
   perlu sentuh `OBE_EXAM_ORDER`/`_qTypeWeightByIdx` shared yg dipakai
   SEMUA 6 exam config — precedent replace-in-place, bukan expand):
   `tf8` (DC offset + window function pre-FFT), `mc16` (Δf=fs/N,
   trade-off resolusi vs stasioneritas — distractor zero-padding &
   naikkan-fs-tanpa-ubah-N sengaja masuk krn keduanya trap umum),
   `mc17` (parametric, hitung BPFO dari geometri bearing 6205 riil +
   fr dari NIM, distractor = BPFI/FTF/rumus naif n·fr), `mc18` (kenapa
   envelope analysis dipakai utk bearing fault), `c9` (parametric,
   hitung Δf=fs/N dari fs=25.6kHz & N dari NIM). Diverifikasi numerik
   (`node -e`) di rentang N=0-99: tidak ada NaN/tabrakan nilai
   antar-opsi MC. `modul:` tag diset `7` (bukan 8-14) — jujur reflect
   asal konten, field ini murni dokumentasi/audit, tidak dipakai logic
   grading.
2. **CRITICAL — jawaban benar TIDAK ada di client/repo utk tipe MC/TF**:
   ditemukan saat implementasi bahwa `checkExamAnswer` membaca
   `examAnswers/{examId}/qs/{qId}` dari **Firestore** (bukan
   `functions/index.js`, bukan client `UAS.html`) — client HANYA
   simpan teks soal/opsi/hint, JAWABAN BENAR sengaja dipisah server-side
   demi anti-cheat (mencegah lihat-jawaban via view-source). Source of
   truth Firestore-nya sendiri adalah file repo
   `functions/seed/uas-optoauto-answers.js` (di-seed via
   `node seed/seed-firestore.js --exam optoauto-uas`, WAJIB dijalankan
   admin/dosen setelah merge — sama seperti kendala `firebase deploy`,
   sesi ini tidak punya akses Firestore/Firebase CLI). Ke-5
   entry `tf8`/`mc16`/`mc17`/`mc18`/`c9` diganti persis mirror
   client (rumus `compute(N)` identik, `correctIdx`/`answer`/`expected`
   dihitung dari rumus yang sama).
3. **12 soal Tugas-leak** (`tf9`,`tf10`,`mc19`,`mc20`,`c10`,`c14`,`c15`
   — modul13 Logistic Regression & modul14 Random Forest/SVM, per RPS
   Sub-CPMK 5.1/5.2 "Bentuk: Tugas") dikeluarkan dari
   `OBE_EXAM_CONFIG['optoauto-uas'].mapping` — 5 di antaranya
   di-repurpose jadi soal 3.1 (poin 1), sisa **7 soal**
   (`tf9,tf10,mc19,mc20,c10,c14,c15`) TETAP tampil & bisa dijawab
   (feedback benar/salah jalan normal), TAPI di-set poin **eksplisit 0**
   di client `EXAM_QID_POINTS` — bukan `null`/fallback (1/2/4 default
   `getQPoints`), krn kalau dibiarkan fallback, total achievable score
   exam bisa tembus 114/100 (0-100 scale invariant "Σ poin = 100"
   dipakai di SELURUH UI: score-bar, progress-bar width%, panel info,
   Dokumen-OBE Rekap) — meski ada safety-net `Math.min(pts,
   cfg.totalPoints)` di `recomputeExamPoints` server, LIVE progress bar
   client selama mahasiswa mengerjakan TIDAK dicap sehingga bisa
   nongol "104/100" yg membingungkan. 0-poin eksplisit adalah resolusi
   paling sederhana yg menjaga Σ=100 EXACT tanpa perlu formula split
   baru (mis. "90+10").
4. Mapping final `mapping:{"3.1":[8,26,27,28,39],"3.2":[1,2,11,12,13,
   31,32,41],"3.3":[3,4,14,15,16,33,42],"4.1":[5,17,18,19,34,35,43],
   "4.2":[6,20,21,22,36,37],"4.3":[7,23,24,25,38]}` — 38 Q ter-mapping
   (tanpa duplikat/overlap, diverifikasi terprogram), 7 sengaja tidak
   dipetakan ke Sub-CPMK manapun. `bobot` TIDAK berubah (Σ=20, semua 6
   Sub-CPMK 3.1-4.3 tetap ada bobotnya, cuma sekarang 3.1 benar-benar
   py soal). `EXAM_CONFIG` dead-code disamakan (konsistensi dok).
5. `EXAM_QID_POINTS` client diregenerate ulang (38 nilai dari formula
   `_examQPoints`, 7 eksplisit 0) — Σ=100 tepat pada 38 soal
   ter-mapping, diverifikasi 45/45 key hadir tanpa duplikat.
6. Panel statis "Cara hitung nilai" & `Banner-UAS.html` (materi chip +
   subtitle) diupdate: Sub-CPMK 3.1 sekarang tampil **"Pertemuan 7"**
   (bukan kosong/salah), materi chip jadi "Pertemuan 7, 9–15" — RPS
   sendiri MEMANG sengaja menguji Minggu 7 (pra-UTS secara nomor file)
   di UAS (bukan UTS), desain resmi bukan bug.
7. Validasi: `node --check` pada `functions/index.js`,
   `functions/seed/uas-optoauto-answers.js`, dan seluruh `<script>`
   inline `UAS.html`; sanity check terprogram 38 Q unik tanpa overlap;
   45/45 key `EXAM_QID_POINTS` hadir, Σ=100 pada bagian ter-mapping.

**⚠️ PENTING — 2 langkah manual WAJIB dijalankan admin/dosen sebelum
efektif live** (sesi ini tidak py akses ke keduanya):
  1. `firebase deploy --only functions` (`OBE_EXAM_CONFIG` live).
  2. `node seed/seed-firestore.js --exam optoauto-uas` dari
     `functions/` (isi jawaban benar 5 soal baru ke Firestore
     `examAnswers/optoauto-uas/qs/`) — **tanpa langkah ini, ke-5 soal
     baru akan selalu gagal disubmit** (`checkExamAnswer` throw
     `not-found: Answer key for {qId} not configured"`).
  Urutan tidak masalah, tapi keduanya WAJIB sebelum mahasiswa
  mengerjakan UAS Opto (Kamis, 30 Juli 2026 · 19:30 WIB per
  `Banner-UAS.html`). Disarankan jalankan `--dry-run` dulu utk preview
  seed.

### 40.29 Teks Soal UAS Server-Gated (bukan cuma jawaban) — ⚠️ BUTUH `firebase deploy --only functions` (BARU di v21, Jul 2026)

Follow-up dosen: "pastikan mahasiswa tidak bisa login ke halaman UAS
jika waktunya belum tiba" (terkonfirmasi sudah benar — `submitVisitor()`
block via `now < schedStart` SEBELUM verifikasi PIN, kontras
`submitDosenLogin()` yg sengaja tanpa gate) diikuti pertanyaan lanjutan
lebih tajam: "perkuat supaya **teks soal** tidak kebaca sebelum jadwal
buka". Audit menemukan celah nyata — kunci jawaban MC/TF sudah lama
server-only (Firestore `examAnswers/{examId}/qs/{qId}`, §40.28 poin 2),
TAPI teks soal sendiri (`window.UAS_TF`/`UAS_MC`/`UAS_COMP_EZ`/
`UAS_COMP_HARD`, ~18KB per file) masih ter-embed statis di `<script>`
`UAS.html` — bisa dibaca penuh via View Source/DevTools KAPAN SAJA,
independen dari status login/jadwal, krn browser menerima full page
source unconditionally saat `page load`. Prinsip yg dipakai: **konten
yang tidak boleh terlihat sebelum otorisasi TIDAK BOLEH pernah dikirim
ke client saat page-load** — harus difetch via call ber-autentikasi
SETELAH otorisasi sukses (obfuscation apa pun utk konten statis
fundamentally defeatable, cuma network-gating yang benar-benar menutup).
Diberi pilihan via `AskUserQuestion` (kerjakan sekarang vs setelah UAS
Getaran 29 Jul) — dosen pilih **kerjakan sekarang, utk SEMUA 3 course
sekaligus** (bukan pilot 1 course dulu), meski UAS Getaran tinggal 2
hari lagi.

1. **Callable BARU `getExamQuestions`** (`functions/index.js`, pola
   identik `checkExamAnswer`/`getMyObeNilai`) — dual-auth: **mahasiswa**
   (nim+pinHash divalidasi ke RTDB `pins/mhs_<nim>`, DAN
   `evalSchedule(rtdb, cfg.schedulePath, cfg.lateMultiplierValue)` HARUS
   `isOpen` — window jadwal PERSIS SAMA dgn yg dipakai `checkExamAnswer`
   utk submit, `EXAM_CONFIG` yg sama) ATAU **dosen** (ID token Firebase dengan
   custom claim `admin: true`, bypass jadwal — utk preview/monitoring, sama spt
   `submitDosenLogin()` client). Soal parametric di-`compute(N)` SERVER-
   SIDE (`N` dari `deriveN(nim)` mahasiswa atau `N=0` dosen) — client
   TIDAK PERNAH menerima source `compute()` mentah, cuma hasil akhir
   text/options/hint yang sudah personalized per NIM. Response TIDAK
   mengandung correctIdx/answer/expected (field itu memang tidak pernah
   ada di bank soal ini — kunci jawaban tetap terpisah total di
   `examAnswers` Firestore, §40.28).
2. **Bank soal dipindah ke server**: 3 file baru
   `functions/questions/uas-<getaran|math4|optoauto>-questions.js`
   (`UAS_TF`/`UAS_MC`/`UAS_COMP_EZ`/`UAS_COMP_HARD`, tanpa kunci
   jawaban) — hasil **ekstraksi VERBATIM** (bukan tulis ulang) dari
   `<script>` `UAS.html` masing-masing, supaya dijamin byte-identik dgn
   yg dulu ada di client (menghindari risiko transkripsi ulang formula
   parametrik yang kompleks, mis. BPFO/BPFI bearing, eigenvalue 2-DoF,
   Δf FFT). Diverifikasi via `require()` penuh + render sample question
   (`c9`/`mc17` Opto) cocok dgn output yg diharapkan.
3. **Client `UAS.html` (3 file) di-rewire**: blok
   `const UAS_TF = [...]` s/d `window.UAS_COMP_HARD = ...;` (~18KB)
   DIHAPUS total, diganti komentar penjelasan. 8 call site
   `.compute(N)` → `const data = q;` (data sudah pre-computed dari
   server, bukan dihitung ulang di client). Fungsi baru
   `_ensureUASQuestionsLoaded()` (idempotent, self-guarding via
   `window._uasQuestionsLoading` + `window._utsRenderedFlag`) memanggil
   `getExamQuestions`, mengisi `window.UAS_TF` dst. dari response, lalu
   trigger `renderUASQuestions()` yang sudah ada — dipanggil dari
   `saveIdentity()` (login sukses), `_setSessionPinHash()` (PIN
   re-entry/Ganti Peran §39.10), DAN safety-net init di akhir file
   (returning visitor dgn identity+PIN cached). Loading state
   ("⏳ Memuat soal UAS...") & error state (pesan server, mis.
   "🔒 Akses UAS belum dibuka — tunggu waktu mulai" utk
   `failed-precondition`) dirender via `_showUASLoadState()` — gagal
   fetch TIDAK crash halaman, cuma tampil pesan di `container-tf`.
4. **GOTCHA scoping non-module vs module `<script>`**: `UAS.html`
   punya 2 `<script>` blok terpisah — plain `<script>` (berisi
   `renderUASQuestions`, `let _utsRendered`) vs
   `<script type="module">` (berisi `saveIdentity`, `EXAM_ID`,
   `_checkExamAnswerCallable`, PIN-session state, tempat
   `_ensureUASQuestionsLoaded` ditambahkan). Deklarasi fungsi top-level
   plain-script auto-attach ke `window` (perilaku standar JS), tapi
   `let`/`const` TIDAK — jadi probe `_utsRendered` dari script module
   WAJIB pakai `typeof _utsRendered !== 'undefined'` (bukan referensi
   langsung, akan throw `ReferenceError` kalau scope beda) sbg guard
   defensif, sedangkan pemanggilan fungsi lintas-script tetap via
   `window.renderUASQuestions` yg sudah di-bridge existing.
5. **Verifikasi via Playwright** (file://, Firebase CDN di-stub lokal
   via `page.route` — pola reusable dari §40.9): 3 skenario × Getaran
   (student happy-path fetch+render, dosen dengan Firebase admin claim+bypass-jadwal,
   error `failed-precondition` graceful) SUKSES, ditambah smoke test
   student happy-path Opto & Math4 (payload+render benar, 0 console
   error dari kode baru). **GOTCHA Playwright BARU**: `page.route()`
   dgn glob pattern (`"**/*gstatic.com*"`) SILENTLY GAGAL intercept di
   environment sandboxed sesi ini (request tetap ke network asli, gagal
   `ERR_TUNNEL_CONNECTION_FAILED` — bahkan dgn `proxy:{server:
   'direct://'}` eksplisit) — fix: pakai regex (`re.compile(r"gstatic\.com")`)
   sbg pattern, yang bekerja normal.
6. Struktural: `grep` konfirmasi 0 sisa `const UAS_TF = [` dan 0 sisa
   `.compute(N)` di ke-3 file, dan bare (non-`window.`) reference ke
   `UAS_TF`/dst. cuma di komentar (bukan kode). `node --check` lolos utk
   `functions/index.js`, ke-3 file `functions/questions/*.js`, dan
   seluruh `<script>` inline ke-3 `UAS.html`.

**⚠️ PENTING — beda dari §40.28**: fix ini HANYA butuh
`firebase deploy --only functions` (`functions/questions/*.js`
otomatis ikut packaged, tidak ada exclude di `firebase.json`/
`.gitignore` functions) — **TIDAK butuh Firestore reseed**, krn tidak
ada perubahan kunci jawaban/data Firestore, murni penambahan 1 callable
baru + relokasi teks soal (yang memang bukan data rahasia server,
cuma harus dikirim network bukan baked statis).

**Follow-up langsung setelah live**: dosen lapor kotak error "🔒 internal"
saat preview UAS pasca-deploy pertama. `getExamQuestions` dibungkus
try/catch (`HttpsError` eksplisit tetap passthrough apa adanya; exception
tak terduga di-log via `console.error` + dikirim ke client sbg pesan
spesifik, bukan "internal" polos dari Firebase). Setelah redeploy #2,
dosen konfirmasi via DevTools Network tab: request `getExamQuestions`
`200 OK` dgn body soal lengkap & benar — membuktikan ini kegagalan sesaat
cold-start invocation PERTAMA function yang baru dibuat (self-resolve
setelah retry/reload, bukan bug logika); try/catch tetap dipertahankan
sbg jaring pengaman observability ke depan.

### 40.30 Tombol Logout di Halaman UAS (BARU di v21, Jul 2026)

Follow-up dosen: "Tambahkan tombol logout (masuk ke halaman login) pada
halaman UAS." UAS/UTS **sengaja tidak diberi** tombol keluar-sesi sejak
§39.10 ("Ganti Peran" — Modul only, "Exam TIDAK diberi tombol ini —
sesi ujian terkunci, keputusan terpisah") — permintaan ini membalik
keputusan itu secara eksplisit, scoped ke UAS (bukan UTS, tidak diminta).

1. Diterapkan identik ke ke-3 course (Getaran/Math4/Opto), mengikuti
   idiom `_switchRole()` Modul (`clear identity → reload → role picker
   muncul lagi` — mekanisme sama yg dipakai force-logout schedule-gate
   di Exam, cukup krn `#roleChooserOverlay` default TIDAK `.hidden`
   di DOM; cuma disembunyikan JS kalau `getIdentity()` non-null saat
   `DOMContentLoaded`). Beda dari Modul: UAS py layer PIN-session
   tambahan (`sessionStorage` + `window._sessionPinHash` in-memory,
   Modul tidak py ini) — `_logoutUAS()` clear ketiganya (localStorage
   identity, sessionStorage PIN hash, `window._sessionPinHash`) supaya
   logout benar-benar bersih, bukan cuma identity.
2. Tombol `🚪 Logout` (`.nav-logout`, styling merah/danger reuse warna
   `.nav-switch-role` Modul) ditaruh di navbar tepat sebelah badge
   identitas — `confirm()` dulu sebelum clear (teks eksplisit "...sesi
   ujian ini..." biar mahasiswa sadar ini exit dari ujian, bukan cuma
   ganti tampilan).
3. **Bonus fix sejalan** (bukan diminta eksplisit, tapi diperlukan biar
   tombol logout masuk akal): badge identitas navbar UAS SEBELUMNYA
   statis hardcode `"👨‍🏫 DEDIK ROMAHADI"` utk SEMUA role (mahasiswa
   yang login pun melihat teks itu) — beda dari Modul yg sudah py chip
   dinamis via `id="navIdentityChip"`. Ditambahkan id yang sama +
   hook update-text di `_applyRoleVisibility()` (dipanggil di 7+ titik
   existing: initVisitor, post-login mhs/dosen, force-logout, dst. —
   tidak perlu titik panggil baru). Efek samping: kode mati
   `enterPreviewMode()` yg sudah lama query `#navIdentityChip` (Preview
   Mode §39.11) otomatis jadi hidup tanpa disentuh — 1 fix, 2 celah
   ditutup.
4. Validasi: `node --check` lolos ke-3 file; grep struktural konfirmasi
   `_logoutUAS`/`navIdentityChip`/`.nav-logout` muncul persis sesuai
   hitungan yang diharapkan (2/1/2) di ke-3 file, byte-identical diff
   (21 baris tiap file); Playwright end-to-end (Getaran): chip berubah
   dinamis sesuai identity tersimpan, klik tombol → `confirm()` →
   `location.reload()` (di-`expect_navigation` biar tidak race) →
   localStorage identity `null` + `#roleChooserOverlay` kembali
   terlihat — 0 error dari kode baru.
5. Client-only — **TIDAK butuh deploy Functions/Firestore apa pun**,
   cukup merge ke `main` (auto-publish via GitHub Pages, §40.25).

### 40.31 Hardening Backend Privat dan Rotasi Bank UAS (BARU di v22, Jul 2026)

Arsitektur keamanan terkini menggantikan mekanisme hash admin di browser dan
memisahkan seluruh backend dari repositori publik:

1. Cloud Functions, Firestore/RTDB rules, seed, bank soal, dan kunci jawaban
   berada di repositori privat `Mechanical-Engineering-Courses-Backend`.
2. GitHub Pages dibangun dengan allowlist frontend. Workflow wajib gagal jika
   menemukan backend, bank soal, service account, atau pola kredensial.
3. Login dosen memakai `createAdminSession` dan Firebase custom token dengan
   claim `admin: true`; sesi maksimal dua jam dan login diberi rate-limit.
4. Write jadwal/reset yang sensitif memerlukan claim admin di RTDB Rules atau
   `_requireAdmin(request)` pada callable. Hash browser tidak diterima lagi.
5. Bank UAS v2 berisi 45 soal baru per mata kuliah (10 TF, 20 MC, 10 komputasi
   mudah/menengah, 5 komputasi hard), semuanya parametrik untuk `N=0..99`.
...
*Update v21: §15.5 BARU — ekstraksi jawaban komputasi multi-layer (`_lineAnswers` per-baris first+last, bracket-strip, field `lineAnswers` terpisah dari `userAnswers`); §36.12 BARU — Kode Verifikasi HMAC utk Export HTML 48 file (server-authoritative points, secret di Secret Manager, `Admin/verify-export-code.html`); §25.19–25.20 BARU — PERMISSION_DENIED stale-set() vs update() sparse + EXAM_CONFIG field hilang tertimpa refactor (CRITICAL, restore PR #627); §39.10 BARU — Ganti Peran + chip identitas dinamis + auto PIN re-verify; §39.11 BARU — Preview Mode tanpa login (48 file, soal non-interaktif via `window._previewGuard`, tidak pernah memanggil callable server, poin & export dinonaktifkan); §24 cakupan chat 42/42 (2 Modul-4 outlier di-port); §38.7 tambah `generateExportCode`/`verifyExportCode` + `rescaleModulLatePenalty` NIM-scoping; §40 BARU — Modul Word & PPT template BOP Kurikulum 2025 (WAJIB mulai dari file template `Template-Modul-Word-dan-PPT/`); §40.4 BARU — pelajaran revisi Modul 1 Opto: fix bug urutan child `w:pPr` (pBdr/shd sblm spacing, WAJIB `validate.py`), LibreOffice headless nonfungsional total di environment sesi (verifikasi via validate.py+pandoc+introspeksi python-pptx, bukan render visual), gambar wajib bernomor+caption+sitasi teks, link versi Preview sebelum Pendahuluan, Tugas disalin persis modul HTML, PPT layout 2-kolom teks/gambar + title auto-fit 1-baris (PIL DejaVu Sans Bold proxy) + verifikasi no-overlap terprogram, Kode MK Optoauto `P132520004` terkonfirmasi; §40.5 BARU — simulasi pagination Word tanpa render visual (`simulate_pages.py`, heuristik PIL+Liberation Sans, `docx-preview` npm terbukti BUKAN solusi krn `breakPages` cuma baca explicit page-break), gotcha `bbox_inches='tight'` matplotlib tak 1:1 dgn `figsize` diminta (ukur pixel aktual baru embed), strategi reorder-dulu-baru-tambah-teks utk cegah ruang kosong; §40.6 BARU — tabel bernomor dgn caption DI ATAS tabel (kebalikan gambar, `make_table(..., caption=)` + `_TABEL_COUNTER`) + wajib disitasi teks, riset Daftar Pustaka via `WebSearch` (bekerja walau `WebFetch` langsung ke doi.org/mdpi.com/pmc.ncbi.nlm.nih.gov diblok proxy 403), aturan "≤10 tahun" berlaku ke SELURUH Daftar Pustaka bukan cuma minimal-5-jurnalnya (ganti paper fondasional tua dgn paper modern yang me-review/menerapkan metode yang sama), **koreksi §40.5**: simulator pagination heuristik TERBUKTI TIDAK AKURAT memprediksi lokasi/besar celah nyata (dosen laporkan gap di lokasi yang tidak diprediksi simulator sama sekali) — jangan percaya angka simulator sbg final, percaya lokasi yang dilaporkan dosen & tambal generous (bukan pas-pasan), checklist audit overlap/clipping per-gambar (legend bbox_to_anchor, donut label 1-baris, diagram kotak compact), PPT reuse PNG dari `figs/` Word apa adanya via `add_picture_fit` scale-to-fit (kecuali roadmap/analogi pakai varian `_vertical` dgn font hardcoded terpisah dari `SZ_*` global, wajib update manual), Daftar Pustaka Word/PPT sinkron manual; §40.7 BARU — **koreksi kritis §40.4/§40.5/§40.6**: LibreOffice headless TERNYATA bisa jalan, akar masalah cuma paket `libreoffice-writer` belum terpasang (fix: `apt-get install libreoffice-writer poppler-utils`), render+verifikasi visual sungguhan jadi mungkin utk pertama kali; tooling baru `analyze_gaps.py` (rasterize PDF + row-darkness analysis) + `find_culprits2.py` (pdftotext caption-matching) menggantikan `simulate_pages.py` yang tak akurat; strategi `max_h_cm` override per-instance (bukan cap global) ditarget dari gap terukur; redesign donut "Distribusi Poin Tugas" jadi horizontal (turun ~15-21cm → ~7,3cm, sumber gap terbesar di hampir semua modul); tabel dipadatkan global (`tcMar` tipis + `line_spacing=1.0`); hasil 13 modul (P2-15) sblm koreksi §40.8: total gap turun ±169cm→±57cm (-66%), 4/13 modul 0 gap; §40.8 BARU — **koreksi kritis**: dosen tolak strategi kecilkan gambar ("teks di gambar jadi terlalu kecil dan tidak terbaca", echo instruksi lama §40.5 poin 4 yg sempat terlanggar) — hapus SELURUH 17 override `max_h_cm` per-instance, `MAX_IMG_H_CM` kembali ke 12,5cm generous safety-net, ganti strategi ke alokasi/tambah teks substantif (worked example, studi kasus) tepat sebelum blok gambar/tabel yg lompat halaman (bukan mengecilkan blok itu) — jauh lebih predictable drpd shrink krn tidak mengubah titik lompat scr non-linear; hasil akhir 10/13 modul 0 gap sama sekali, sisa 3 modul cuma 1 gap residual kecil (3,5-3,9cm) each, total gap ke-13 modul turun jadi ±10,9cm (-94% dari baseline), TANPA satupun gambar dikecilkan di bawah ukuran natural; §40.9 BARU — tombol Export Modul ke PDF (dari Word, pre-rendered via LibreOffice, bukan konversi dinamis), gated login via hook baru di `_applyRoleVisibility()` existing (aktif utk mahasiswa MAUPUN dosen), path PDF relatif sejajar `.docx` sumbernya, verifikasi via Playwright dgn Firebase CDN di-stub lokal (`page.route`) krn sandbox tanpa akses network ke gstatic.com — pola stub reusable utk QA modul HTML lain; pilot Modul 1 Opto lalu digeneralisasi ke SELURUH 14 modul Opto via 1 script Python batch-replace (gotcha nomor Pertemuan vs nama file "post-UTS shift" tidak mempengaruhi mapping PDF krn ikut nomor `Modul-N.html`); §40.10 BARU — 28 modul Word BARU (14 Getaran Mekanik kode `W132500020` + 14 Matematika 4 kode `W132500014`) dibangun via Workflow multi-agent (28 agent paralel, dosen setuju eksplisit krn skala besar), `word_lib.py` digeneralisasi lintas-course (`set_cover()` param `course_name` baru, satu-satunya string Opto-spesifik yg tersisa), session-limit mid-run di-handle via `resumeFromRunId` (17 modul sukses replay cache, 11 gagal jalan ulang), checkpoint-commit incremental per progress-check (bukan 1 commit akhir) berdasar `journal.jsonl`, hasil 28/28 sukses dgn fase verifikasi final menutup 3 gap terbesar (Math4-10: 20,7cm→4,9cm, Getaran-3: 8,6cm→0cm, Getaran-12: 7,4cm→0cm) hingga 24/28 modul nol gap; Export PDF (§40.9) digeneralisasi ke SEMUA 42 modul HTML (Opto+Getaran+Math4) — gotcha pola `LOCAL_IDENTITY` BEDA per course (literal prefix vs `${COURSE_ID}` runtime), wajib grep pola sebenarnya sblm QA manual, bukan asumsi seragam; §40.11 BARU — fix overlap header/footer halaman 1 (cover) di SEMUA 42 modul: template py `different_first_page_header_footer=False` shg footer badge tahun ajaran nge-overlap bar info cover ("2025-2028kultas" = "2026" tabrakan "Fakultas"), fix set `different_first_page_header_footer=True` + `first_page_footer.is_linked_to_previous=False` di `word_lib.init()` (first_page_header ikut kosong otomatis, efek samping OOXML standar yg diinginkan), diterapkan ke 42 docx final via patch struktural langsung (bukan rebuild, nol risiko ke konten yg sudah diverifikasi), page count tiap file identik persis sblm/sesudah (murni properti section, nol dampak body/pagination); §40.12 BARU — audit sitasi prosa "Gambar N"/"Tabel N" thd caption di 42 modul via Workflow 42-agent paralel (FIND-ONLY, cross-check topik sitasi vs topik caption bernomor sama + tetangga N±1, bukan sekadar cocokkan angka), root cause: caption SELALU benar (auto-counter) tapi kalimat prosa rujukan ditulis manual terpisah shg bisa salah hitung — ditemukan 10 mismatch high-confidence di 3/42 modul (Getaran M1 4×, Opto M3 3×, Math4 M8 3×, semua kelas Gambar, nol Tabel; 39 modul lain bersih), fix mekanis via python-docx run-level text replacement (bukan agent lanjutan, sitasi terbukti selalu utuh dlm 1 run), page count PDF ke-3 modul identik sblm/sesudah fix; §40.13 BARU — audit Sub-CPMK modul Word thd RPS resmi 3 course (RPS PDF baru di-upload dosen ke `Attributes/RPS <Nama MK>.pdf`): ditemukan Opto py DUA katalog Sub-CPMK berbeda pengelompokan (RPS PDF CPMK1-4/4-4-4-2 vs `Asesmen-Optimalisasi-dan-Otomasi.json` SSOT grading CPMK1-5/3-3-3-3-2, 14 kalimat sama persis cuma geser slot; modul Word ikut JSON) — TIDAK diputuskan sepihak, perlu keputusan dosen; 3 fix mekanis diterapkan (Getaran M3 CPMK-induk salah 1→3, Opto M1 deskripsi duplikat dgn M3 diganti ke teks Sub-CPMK 1.1 yg benar hingga 14 modul jadi bijeksi bersih, Math4 M13 kehilangan suffix (CPMK 4)); 3 temuan konten Getaran (M6/M10/M12: kode+CPMK-induk benar tapi topik riil beda dari makna resmi kode, mis. M6 berlabel "getaran bebas tanpa redaman" tapi isinya getaran paksa base-motion/rotating-unbalance) di-flag utk keputusan dosen, TIDAK di-reassign sepihak krn menyangkut desain kurikulum; §40.14 BARU — keputusan dosen: Opto direnumber ke grouping RPS PDF (bukan JSON), scoped ke modul Word saja (Asesmen JSON grading TIDAK disentuh sesi ini, jadi utk sementara modul Word & sistem grading py grouping CPMK Opto yg berbeda), reverse-mapping via content-match 14 kalimat (M1-M3 kode sama krn RPS/JSON sepakat utk 3 kalimat pertama, M4-M14 semua bergeser kode), GOTCHA preview-box terpisah (`w:txbxContent`, di luar `doc.paragraphs`) yg menduplikasi ringkasan Sub-CPMK tanpa suffix CPMK — WAJIB diakses via `doc.element.body.iter(qn('w:t'))` raw XML, retroaktif juga menemukan preview-box Modul-1 masih py teks salah lama dari §40.13 yg terlewat, verifikasi akhir scan SELURUH w:t node (bukan cuma 40 paragraf cover) mengonfirmasi bijeksi 1:1 bersih thd 14 entri RPS PDF di seluruh dokumen, page count 14 modul identik sblm/sesudah; §40.15 BARU — keputusan dosen: Getaran Modul-6/10/12 direassign ke Sub-CPMK modul terkait via reuse kode (M6 2.1→3.2 gabung M5, M10 2.2→4.2 gabung M9, M12 6.2→5.1 gabung M14) setelah 3 opsi diajukan via AskUserQuestion dgn rekomendasi eksplisit (rasional: transmissibility diturunkan dari base motion/rotating unbalance = topik sama dgn M5; vibration absorber = sistem 2-DoF didesain dari analisis modal M9; indikator resmi Sub-CPMK 5.1 di RPS eksplisit sebut "data RMS dan nilai Peak" match M12), dosen approve ketiganya tanpa modifikasi; GOTCHA preview-box Getaran BEDA format dari Opto (Getaran pakai frasa RPS resmi terse, Opto pakai klausa custom modul) — string exact-match utk fix WAJIB diverifikasi ulang per-course, jangan asumsi format seragam; scope dibatasi hanya 3 modul diminta, 11 modul lain (termasuk M4/M5/M9/M14 yg kini berbagi kode) tidak disentuh teksnya, page count ke-3 modul identik sblm/sesudah; §40.16 BARU — follow-up dosen: sistem grading Opto (bukan cuma modul Word) direnumber ke grouping RPS PDF, audit menemukan 6 lokasi independen yg py salinan/turunan katalog Sub-CPMK lama (Asesmen JSON, OBE_EXAM_CONFIG server — SATU-SATUNYA yg benar dibaca _examQPoints, OBE_MAP/OBE_TUGAS_COLS fallback Opto-only, EXAM_CONFIG.bobot/mapping dead-code tapi direnumber demi konsistensi dokumentasi, 7 struktur data Dokumen-OBE.html + s3 table header/render slice, doc panel statis UTS/UAS.html), CPL/CPMK RPS terkonfirmasi bersih 1:1 (CPL3-6→CPMK1-4) kontras T4 lama yg split CPL3 antar 2 CPMK, bobot CPL institusional (43/21/22/14) DIPERTAHANKAN independen dari sub-weight breakdown internal (27/30/29/14) krn beda makna, arithmetic 100% diverifikasi via Node.js eval bukan asumsi; 2 hal WAJIB tindak lanjut manual dosen tak bisa diselesaikan dari sesi ini: (1) `firebase deploy --only functions` blm dijalankan (sesi tanpa Firebase CLI/kredensial) shg repo vs live BERBEDA sementara, (2) data OBE sudah-publish dgn kode lama (jika ada) perlu recomputeAllObeScores/republish setelah deploy, sesi tanpa akses Firestore utk cek/migrasi; §40.17 BARU — pembalikan penuh keputusan §40.14/§40.16: dosen kirim 6 screenshot Dokumen-OBE.html yg tunjukkan skema CPMK1-5 (3/3/3/3/2) lama sbg acuan benar, 6 lokasi live-system + 14 modul Word dikembalikan byte-exact via git-history diff-reverse (bukan re-derivasi manual), RPS PDF (bukan sistem) yg diedit via hybrid PyMuPDF redact+reinsert (tabel mingguan in-place) + python-docx rebuild+splice (tabel CPL/CPMK 4→5 baris); §40.18 BARU — RPS Opto dibangun ulang total sbg `.docx` editable native (bukan lagi hybrid redact/splice PDF) setelah dosen hapus PDF lama & upload versi convert-apa-adanya yg ternyata masih generik/tak sinkron: konten 16-minggu direset dari riset ulang 14 modul Word, 2 bug sinkronisasi ditemukan & fix sebelum commit (bobot per-minggu drift dari SSOT Asesmen JSON 92%→100%; split UTS/UAS 7+7 hardcode vs 5+6 riil di JSON, 3 Sub-CPMK murni Tugas tanpa exam), page-number field asli (`PAGE` fldChar) dgn Catatan restart nomor dari "1", checkpoint-per-tahap (7 stage, render+baca visual tiap stage) sebelum lanjut; §40.19 BARU — RPS Opto (file dosen sendiri) diedit in-place (bukan rebuild) via unzip+merge_runs+lxml+validate.py, gotcha python-docx (`Row.cells[i]` auto-resolve vMerge master shg salah tulis di continuation row, rPr hilang saat promote continuation→restart cell, `w:hyperlink` invisible ke `p.runs` shg `add_run()` dobel teks, auto-numbered list dobel nomor kalau clone paragraph numbered sbg template, `Table(tbl,None)` gagal di `add_picture()` krn parent chain), verifikasi cross-check 6 screenshot dosen (2 mismatch ditemukan+fix), 3 tanda tangan Otorisasi diekstrak dari base64 JSONL transcript (pasted image tidak tersimpan di filesystem) + fix mapping caption-to-image yg sempat salah, centering+enlarge TTD Dosen Pengemban RPS; §40.20 BARU — `Unduhan-Gabungan/` 9 PDF gabungan (Modul 14×+Exam UTS/UAS+RPS per 3 course) link GitHub raw persisten, gotcha sort numerik vs lexicographic nama file Modul-N, kompresi Ghostscript custom 75dpi (bukan preset) utk penuhi constraint ≤5MB tanpa degradasi visual chart; §40.21 BARU — `docs/` situs GitHub Pages utk preview PDF standalone (bukan raw.githubusercontent.com yg force-download+X-Frame-Options:deny, bukan github.com/blob yg kebungkus UI penuh), discope ke folder `docs/` saja (bukan root branch) demi cegah 42 modul HTML live ikut ter-publish sbg situs paralel tak sengaja, setup Pages di Settings perlu 1× aksi manual dosen krn tidak ada tool API utk itu di sesi ini; **§40.21b KOREKSI KRITIS §40.21 — resep `/docs` SALAH & MERUSAK SITUS**: repo TERNYATA sudah py Pages aktif via GitHub Actions (`deploy-slides.yml`, sejak Jun 2026) yg mem-publish SELURUH root repo, dan itulah hosting LIVE seluruh LMS (link Preview tiap modul + **`STUDENTS_JSON_URL` roster login mahasiswa** di-fetch dari sana; `firebase.json` TIDAK py key `hosting` — Firebase cuma backend), shg mengubah Source ke "Deploy from a branch → /docs" bikin SEMUA path di luar `docs/` jadi 404 (dosen lapor "modul tidak bisa diakses, 404" utk 42 modul) — fix: kembalikan Source ke "GitHub Actions"; konsekuensi: `docs/` jadi redundan krn `Unduhan-Gabungan/*.pdf` sudah bisa diakses langsung dari root — **folder `docs/` akhirnya DIHAPUS (10 file) atas persetujuan dosen, link kanonik kini `.../Unduhan-Gabungan/<file>.pdf`, link `.../docs/pdf/...` sudah mati**; PELAJARAN: `ls .github/workflows/` + `grep -rn "github.io" .` DULU sebelum menyentuh konfigurasi hosting, jangan simpulkan "belum ada Pages" dari ketiadaan dokumentasi; §40.22 BARU — TTD ditambahkan ke 6 exam Word (UTS/UAS × 3 course, tabel VERIFIKASI SOAL UJIAN, signer sama dgn 2/3 pihak Otorisasi RPS jadi PNG di-reuse), gotcha row-height overflow di LibreOffice PDF export (gambar lebih tinggi dari teks placeholder bikin baris tabel kepecah lintas halaman, nama signer "yatim" muncul sendirian di halaman berikutnya — fix: hapus paragraf kosong padding, BUKAN naikkan trHeight yg terbukti tak mempan krn batasnya sisa ruang halaman bukan tinggi minimum baris), slack halaman beda per file shg lebar TTD final (3,0cm seragam, ukuran sama persis sesuai request dosen) dipilih dari file terketat (UAS Getaran Mekanik) via binary search empiris, 2 lokasi combined PDF (Unduhan-Gabungan + docs/pdf) diregenerate ulang; §40.23 BARU — fix tombol copy kode Python (`cpC()`) di 42 modul: 2 bug independen (selector `.closest('.code-wrap')` gagal utk struktur tab Setup Modul-1 `.sp-cb`/`sp-cbh-copy` → throw `null.querySelector`, TIDAK ada fallback clipboard spt Forum copy shg gagal senyap di browser/WebView yg blokir Clipboard API), direproduksi & diverifikasi fix via Playwright headless (file://, DOM+JS terisolasi tanpa load Firebase), batch-replace 42 file dgn guard byte-identik (0 mismatch) + `node --check` tiap `<script>` inline; §40.24 BARU — `PDD-UKTPT/Video/index.html` cadangan video via GitHub Release asset (bukan git commit/LFS, batas 100MB/push tidak berlaku ke Release, 2GB/file, tanpa kuota bandwidth ketat LFS), toggle YouTube↔GitHub tanpa preload video besar, verifikasi `Accept-Ranges: bytes` (streaming/seek tanpa unduh penuh) via redirect target S3, gotcha `.gitignore` directory-exclude memblokir file baru di folder yg sama walau bukan video (fix `dir/*` + negasi eksplisit per-file); follow-up "lama menunggu videonya jalan" ternyata BUKAN soal range-request tapi lokasi `moov atom` di AKHIR file sumber (umum utk hasil rekam Zoom/OBS) — fix via `ffmpeg -c copy -movflags +faststart` (remux murni lokal dosen, zero quality loss, cepat) lalu halaman disederhanakan total (toggle+YouTube dihapus, tinggal 1 `<video>` langsung ke asset GitHub); §40.25 BARU — deploy Pages otomatis: `deploy-slides.yml` dulu cuma `workflow_dispatch` shg tiap merge/upload perlu klik Run workflow manual & sering terlupa, ditambah trigger `push: branches:[main]` (manual tetap ada utk re-deploy tanpa commit), diverifikasi empiris merge via GitHub App token TETAP memicu `push` workflow (aturan "GITHUB_TOKEN tidak memicu workflow" hanya berlaku utk token DI DALAM workflow) — run #27 `event: push` vs 25 run sebelumnya semuanya manual; konsekuensi: instruksi lama "silakan re-run workflow setelah merge" di §40.21/§40.24 dst. TIDAK berlaku lagi; §40.26 BARU — audit dosen "pastikan UAS sudah benar" (Workflow multi-agent, verifikasi skeptis 2×) menemukan `EXAM_QID_POINTS` client 3 course di-generate dari `EXAM_CONFIG` dead-code alih-alih `OBE_EXAM_CONFIG` live (Math4 bahkan hilang 3 entri `c1`/`c2`/`c3` total), fix regenerate persis via formula `_examQPoints()` dari `OBE_EXAM_CONFIG`+`OBE_ORDER` (verifikasi 45/45 key no-dup Σ≈100 tiap exam); bug independen anti-cheat Opto `_frictionLayer` pakai localStorage key hardcoded `optoauto_identity_uts` di halaman UAS (harusnya `_uas`) shg deteksi mahasiswa login selalu gagal, fix ganti literal; panel statis "Cara hitung nilai & bobot Sub-CPMK" 3 course diperbaiki Q-range-nya sinkron `OBE_EXAM_CONFIG.mapping` termasuk Opto (membalik keputusan §40.16 poin 4 "biarkan apa adanya", dosen approve eksplisit perbaiki semua); turut dirapikan `EXAM_CONFIG["math4-uas"].mapping` dead-code utk konsistensi dokumentasi (presedan §40.16 poin 4), tidak ada perubahan logika server jadi tidak perlu redeploy Functions; §40.27 BARU — follow-up dosen "pastikan cakupan soal-soal juga sudah sesuai": audit konten 45 soal per course (bukan cuma poin tampilan) menemukan bug LEBIH DALAM — `OBE_EXAM_CONFIG.mapping` (LIVE, beda dari EXAM_CONFIG dead-code) memotong Q1-45 sbg rentang datar padahal array soal disusun kronologis per Modul (tag `modul:` per soal, sudah ada di data tapi tak pernah dipakai grouping) — cuma ~22% soal per course benar-benar menguji Sub-CPMK yg di-assign; fix Getaran & Math4 (mapping dibangun dari tag modul: riil, 45/45 soal terverifikasi, EXAM_QID_POINTS diregenerate, panel diganti dari rentang Q ke rentang Pertemuan) — **BUTUH `firebase deploy --only functions`, belum dijalankan sesi ini**; Opto DITUNDA krn Sub-CPMK 3.1 (bobot 4, topik FFT/PSD/bearing-fault Minggu 7) tidak py satupun soal cocok di bank + 12 soal modul13/14 (LogReg/RF-SVM) per RPS resmi Sub-CPMK 5.1/5.2 "Bentuk: Tugas" bukan UAS — butuh keputusan dosen, bukan sekadar remapping; §40.28 BARU — Opto selesai: dosen putuskan (A) tulis 5 soal baru utk Sub-CPMK 3.1 sekarang (bukan tunda/redistribusi bobot), (B) keluarkan 12 soal Tugas-leak dari mapping OBE — 5 soal baru (tf8,mc16-18,c9) ditulis grounded dari isi riil Modul-7.html (FFT/PSD/BPFO-BPFI-BSF-FTF/envelope analysis), REPURPOSE 5 dari 12 slot Tugas-leak (bukan expand total soal, hindari sentuh OBE_EXAM_ORDER shared); CRITICAL ditemukan saat implementasi: jawaban benar MC/TF disimpan di Firestore `examAnswers/{examId}/qs/{qId}` (bukan repo/client, demi anti-cheat), source-of-truth-nya file repo `functions/seed/uas-optoauto-answers.js` di-seed via `node seed/seed-firestore.js` — WAJIB dijalankan admin stlh merge, sesi tanpa akses Firestore; 7 soal Tugas-leak sisa di-set poin eksplisit 0 (bukan fallback default) supaya Σ tetap 100 persis (fallback 1/2/4 default akan bikin live progress-bar client tembus 104% sblm di-cap server); mapping final 38 Q ter-mapping + 7 dikecualikan, panel & banner materi diupdate "Pertemuan 7, 9-15" (RPS sengaja uji Minggu 7 di UAS bukan UTS, desain resmi bukan bug); 2 langkah manual WAJIB sblm efektif live: `firebase deploy --only functions` + `node seed/seed-firestore.js --exam optoauto-uas`. PR references: #613–#627, #631–#652, #653–#671; §40.29 BARU — follow-up dosen "perkuat supaya teks soal tidak kebaca sebelum jadwal buka": teks soal UAS (bukan cuma kunci jawaban, yang sudah lama server-only) dulu ter-embed statis di client shg bisa dibaca via View Source/DevTools kapan saja — fix callable BARU `getExamQuestions` (dual-auth: mahasiswa PIN+jadwal via `evalSchedule` sama persis dgn `checkExamAnswer`, atau dosen kredensial hash admin legacy bypass jadwal), bank soal dipindah ke `functions/questions/uas-<course>-questions.js` (ekstraksi VERBATIM dari client demi nol risiko transkripsi ulang formula parametrik), client 3 `UAS.html` di-rewire fetch-after-login via `_ensureUASQuestionsLoaded()` (loading/error state graceful, tidak crash), diterapkan ke SEMUA 3 course sekaligus atas keputusan dosen eksplisit meski UAS Getaran tinggal 2 hari lagi; GOTCHA scoping non-module vs module `<script>` (probe var lintas-script via `typeof` guard) + GOTCHA Playwright BARU (`page.route()` glob pattern silently gagal di sandbox sesi ini, fix regex pattern); verifikasi Playwright 3 skenario (student/dosen/error) × Getaran + smoke test Opto/Math4, SEMUA lolos; **HANYA butuh `firebase deploy --only functions`, TIDAK butuh Firestore reseed** (beda dari §40.28) krn tidak ada perubahan kunci jawaban; follow-up langsung: kotak error "internal" dilaporkan dosen pasca-deploy pertama, `getExamQuestions` dibungkus try/catch (HttpsError passthrough, exception tak terduga → log + pesan spesifik ke client), terkonfirmasi via DevTools Network tab (`200 OK`, body benar) ini kegagalan sesaat cold-start invocation pertama function baru, self-resolve, bukan bug; §40.30 BARU — tombol `🚪 Logout` navbar di ke-3 UAS.html (follow-up dosen, membalik keputusan §39.10 "Exam TIDAK diberi tombol Ganti Peran — sesi ujian terkunci") — idiom sama `_switchRole()` Modul (clear identity → reload → role picker) + extra clear PIN-session (sessionStorage + `window._sessionPinHash`, layer yg tidak dipunyai Modul); bonus fix sejalan: badge navbar UAS dulu statis hardcode "DEDIK ROMAHADI" utk semua role, ditambah `id="navIdentityChip"` + hook `_applyRoleVisibility()` jadi dinamis sesuai user login (efek samping: kode mati `enterPreviewMode()` §39.11 yg sudah lama query id itu otomatis hidup); Playwright end-to-end konfirmasi klik→confirm→reload→identity ter-clear→role picker muncul lagi, 0 error; client-only, tidak butuh deploy Functions/Firestore.*
...
*Terakhir diperbarui: Juli 2026 — autentikasi admin Firebase, backend privat, allowlist artefak GitHub Pages, dan bank UAS terotasi. Riwayat implementasi sebelumnya dipertahankan untuk konteks migrasi.*
