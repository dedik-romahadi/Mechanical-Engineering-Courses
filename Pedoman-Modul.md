...
- Validator: TTL tetap di `obeOnlyRoots` validator keamanan (baru ada UTS; `courseRoots` mensyaratkan UTS dan UAS sekaligus); `validate-all-course-modern-design.mjs` memeriksanya lewat `moduleCount` (sekarang 14, lengkap). Hitungan: 70 modul dengan tombol ekspor terjaga dan 90 halaman ber-autentikasi admin (79 Modul/Exam). `OBE_COURSE_EXAMS` backend dan `verify-obe-mapping.js` belum memuat TTL karena keduanya mengharapkan pasangan UTS+UAS; halaman OBE TTL sudah memuat `DEFAULT_MAPPING.uts` (MAP_KEY v4).
...
- 70 modul: 14 per course aktif + Modul 1–14 Teknik Tenaga Listrik;
