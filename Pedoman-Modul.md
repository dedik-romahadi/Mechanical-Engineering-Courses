...
- **Validator:** modul CAD dihitung `validate-all-course-modern-design.mjs` lewat `moduleCount` (1); CAD tetap di `obeOnlyRoots` validator keamanan sampai UTS/UAS-nya ada. Hitungan: 71 modul dengan tombol ekspor terjaga dan 93 halaman ber-autentikasi (81 Modul/Exam + 6 OBE + 6 Admin). Roster memuat akun simulasi. Belum: UTS/UAS, `OBE_COURSE_EXAMS`, registry chat AI backend, versi Word/PDF, banner LMS.
...
- 71 modul: 14 per course aktif + Modul 1–14 Teknik Tenaga Listrik + Modul 1 Pemodelan CAD;
...
`validate-public-security.mjs` memindai seluruh HTML dalam allowlist Pages pada lima course aktif serta folder Pemodelan CAD. Ia menjaga artefak sensitif, sintaks inline script, 93 halaman berautentikasi admin (81 Modul/Exam + 6 OBE + 6 Admin), gate dan friction exam, WIB, preview modul, reset, presence, format poin, pemulihan `scoreDeltas`, serta keamanan publikasi. Jumlah halaman autentikasi dipatok di validator; perbarui bersama bila inventaris berubah.
