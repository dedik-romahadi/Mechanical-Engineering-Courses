...
9. **KRITIS — DUA HAL YANG TIDAK BISA DISELESAIKAN DARI SESI INI,
   ...
   - **Deploy Cloud Functions**: perubahan `functions/index.js`
     ...
     dosen jalankan `firebase deploy --only functions` (lihat
     `functions/DEPLOY.md`) — environment sesi ini TIDAK py Firebase
...
1. **Callable BARU `getExamQuestions`** (`functions/index.js`, pola
   ...
   utk submit, `EXAM_CONFIG` yg sama) ATAU **dosen** (adminPwHash cocok
   `ADMIN_PW_HASH`, bypass jadwal — utk preview/monitoring, sama spt
...
5. **Verifikasi via Playwright** (file://, Firebase CDN di-stub lokal
   ...
   (student happy-path fetch+render, dosen adminPwHash+bypass-jadwal,
