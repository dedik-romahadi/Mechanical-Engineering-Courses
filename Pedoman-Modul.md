...
- panel Skor Sementara dan Export HTML tetap mengikuti perilaku modul mata kuliah lain: hero Tugas setinggi `60vh`, tab Tugas langsung menggulir ke panel, dan panel tetap `sticky` pada `top:64px` saat pertanyaan digulir. Agar sticky mengacu ke viewport utama, `body` memakai `overflow-x:clip` dan `overflow-y:visible`, sedangkan overlay login tetap menggulir di kontainernya sendiri. Geometri panel dibuat compact melalui padding `14px 20px`, gap `14px`, tombol setinggi minimum `38px`, serta teks petunjuk/status yang lebih rapat; pada layar sempit nilainya menyesuaikan lagi tanpa menyembunyikan informasi. Panel mempertahankan rincian 10 PG + 10 komputasi E/M + 5 komputasi Hard, pesan kelengkapan, kolom Google Drive, penyimpanan draft, serta tombol Export yang baru aktif setelah seluruh prasyarat lengkap. `validate-sisken-modules.mjs` membandingkan logika panel dengan Modul 1 Getaran Mekanik dan menjalankan transisi terkunci → aktif pada DOM uji;
