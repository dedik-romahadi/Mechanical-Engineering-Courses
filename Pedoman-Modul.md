...
Solusi: **GitHub Pages**, folder khusus `docs/` di root repo (BUKAN
root branch — sengaja discope ke 1 folder biar tidak accidentally
mem-publish seluruh 42 modul HTML LMS yang live/terhubung Firebase
sebagai situs statis paralel yang tak terduga/tak teruji).
...
- **Setup manual WAJIB dosen (1×, tidak bisa diotomasi dari sesi ini)**:
  repo Settings → Pages → Source: "Deploy from a branch" → Branch:
  `main` → Folder: `/docs` → Save. Tidak ada tool GitHub MCP di sesi
  ini utk `PUT /repos/{owner}/{repo}/pages` (API Pages settings, beda
  dari file/branch/PR tools yang tersedia). Repo sudah public
  (`private:false` dicek via `search_repositories`), jadi enable Pages
  TIDAK mengubah status privasi apa pun, cuma menambah 1 permukaan URL
  publik baru yang sudah eksplisit di-approve dosen via AskUserQuestion.
- URL hasil: `https://dedik-romahadi.github.io/Mechanical-Engineering-Courses/`
  (index) dan `.../pdf/<file>.pdf` (langsung).
