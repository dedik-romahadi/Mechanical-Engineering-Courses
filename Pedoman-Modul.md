...
**Konsekuensi lanjutan:** karena root repo memang disajikan, folder
`docs/` jadi **redundan** — `Unduhan-Gabungan/*.pdf` (§40.20) sudah bisa
diakses langsung di `.../Unduhan-Gabungan/<file>.pdf` dgn perilaku
inline-preview yang sama persis. Duplikat `docs/pdf/` dipertahankan utk
sementara (link lama yang sudah terlanjur dibagikan), tapi menambah
beban sinkronisasi ganda (§40.21 poin `docs/pdf/*.pdf`). Kalau
dibersihkan nanti, pastikan tidak ada link `docs/...` yang masih
beredar.
...
4. Setelah edit, PDF individual 6 file diregenerate (LibreOffice headless)
   IN-PLACE, lalu turunan combined `Exam-Gabungan-<Course>.pdf` di
   **KEDUA** lokasi (`Unduhan-Gabungan/` §40.20 dan `docs/pdf/` §40.21)
