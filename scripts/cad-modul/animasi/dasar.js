...
// Label berpelat: kotak warna latar kanvas digambar di belakang teks sehingga garis yang
// melintas di belakangnya terputus (seperti gambar teknik yang memutus garis bantu di belakang
// angka dimensi). Pakai untuk angka dimensi di atas garis dimensinya atau label di atas kisi/kurva
// yang tidak bisa dipindah. Pelat hanya menutupi garis yang digambar SEBELUM label.
// opsi: {pad (bawaan 2), latar (bawaan '#050a16')}. Perataan mengikuti ctx.textAlign/textBaseline.
function _ttlLabel(ctx,teks,x,y,opsi){
  opsi=opsi||{};
  const u=ctx.measureText(teks), pad=opsi.pad==null?2:opsi.pad;
  const x0=x-u.actualBoundingBoxLeft-pad, x1=x+u.actualBoundingBoxRight+pad, y0=y-u.actualBoundingBoxAscent-pad, y1=y+u.actualBoundingBoxDescent+pad;
  const warna=ctx.fillStyle; ctx.fillStyle=opsi.latar||'#050a16'; ctx.fillRect(x0,y0,x1-x0,y1-y0); ctx.fillStyle=warna;
  ctx.__plat=true; ctx.fillText(teks,x,y); ctx.__plat=false;
}
