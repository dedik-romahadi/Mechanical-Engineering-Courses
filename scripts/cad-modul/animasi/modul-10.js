...
function drawRusuk(){
  ...
  if(sempit){
    ...
    bx0=26; bx1=W-14; by=y0+50; skala=Math.max(24,Math.min(48,H-by-30))/Math.max(d0,1e-9);
  } else {
  ...
  // Label rusuk selalu di atas puncak rusuk: di samping puncak rusuk ia dicoret tepi atas pelat dan
  // sumbu netral selama rusuk masih pendek.
  if(hr>0) _cad10Teks(ctx,'rusuk '+tr.toFixed(0)+' × '+hr.toFixed(1),cx,Y(t+hr)-6,'#f59e0b',f10,'center');
  ...
  // Beban F di ATAS ujung bebas, panah menunjuk ke bawah. Dulu panah dan labelnya di bawah ujung, dan label
  // itu dicoret kurva pelat polos. Label di kiri pangkal panah; bila kurva lendutan di bawah tepi kiri label
  // lebih tinggi (kantilever pendek), label dinaikkan di atasnya.
  const yU=by+d1*skala, teksF='F = '+F.toFixed(0)+' N'; ctx.font=f10; const wF=ctx.measureText(teksF).width;
  _ttlGaris(ctx,bx1-6,yU-34,bx1-6,yU-12,'#f59e0b',1.8); ctx.fillStyle='#f59e0b'; ctx.beginPath(); ctx.moveTo(bx1-6,yU-3); ctx.lineTo(bx1-11,yU-12); ctx.lineTo(bx1-1,yU-12); ctx.fill();
  const uF=Math.max(0,(bx1-12-wF-bx0)/(bx1-bx0)), yKiri=by+d1*skala*(3*uF*uF-uF*uF*uF)/2;
  _cad10Teks(ctx,teksF,bx1-12,Math.min(yU-26,yKiri-4),'#f59e0b',f10,'right');
  // label pelat polos di bawah kurvanya (ponsel: baris sendiri di bawah ujung kurva)
  _cad10Teks(ctx,'pelat polos δ = '+d0.toFixed(2)+' mm',sempit?12:bx0+8,by+d0*skala+(sempit?20:16),'rgba(148,163,184,.85)',f10);
