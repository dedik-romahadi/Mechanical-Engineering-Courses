...
// Label sumbu dijepit ke dalam kanvas agar tidak terpotong pada nilai slider apa pun.
function _cad5Sumbu(ctx,az,el,sk,cx,cy,L){
  const O=_cad5P([0,0,0],az,el,sk,cx,cy), W=ctx.canvas.width, H=ctx.canvas.height;
  [[[L,0,0],_C5X,'X'],[[0,L,0],_C5Y,'Y'],[[0,0,L],_C5Z,'Z']].forEach(([v,w,n])=>{const P=_cad5P(v,az,el,sk,cx,cy); ctx.strokeStyle=w; ctx.lineWidth=1.4; ctx.beginPath(); ctx.moveTo(O[0],O[1]); ctx.lineTo(P[0],P[1]); ctx.stroke(); ctx.fillStyle=w; ctx.font="bold 10px 'JetBrains Mono',monospace"; ctx.textAlign='left'; ctx.fillText(n,Math.min(Math.max(P[0]+4,4),W-11),Math.min(Math.max(P[1]-3,12),H-4));});
}
...
function drawPad(){
  ...
  const az=35, el=28, L=Math.max(a,b)*0.5;
  ...
  // Kotak batas balok setinggi hM (tinggi terbesar selama animasi) dan ujung sumbu, satuan model.
  const kb=_cad5Kotak([[0,0,0],[a,0,0],[a,b,0],[0,b,0],[0,0,hM],[a,0,hM],[a,b,hM],[0,b,hM],[L,0,0],[0,L,0],[0,0,L]],az,el);
  let sk,cx,cy,tx,ty;
  if(!sempit){
    // Lebar: balok di kiri, rumus di kanan; skala dibatasi agar balok tidak menyentuh rumus/judul.
    ...
    sk=Math.max(0.05,Math.min(Math.min(W*0.55,H*1.1)/(a+b+hM)*1.1,(tx-24-cx)/kb[2],(cy-30)/-kb[1]));
  } else {
    ...
    tx=12; ty=H-50;
    ({sk,cx,cy}=_cad5Muat(kb,14,30+14*nJ,W-24,ty-24));
  }
  _cad5Sumbu(ctx,az,el,sk,cx,cy,L);
...
function drawRevolve(){
  ...
  const tepi=[[L,0,0],[0,L,0],[0,0,L]]; for(let i=0;i<24;i++){const t=i*Math.PI/12, x=ro*Math.cos(t), y=ro*Math.sin(t); tepi.push([x,y,0],[x,y,h]);}
  const kb=_cad5Kotak(tepi,az,el);
  let sk,cx,cy,tx,ty;
  if(!sempit){
    let lT=0; ket.forEach(([s,,px])=>{ctx.font=_F5(px); lT=Math.max(lT,ctx.measureText(s.join(' ')).width);});
    tx=Math.min(W*0.66,W-12-lT); ty=H*0.3;
    // Skala asal dibatasi dan benda digeser seperlunya agar tidak keluar tepi atas (menimpa judul).
    sk=Math.max(0.05,Math.min(Math.min(W*0.5,H*1.2)/(2*ro+h)*1.05,(H-46)/(kb[3]-kb[1]),(tx-34)/(kb[2]-kb[0])));
    cx=Math.min(Math.max(W*0.34,10-sk*kb[0]),tx-24-sk*kb[2]);
...
function drawFillet(){
  ...
  if(!sempit){
    ...
    sk=Math.max(0.05,Math.min((W*0.55)/(a+20),(H-56)/(b+20),(tx-16-lLab-W*0.06)/a));
  ...
  ctx.fillStyle='#f59e0b'; ctx.font="10px 'JetBrains Mono',monospace"; ctx.textAlign='left'; ctx.fillText((mode?'C':'R')+f.toFixed(1),Math.min(X(a-f)+6,W-4-lLab),Y(b-f)-8);
