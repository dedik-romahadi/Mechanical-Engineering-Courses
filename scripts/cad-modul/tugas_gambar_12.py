...
def gambar():
    ...
    body += t(p[0], p[1] + 4, "B", 12, AM, "middle", "700")
    p = P(a1 - dl / 2, b / 2, h)
    ...
    p = P(0, 0, h * 0.78)
    body += t(p[0] - 9, p[1] + 4, "h", 11, AM, "end", "600")
    p = P(a1 - dl / 2, -4, 0)
    ...
    body += _garis(z0[0], z0[1], z1[0], z1[1], RD, 1.1, "8 3 2 3") + t(z1[0] + 6, z1[1] + 2, "sesumbu (Z)", 9.5, RD, "start")
    p = iso(ro, 0, hz / 2, cx, cy, s)
    body += t(p[0] + 8, p[1] + 10, "cincin: lubang", 10, CY, "start", "600") + t(p[0] + 8, p[1] + 24, "⌀D + ES/1000 (maks)", 10, CY, "start")
    p = iso(0, 0, hs, cx, cy, s)
    body += t(p[0] - 52, p[1] - 26, "poros ⌀D + ei/1000 (min)", 10, AM, "middle", "600")
    p = iso(0, -(rh + rs) / 2, hz, cx, cy, s)
    body += _garis(p[0], p[1], p[0] - 30, p[1] + 34, GR, 0.9, "3 2") + t(p[0] - 30, p[1] + 48, "celah radial", 10, GR, "middle", "600") + t(p[0] - 30, p[1] + 61, "= c_maks / 2", 10, GR, "middle")
    body += catatan(["Body 1: cincin, lubang ⌀(D + ES/1000)", "Body 2: poros ⌀(D + ei/1000), ei < 0", "  sesumbu, konstrain Diameter 4 desimal", "c_maks = ⌀lubang maks − ⌀poros min", "baca: c_maks = (ES − ei)/1000 (mm)"], 330, 40)
    out.append(gambar_tugas(body, "Tugas 2 — fit lubang–poros pada ukuran batas dan kelonggaran maksimum", h=236))
    ...
    body += t(p[0] - 9, p[1] + 4, "h", 11, AM, "end", "600")
    p = P(a - w / 2, 0, h)
    body += _garis(p[0], p[1], p[0] + 22, p[1] - 14, GR, 0.9, "3 2") + t(p[0] + 26, p[1] - 16, "dinding w", 10, GR, "start", "600")
    p = P(a - w, b - w, h - p_ / 2)
    ...
    body += catatan(["Sketch XY a × b simetris origin → Pad t", "Slot obround di pusat: Ls (pusat–pusat),", "  lebar ws, searah X → Pocket Through all", "e = ujung busur slot → tepi pendek pelat", "baca: e = (a − (Ls + ws))/2 (arah X)"], 330, 40)
    ...
    body += _ling(ox, oy, 4, TX, "none") + t(ox - 6, oy + 15, "sumbu (0, 0)", 9, AX, "end")
    body += _ling(ox + rc, oy, 3.5, PK, "none")
    body += ext(ox + rc, oy + 4, ox + rc, oy + 46, GR) + ext(ox + W, oy + 100, ox + W, oy + 118, GR) + dim_h(ox + rc, ox + W, oy + 40, "c_min", GR, atas=False)
    body += ext(ox, oy - 4, ox, oy - 116) + ext(ox + W, oy - 100, ox + W, oy - 116) + dim_h(ox, ox + W, oy - 108, "W")
    body += t(ox + W + 7, oy - 116, "dinding", 9.5, AX, "middle")
    p = rot(R * 0.5, w / 2 + 12)
    ...
    body += t(ox + rc + 6, oy - 8, "sudut terjauh", 9.5, PK, "start")
    ...
    out.append(gambar_tugas(body, "Tugas 5 — lengan berputar dekat dinding dan jarak bebas minimum", h=250))
    return out
