...
def gambar():
    ...
    body += f'<line x1="{z0[0]:.1f}" y1="{z0[1]:.1f}" x2="{z1[0]:.1f}" y2="{z1[1]:.1f}" stroke="{RD}" stroke-width="1" stroke-dasharray="8 3 2 3"/>' + t(z1[0] + 6, z1[1], "sumbu Z", 9.5, RD, "start")
    p = iso(0, -b1 / 2 - 4, 0, cx, cy, s)
    body += t(p[0], p[1] + 16, "a₁ × b₁ (z = 0)", 10.5, AM, "middle", "600")
    p = iso(0, 0, h, cx, cy, s)
    body += t(p[0] - 30, p[1] - 28, "a₂ × b₂ (z = h)", 10.5, CY, "middle", "600")
    p = iso(a1 / 2, -b1 / 2, h / 2, cx, cy, s)
    body += t(p[0] + 8, p[1] + 4, "h", 11, TX, "start", "700")
    p = iso(a1 / 2, b1 / 2, h / 2, cx, cy, s)
    body += t(p[0] + 10, p[1] + 4, "ruled", 10, AX, "start")
    ...
    body += t(p[0] + 4, p[1] + 4, "b", 11, AM, "start", "600")
    p = iso(-6, 0, h / 2, cx, cy, s)
    body += t(p[0] - 4, p[1] + 4, "h", 11, AM, "end", "600")
    ...
    body += f'<circle cx="{cx0}" cy="{cy0}" r="{rbc}" fill="none" stroke="{AM}" stroke-width="1" stroke-dasharray="5 4"/>'
    for i in range(n):
        ang = 2 * math.pi * i / n
    ...
    body += t(cx0, cy0 + 4, "⌀d₀", 10, CY, "middle", "600")
    body += t(cx0 + rbc + rb + 6, cy0 + 4, "n × ⌀d_b (induk)", 10, GR, "start", "600")
    body += t(cx0, cy0 + rbc + 18, "lingkaran baut ⌀D_bc", 10, AM, "middle", "600")
    ...
    out.append(gambar_tugas(body, "Tugas 4 — flens dengan lubang pusat dan pola polar n lubang baut", h=250))
    ...
    body += t(p[0], p[1] + 16, "Pelat a × b × t", 10.5, CY, "middle", "600")
    p = iso(a / 4, b / 2, tt, cx, cy, s)
    body += t(p[0], p[1] + 22, "(a/4, b/2)", 9, AX, "middle")
    p = iso(3 * a / 4, b / 2, tt, cx, cy, s)
    body += t(p[0] + 4, p[1] + 26, "(3a/4, b/2)", 9, AX, "middle")
