...
export const PENJELASAN_RUMUS = {
  ...
  "spesifikasi = {t_r, t_s, M_p, e_ss, |u|max, robustness}": {
    apa: "menuliskan keinginan pelanggan sebagai himpunan angka yang bisa diuji. 'Cepat dan halus' tidak bisa diperiksa; enam besaran ini bisa, sehingga sebuah rancangan lulus atau gagal tanpa perdebatan.",
    variabel: [
  ...
  "G(s) = K*exp(-Ls)/(tau*s+1)   |   m*x'' + b*x' + k*x = F(t)": {
    ...
    variabel: [
      ...
      ["tau", "konstanta waktu: ukuran kelambanan, yakni 63% jalan ditempuh dalam satu {{tau}}"],
  ...
  "dt <= 0.1 * tau_tercepat, lalu ulangi pada dt/2 untuk verifikasi": {
    apa: "aturan memilih langkah waktu simulasi: paling besar sepersepuluh dari konstanta waktu tercepat di sistem. Ulangi simulasi pada langkah setengahnya, karena bila hasilnya berubah berarti langkah pertama masih terlalu kasar.",
    variabel: [
      ["dt", "langkah waktu simulasi: jarak antar titik hitung"],
      ["tau_tercepat", "konstanta waktu terkecil di antara semua dinamika sistem, sebab dialah yang menentukan seberapa rapat simulasi harus melangkah"],
    ],
  },
  ...
  "F(s) = integral_0^inf f(t)*exp(-st) dt   |   L{f*g} = F(s)*G(s)": {
    apa: "definisi transformasi Laplace: sinyal waktu ditimbang peluruhan {{exp(-st)}} lalu dijumlahkan dari nol sampai tak hingga sehingga hasilnya fungsi dalam {{s}}. Hadiah terbesarnya di ruas kanan: konvolusi yang rumit di domain waktu menjadi perkalian biasa di domain-s.",
    variabel: [
  ...
  "stabil <=> Re(p_i) < 0 untuk semua i   |   y(inf) = lim s->0 s*Y(s)": {
    apa: "dua alat baca tercepat di domain-s. Kiri: sistem stabil jika dan hanya jika SEMUA pole berada di kiri sumbu khayal, sebab satu saja di kanan, respons meledak. Kanan: teorema nilai akhir menghitung nilai tunak tanpa menunggu, cukup substitusi {{s}} menuju 0.",
    variabel: [
      ["Re(p_i)", "bagian nyata pole ke-i: negatif berarti mode-nya meluruh"],
      ["y(inf)", "nilai keluaran setelah lama sekali (nilai tunak)"],
      ["lim s->0", "limit s menuju nol, yang hanya sah bila sistemnya stabil"],
    ],
  },
  ...
  "termal: C*dT/dt = q_in - h*(T - T_amb)  =>  G(s) = K/(tau*s + 1)": {
    apa: "contoh lengkap dari fisika ke fungsi transfer: neraca energi benda dipanaskan (kalor masuk dikurangi kalor terbuang) di-Laplace-kan menjadi model orde satu. Semua sistem 'tangki bocor', misalnya termal, tangki air, dan RC, berujung pada bentuk yang sama ini.",
    variabel: [
  ...
  "y(0+) = lim s->inf s*Y(s)   |   y(inf) = lim s->0 s*Y(s)": {
    apa: "sepasang teorema untuk memeriksa jawaban tanpa inversi penuh: nilai sesaat setelah mulai dibaca dari limit {{s}} besar, nilai akhir dari limit {{s}} kecil. Keduanya alat deteksi salah hitung tercepat, karena bila y(0+) atau y(∞) hasil inversi tidak cocok, pasti ada langkah yang keliru.",
    variabel: [
  ...
  "G(s) = Y(s)/U(s) pada kondisi awal nol   |   orde penyebut >= orde pembilang": {
    apa: "definisi fungsi transfer sekaligus dua syarat sahnya: ia perbandingan keluaran/masukan HANYA saat kondisi awal nol, dan sistem fisik selalu punya orde penyebut minimal sama dengan pembilang, sebab bila lebih kecil, modelnya menuntut sistem meramal masa depan.",
    variabel: [
      ["G(s)", "fungsi transfer sistem"],
  ...
  "G_eq = G1*G2 hanya bila blok kedua tidak membebani blok pertama": {
    apa: "aturan blok seri: gain ekivalennya hasil kali, TETAPI hanya bila blok kedua tidak menyedot daya dari blok pertama. Dua rangkaian RC yang disambung langsung TIDAK boleh dikalikan begitu saja; sisipkan penyangga dulu, baru rumus ini sah.",
    variabel: [
  ...
  "G_eq = G1 +/- G2   |   T = G/(1 + G*H)   |   bila G*H >> 1 maka T ~ 1/H": {
    apa: "tiga rumus penyusun diagram: paralel dijumlahkan, loop tertutup memakai rumus baku {{G/(1 + G*H)}}, dan konsekuensi terpentingnya di ruas kanan, yaitu saat gain loop sangat besar, perilaku sistem ditentukan oleh sensor {{H}}, bukan oleh plant. Itulah mengapa kualitas sensor menentukan kualitas kendali.",
    variabel: [
  ...
  "S = 1/(1 + G*H)   |   dT/T = S * dG/G": {
    apa: "alasan kuantitatif umpan balik dipakai: bila plant bergeser 20% ({{dG/G}} = 0,2), perilaku loop tertutup hanya bergeser {{S}} kalinya. Dengan gain loop 9, S = 0,1, artinya pergeseran 20% di plant tinggal terasa 2%. Umpan balik menukar gain menjadi kekebalan terhadap ketidakpastian.",
    variabel: [
  ...
  "eksperimen = {model, parameter, kondisi awal, masukan, solver, metrik}": {
    apa: "daftar isi sebuah simulasi yang bisa diulang orang lain: enam komponen ini harus tercatat semua. Grafik tanpa keenam komponen pendampingnya bukan hasil eksperimen, melainkan cuma gambar yang tidak bisa diperiksa maupun diulang.",
    variabel: [
  ...
  "RMSE = sqrt(mean((y_ukur - y_model)^2))   |   pisahkan data latih dan data uji": {
    ...
    variabel: [
      ["RMSE", "akar rata-rata kuadrat galat; angka 0 berarti model menembus semua titik data"],
  ...
  "dx' = A*dx + B*du dengan A dan B turunan parsial di titik kerja": {
    apa: "cara memakai seluruh perkakas linier pada sistem nonlinier: bekerja dengan SIMPANGAN kecil di sekitar titik kerja, bukan nilai mutlaknya. Matriks {{A}} dan {{B}} adalah kemiringan lokal (turunan parsial) di titik itu; hasilnya sah selama simpangan tetap kecil, dan harus dihitung ulang bila titik kerjanya pindah.",
    variabel: [
  ...
  "gangguan: frekuensi rendah di masukan plant | derau: frekuensi tinggi di umpan balik": {
    apa: "dua musuh yang menuntut obat berlawanan, dibedakan dari tempat masuk dan frekuensinya: gangguan (beban berubah, angin) masuk di plant dan berfrekuensi rendah, maka dilawan dengan gain besar; derau sensor masuk di umpan balik dan berfrekuensi tinggi, maka justru DIPERPARAH gain besar. Kompromi inilah inti perancangan.",
    variabel: [
      ["gangguan", "sinyal tak diinginkan yang mendorong plant sungguhan"],
      ["derau", "kesalahan baca sensor, sementara plant-nya sendiri tidak terdorong"],
    ],
  },
  ...
  "pisahkan hitung dari perangkat keras   |   uji keadaan tepi, bukan hanya keadaan normal": {
    apa: "dua kebiasaan yang membuat kode kendali bisa diuji: logika hitung dipisah dari akses perangkat keras supaya bisa diuji di komputer biasa tanpa mesin, dan pengujian justru menyasar keadaan tepi, misalnya error terbesar, aktuator jenuh, dan sensor putus, karena di sanalah kecelakaan terjadi, bukan di keadaan normal.",
    variabel: [
  ...
  "T_r = G*C/(1+G*C*H)   |   T_d = G/(1+G*C*H)   |   penyebut sama, pembilang berbeda": {
    apa: "dua fungsi transfer yang menjelaskan mengapa uji setpoint saja tidak cukup: jalur setpoint lewat controller ({{G*C}} di pembilang), jalur gangguan tidak ({{G}} saja). Penyebutnya sama sehingga kestabilannya satu, tetapi kecepatan menolak gangguan bisa jauh berbeda dari kecepatan mengikuti perintah; karena itu keduanya harus diuji terpisah.",
    variabel: [
      ["T_r", "fungsi transfer setpoint → keluaran"],
      ["T_d", "fungsi transfer gangguan → keluaran"],
      ["G", "plant"],
  ...
  "T(s) = wn^2/(s^2 + 2*zeta*wn*s + wn^2)   |   zeta mengatur bentuk, wn mengatur kecepatan": {
    apa: "bentuk baku orde dua, yakni acuan pembanding hampir semua sistem umpan balik. Kekuatannya pada pembagian kerja yang bersih: {{zeta}} SENDIRIAN menentukan bentuk kurva (melonjak atau tidak, berapa persen), {{wn}} SENDIRIAN menentukan skala waktunya. Dua tombol, dua urusan, tidak saling campur.",
    variabel: [
      ["T(s)", "fungsi transfer loop tertutup baku"],
      ["wn", "frekuensi alami (rad/s): tempo dasar, dan makin besar berarti makin cepat segalanya"],
  ...
  "T_r = GC/(1+GCH)   |   T_d = G/(1+GCH)   |   penyebut sama, pembilang berbeda": {
    apa: "pasangan fungsi transfer yang memisahkan dua tugas kendali: mengikuti perintah (jalur setpoint melewati controller, {{GC}} di pembilang) dan menolak gangguan (jalur gangguan hanya melewati plant, {{G}}). Penyebut yang sama berarti satu kestabilan bersama; pembilang yang berbeda berarti kinerja keduanya TIDAK otomatis sama, sehingga harus diuji masing-masing.",
    variabel: [
      ["T_r", "fungsi transfer setpoint → keluaran"],
      ["T_d", "fungsi transfer gangguan → keluaran"],
      ["G", "plant"],
  ...
  "u = Kp*e + Ki*integral(e) + Kd*de/dt   |   sekarang, masa lalu, dan arah": {
    apa: "hukum PID dalam bentuk paralel: sinyal kendali adalah jumlahan tiga jawaban atas tiga pertanyaan, yaitu seberapa besar error SEKARANG (suku P), berapa banyak error yang sudah MENUMPUK (suku I), dan ke mana error sedang BERGERAK (suku D). Tiga sudut pandang waktu dalam satu baris.",
    variabel: [
  ...
  "Ziegler-Nichols: Kp = 0,6*Ku, Ti = 0,5*Tu, Td = 0,125*Tu": {
    apa: "resep penyetelan klasik dari eksperimen ambang: naikkan gain P murni sampai sistem berosilasi tetap, lalu catat gain kritisnya {{Ku}} dan periode osilasinya {{Tu}}, kemudian ketiga parameter PID dibaca dari tabel ini. Hasilnya agresif (lonjakan ±25%); pakai sebagai titik awal yang kemudian dihaluskan, bukan jawaban akhir.",
    variabel: [
  ...
  "anti-windup: I += Kt*(u_nyata - u_minta)*T bersama pembatasan keluaran": {
    apa: "back-calculation: selama aktuator jenuh, selisih antara yang benar-benar keluar dan yang diminta bernilai negatif, sehingga suku ini MENGURAS akumulator integral secepat {{Kt}}, sehingga saat error berbalik, controller siap bereaksi seketika, bukan menunggu gelembung integralnya kempes dulu.",
    variabel: [
      ["I", "akumulator suku integral"],
  ...
  "L/tau besar => penguatan aman mengecil; kurangi L dulu sebelum menyetel": {
    apa: "rasio {{L/tau}} adalah indeks kesulitan proses: makin besar porsi waktu mati dibanding konstanta waktunya, makin kecil gain yang masih aman, sekaligus makin sia-sia penyetelan canggih. Urutan kerja yang benar: perangi dulu waktu matinya (pindahkan sensor, percepat komunikasi), baru setel; menyetel keras pada L besar hanya mengundang osilasi.",
    variabel: [
  ...
  "reduksi blok: bertahap dan rawan tanda   |   Mason: sekali hitung dari daftar": {
    apa: "perbandingan dua rute menghitung fungsi transfer: reduksi blok menyederhanakan diagram selangkah demi selangkah, dan tiap langkah adalah kesempatan salah tanda; rumus Mason menghitung SEKALI dari daftar yang disusun sistematis (jalur maju, loop, sentuhan). Pada diagram berloop bersilangan, Mason bukan sekadar lebih cepat, sebab reduksi bertahap nyaris mustahil.",
    variabel: [
  ...
  "T = (1/Delta) * sum(P_k * Delta_k)": {
    apa: "rumus penguatan Mason selengkapnya: fungsi transfer total adalah jumlahan tiap jalur maju dikali determinan sisanya, dibagi determinan penuh. {{Delta_k}} dihitung seperti {{Delta}} tetapi HANYA dari loop yang tidak disentuh jalur ke-k, sedangkan jalur yang menyentuh semua loop mendapat {{Delta_k}} = 1.",
    variabel: [
  ...
  "Delta = penyebut fungsi transfer = persamaan karakteristik": {
    apa: "tiga nama untuk satu benda: determinan grafik yang dihitung Mason IDENTIK dengan penyebut fungsi transfer, dan penyebut yang disamakan nol itulah persamaan karakteristik penentu kestabilan. Rumus Mason bukan trik terpisah, melainkan jalan lain menuju objek yang sama dengan aljabar blok dan ruang keadaan.",
    variabel: [
  ...
  "Delta = 0 adalah persamaan karakteristik; pembilang tidak diperlukan": {
    apa: "jalan pintas analisis kestabilan: karena kestabilan hanya ditentukan pole, dan pole adalah akar {{Delta}} = 0, maka cukup determinannya saja yang dihitung, sementara daftar jalur maju dan seluruh pembilang boleh dilewati. Setengah pekerjaan Mason sudah menjawab pertanyaan stabil-atau-tidak.",
    variabel: [
      ["Delta", "determinan grafik"],
      ["pembilang", "bagian rumus Mason yang memuat jalur maju, dan tak berpengaruh pada kestabilan"],
    ],
  },
  ...
  "w := w - eta * dL/dw   |   permukaan tidak cembung, minimum global tidak dijamin": {
    apa: "gradient descent, jantung semua pelatihan: tiap bobot digeser MELAWAN arah tanjakan kerugian, selangkah sebesar {{eta}}. Peringatan kanannya jujur: permukaan kerugian jaringan saraf berbukit-bukit, sehingga penurunan gradien hanya menjamin sampai di SUATU lembah, belum tentu lembah terdalam. Karena itu pelatihan diulang dari beberapa titik awal.",
    variabel: [
  ...
  "mu(x) di antara 0 dan 1   |   derajat keanggotaan bukan peluang": {
    apa: "fondasi logika fuzzy: keanggotaan pada sebuah himpunan bukan ya/tidak melainkan derajat 0–1; contohnya suhu 28° boleh 'agak panas' sebesar 0,6. Klausa kanan menepis salah paham tersering: 0,6 itu BUKAN peluang 60% panas; ia mengukur kekaburan batas kata, bukan ketidakpastian kejadian, dan jumlah derajat antar-himpunan tidak wajib satu.",
    variabel: [
  ...
  "kesimpulan berupa himpunan: terbaca | kesimpulan berupa bilangan: ringan dan mudah dioptimalkan": {
    apa: "dua mazhab inferensi dan alasan memilihnya: Mamdani menyimpulkan HIMPUNAN fuzzy yang enak dibaca dan diaudit pakar, cocok saat kejelasan diutamakan; Sugeno menyimpulkan BILANGAN (atau fungsi) langsung sehingga lebih ringan dihitung di perangkat tertanam dan lebih ramah untuk dioptimalkan otomatis, mis. disetel GA.",
    variabel: [
  ...
  "J = w1*integral e^2 + w2*integral u^2 + w3*lonjakan + denda pelanggaran batas": {
    apa: "fungsi kebugaran untuk penyetelan gain: kuadrat error menghukum penyimpangan besar, kuadrat sinyal kendali menghukum pemborosan aktuator, suku lonjakan menjaga kehalusan, dan denda pelanggaran memagari batas operasi. Bobot-bobotnya adalah kemudi Anda, sebab GA hanya sejujur fungsi yang disodorkan kepadanya.",
    variabel: [
