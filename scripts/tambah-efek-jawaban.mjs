...
(function(){
  ...
  document.addEventListener('click', function (ev) {
    var el = ev.target && ev.target.closest && ev.target.closest('.radio-option,.tf-option,button,[role="button"]');
    if (!el || el.classList.contains('ej-tombol-bisu')) return;
    if (el.disabled || el.getAttribute('aria-disabled') === 'true') return;
    if (el.tagName !== 'BUTTON' && el.getAttribute('role') !== 'button' && getComputedStyle(el).pointerEvents === 'none') return;
