...
Pakai:
    ...
    python scripts/ttl-modul/pasang-tautan-pdf.py --periksa  # laporan saja
"""
import re
import sys
...
def main():
    ...
    for html in sorted(KURSUS.glob("Modul/Modul-*.html"), key=lambda p: int(re.search(r"\d+", p.name).group())):
        ...
        if not (KURSUS / "Modul-Word" / f"{nama}.pdf").exists():
            lewat += 1
            continue
        baru = re.sub(r"const MODUL_PDF_URL = '[^']*';", f"const MODUL_PDF_URL = '../Modul-Word/{nama}.pdf';", s, count=1)
