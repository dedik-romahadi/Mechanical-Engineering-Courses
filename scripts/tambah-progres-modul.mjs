 ...
 *      sebelumnya. Modul 1 selalu terbuka. Tombol "Periksa lagi" di overlay
 *      menanyakan ulang ke server tanpa muat ulang/login ulang: bila modul
 *      sebelumnya sudah lengkap (mis. dilengkapi di tab lain), overlay ditutup
 *      dan halaman langsung dapat dipakai; bila belum, rinciannya diperbarui.
...
.pm-kunci-modal .pm-periksa{width:100%;padding:12px;border-radius:14px;margin-top:12px;border:1px solid rgba(0,224,158,.55);background:rgba(0,224,158,.12);color:#9ff3d2;font-family:inherit;font-weight:700;font-size:.92rem;cursor:pointer;transition:background .2s}
.pm-kunci-modal .pm-periksa:hover{background:rgba(0,224,158,.2)}
.pm-kunci-modal .pm-periksa:disabled{opacity:.6;cursor:wait}
.pm-kunci-modal .pm-catatan{font-size:.8rem;color:#94a3b8;text-align:center;margin:10px 0 0}
...
(function () {
  ...
  // Mahasiswa dan akun simulasi digerbang (simulasi = persis mahasiswa, kecuali
  // boleh membatalkan centang terakhir untuk uji ulang). Dosen & preview bebas.
  function mhsAktif() {
    var me = identitas();
    return !!(me && me.role === 'student' && me.nim && !window._previewMode);
  }
  function akunSimulasi() { var me = identitas(); return !!(me && SIM_NIMS.has(String(me.nim))); }
  function dasar() {
    var me = identitas();
  ...
  function render() {
    kotak.forEach(function (el, i) {
      ...
      cb.disabled = (bebas || akunSimulasi()) ? !(giliran || i === centang - 1) : !giliran;
    ...
    // Tab Tugas/Forum/Hasil terkunci untuk mahasiswa & akun simulasi sampai
    // centang lengkap; dosen/preview (mode bebas) tidak dikunci. Mode bebas
    // diingat di localStorage.
    kunciTab(!bebas && centang < total);
  ...
  if (typeof switchAsli === 'function') {
    window.switchTab = function (tab) {
      if (!bebas && centang < total && TAB_TERKUNCI.indexOf(tab) >= 0) {
  ...
  kotak.forEach(function (el, i) {
    el.querySelector('input').addEventListener('change', function (ev) {
      ...
      if (!cb.checked) {
        // Mahasiswa tidak bisa membatalkan; akun simulasi boleh membatalkan yang terakhir (server memvalidasi).
        if (!(akunSimulasi() && i === centang - 1) || sibuk) { cb.checked = true; return; }
        sibuk = true; cb.disabled = true;
        var db = dasar(); db.index = i; db.total = total; db.batal = true;
        panggil('setModulCentang', db).then(function (r) { centang = Number(r.centang) || i; })
          .catch(function (e) { cb.checked = true; toast('⚠ ' + ((e && e.message) || 'Gagal membatalkan centang.')); })
  ...
  function tampilkanKunci(pras) {
    var lama = document.getElementById('pmKunci'); if (lama) lama.remove();
    var ov = document.createElement('div');
    ov.id = 'pmKunci'; ov.className = 'visitor-overlay'; ov.style.zIndex = '100001';
    var n = pras.n;
    ...
    ov.innerHTML = '<div class="visitor-modal pm-kunci-modal">'
      ...
      + '<button class="pm-periksa" type="button" id="pmPeriksa">🔄 Periksa lagi</button>'
      + '<p class="pm-catatan">Sudah melengkapi Modul ' + n + ' di tab lain? Tekan <strong>Periksa lagi</strong>, tidak perlu login ulang.</p>'
    ...
    var periksa = ov.querySelector('#pmPeriksa');
    periksa.addEventListener('click', function () {
      var d = dasar();
      if (!d.pinHash) { toast('⚠ Sesi login berakhir. Tekan Keluar, lalu login lagi.', 4500); return; }
      periksa.disabled = true; periksa.textContent = '⏳ Memeriksa…';
      panggil('getModulProgress', d).then(function (p) {
        if (p.akses && p.akses.boleh === false && p.akses.prasyarat) {
          tampilkanKunci(p.akses.prasyarat);   // gambar ulang rincian dengan data terbaru
          toast('Modul ' + n + ' belum lengkap. Lengkapi syarat yang bertanda ❌.', 4500);
          return;
        }
        ov.remove();
        if (fab) fab.style.display = '';
        terapkanProgres(p);
        toast('✅ Modul ' + n + ' sudah lengkap. Selamat mempelajari Modul ' + nomorModul + '.', 4500);
      }).catch(function (e) {
        periksa.disabled = false; periksa.textContent = '🔄 Periksa lagi';
        toast('⚠ Gagal memeriksa: ' + ((e && e.message) || 'koneksi'), 4500);
        if (/login ulang/i.test(String(e && e.message))) window._sessionPinHash = null;
      });
    });
  }
