...
(function () {
  ...
  function render() {
    kotak.forEach(function (el, i) {
      ...
      cb.checked = selesai;
      cb.disabled = !bebas && !giliran;
      el.classList.toggle('pm-selesai', selesai);
      el.classList.toggle('pm-nonaktif', !bebas && !selesai && !giliran);
  ...
  kotak.forEach(function (el, i) {
    el.querySelector('input').addEventListener('change', function (ev) {
      var cb = ev.target;
      if (bebas) { if (cb.checked) centang = Math.max(centang, i + 1); else centang = Math.min(centang, i); render(); return; }
  ...
  function muatProgres() {
    if (!mhsAktif()) { bebas = true; render(); return; }
    var d = dasar();
