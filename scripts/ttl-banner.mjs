...
const WRAP_OPEN = '<div style="background:#07101f;font-family:\'Segoe UI\',Arial,sans-serif;border-radius:22px;overflow:hidden;border:1px solid #22345a;color:#eaf2ff;box-shadow:0 16px 36px rgba(2,8,23,.28);"><div style="height:5px;background:linear-gradient(90deg,#22d3ee 0%,#6366f1 42%,#a855f7 72%,#f59e0b 100%);"></div><div style="padding:30px 34px;background:radial-gradient(circle at 82% 18%,rgba(168,85,247,.22),transparent 34%),radial-gradient(circle at 8% 90%,rgba(34,211,238,.13),transparent 32%),#07101f;">';
const FOOTER = (tahun = "2026/2027") => `<div style="padding:11px 34px;background:#0a1527;border-top:1px solid #22345a;color:#8ea1bd;font-size:11px;">Dedik Romahadi &middot; Teknik Tenaga Listrik &middot; S1 Teknik Mesin &middot; Universitas Mercu Buana &middot; ${tahun}</div></div>`;
const pill = (teks, bg, bd, fg, extra = "") => `<span style="display:inline-block;${extra}padding:5px 11px;border-radius:999px;background:${bg};border:1px solid ${bd};color:${fg};font-size:11px;font-weight:800;letter-spacing:1.2px;">${teks}</span>`;
const chip = (teks, i) => i % 2 === 0
  ? `<span style="display:inline-block;margin:0 6px 7px 0;padding:6px 10px;border-radius:8px;background:#0c2940;border:1px solid #1d5873;color:#8ce8ff;font-size:11px;">${teks}</span>`
  : `<span style="display:inline-block;margin:0 6px 7px 0;padding:6px 10px;border-radius:8px;background:#17264a;border:1px solid #334f87;color:#b9d3ff;font-size:11px;">${teks}</span>`;
const btnAktif = (href, ikon, teks, gaya) => `<a href="${href}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;justify-content:center;gap:8px;width:100%;box-sizing:border-box;${gaya}color:#fff;text-decoration:none;font-size:12px;line-height:1.35;font-weight:800;"><span style="font-size:19px;line-height:1;">${ikon}</span><span>${teks}</span></a>`;
const btnMati = (ikon, teks, mt = 9) => `<span role="button" aria-disabled="true" style="display:inline-flex;align-items:center;justify-content:center;gap:8px;width:100%;box-sizing:border-box;margin-top:${mt}px;padding:12px 14px;border-radius:12px;background:#334155;color:#cbd5e1;font-size:12px;line-height:1.35;font-weight:800;border:1px solid #64748b;cursor:not-allowed;"><span style="font-size:18px;line-height:1;">${ikon}</span><span>${teks}</span></span>`;
const GAYA_MODUL = "margin-top:12px;padding:13px 14px;border-radius:12px;background:linear-gradient(135deg,#0891b2,#4f46e5 62%,#7e22ce);box-shadow:0 8px 22px rgba(79,70,229,.35);";
const GAYA_MEET = "margin-top:9px;padding:12px 14px;border-radius:12px;background:#0f766e;border:1px solid #2dd4bf;";
...
${WRAP_OPEN}<table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"><tr><td valign="middle" style="padding-right:24px;"><div style="margin-bottom:12px;">${pill(`PERTEMUAN ${k.p}`, "#12304a", "#287da0", "#8ce8ff")}${pill(`SUB-CPMK ${m.sub}`, "#28184c", "#6336a5", "#d8b4fe", "margin-left:7px;")}${pill(t.label, t.bg, t.bd, t.fg, "margin-left:7px;")}</div><h2 style="margin:0 0 10px;color:#ffffff;font-size:25px;line-height:1.2;letter-spacing:-.4px;">${m.judul}</h2><p style="margin:0 0 14px;color:#c4d3ea;font-size:13.5px;line-height:1.7;max-width:620px;">${m.desk}</p><div style="margin-bottom:12px;">${m.chips.map(chip).join("")}</div><div style="display:inline-block;padding:8px 12px;border-radius:10px;background:#0b172a;border:1px solid #26395c;font-size:11px;line-height:1.6;color:#d6e2f5;">&#128197; <strong style="color:#fff;">${fmtPanjang(k.tgl)}</strong> &middot; ${JAM} &middot; ${t.nama}<br>&#9203; Deadline Tugas &amp; Forum: <strong style="color:#fcd34d;">${fmtPanjang(dl)}, 23:59 WIB</strong></div></td><td width="230" valign="middle" style="text-align:center;"><div style="background:#0b172a;border:1px solid #26395c;border-radius:16px;padding:16px 17px;text-align:left;"><div style="font-size:11px;font-weight:800;color:#93c5fd;letter-spacing:1px;margin-bottom:11px;">ALUR PEMBELAJARAN</div><div style="font-size:11px;line-height:1.65;color:#d6e2f5;">${alur}</div><div style="height:1px;background:#26395c;margin:14px 0;"></div><div style="font-size:10px;line-height:1.6;color:#f8c77b;"><strong>&#9200; Aturan terlambat:</strong><br>Setelah deadline, poin setiap soal dipotong <strong>35%</strong>.</div></div>${tombolModul}${tombolKedua}</td></tr></table></div>${FOOTER()}
`;
}
...
export function bannerUjian(k) {
  ...
  const cakupanModul = uts ? "Modul 1–7" : "Modul 8–14";
  const cakupanSub = uts ? "Sub-CPMK 1.1, 1.2, 1.3, 2.1, 2.2, 3.1, dan 3.2" : "Sub-CPMK 4.1, 4.2, 5.1, 5.2, 6.1, 7.1, dan 7.2";
  const bobot = uts ? "25%" : "32%";
  const terbit = EXAM_PUBLISHED[k.tipe];
  const tombol = terbit
    ? `<a href="${BASE}/Exam/${k.tipe}.html" target="_blank" rel="noopener noreferrer" style="display:block;box-sizing:border-box;width:100%;margin-top:9px;padding:12px 14px;border-radius:12px;background:${warna.btn};color:${warna.btnFg};text-decoration:none;font-size:12px;font-weight:900;text-align:center;">Buka ${k.tipe} &rarr;</a>`
    : `<span role="button" aria-disabled="true" style="display:block;box-sizing:border-box;width:100%;margin-top:9px;padding:12px 14px;border-radius:12px;background:#334155;color:#cbd5e1;font-size:12px;font-weight:900;text-align:center;border:1px solid #64748b;cursor:not-allowed;">Halaman ${k.tipe} dibuka menjelang ujian</span>`;
  return `<!-- Banner ${k.tipe} Teknik Tenaga Listrik — LMS compatible, inline styles; dibuat oleh scripts/ttl-banner.mjs -->
<div style="background:${warna.bg};font-family:'Segoe UI',Arial,sans-serif;border-radius:18px;overflow:hidden;border:1px solid ${warna.bd};color:${warna.fg};box-shadow:0 14px 30px ${warna.bayang};"><div style="height:4px;background:${warna.bar};"></div><div style="padding:20px 24px;background:${warna.inner};"><table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"><tr><td valign="top" style="padding-right:20px;"><div style="font-size:10.5px;font-weight:900;color:${warna.label};letter-spacing:1.6px;margin-bottom:7px;">${k.tipe} &middot; TEKNIK TENAGA LISTRIK &middot; PERTEMUAN ${k.p}</div><h2 style="margin:0 0 8px;color:#fff;font-size:22px;line-height:1.15;">${uts ? "Ujian Tengah Semester" : "Ujian Akhir Semester"}</h2><p style="margin:0 0 10px;color:${warna.teks};font-size:12.5px;line-height:1.6;">Kerjakan mandiri, lengkapi bukti proses autentik, lalu export dan submit file HTML melalui Fast Learning.</p><div style="font-size:11px;line-height:1.65;color:${warna.muted};">Cakupan materi: <strong style="color:${warna.kuat};">${cakupanModul}</strong> (${cakupanSub}).<br>Bentuk: 45 soal &middot; 4 bagian &middot; skala 100 &middot; open-book &middot; Python. Bobot nilai akhir: <strong style="color:${warna.kuat};">${bobot}</strong>.<br>Soal komputasi bersifat parametrik dari 2 digit terakhir NIM.</div></td><td width="250" valign="top"><div style="background:${warna.kartu};border:1px solid ${warna.kartuBd};border-radius:12px;padding:12px 14px;font-size:11px;line-height:1.6;color:${warna.fg};"><div style="font-size:10px;font-weight:900;color:${warna.label};letter-spacing:1.2px;margin-bottom:5px;">JADWAL PELAKSANAAN</div><div><strong>Masa ${k.tipe}:</strong> ${rentang(k.tgl, k.akhir)}<br><strong>Hari/Jam:</strong> mengikuti jadwal resmi di web SIA<br><strong>Durasi:</strong> 180 menit + perpanjangan 120 menit (terlambat, potongan 35%)</div></div>${tombol}</td></tr></table></div><div style="padding:9px 24px;background:${warna.foot};border-top:1px solid ${warna.footBd};color:${warna.footFg};font-size:10.5px;">Soal bersifat parametrik &mdash; angkanya dihitung dari 2 digit terakhir NIM, sehingga jawaban tiap mahasiswa berbeda. Login dengan NIM &amp; PIN Anda, kerjakan sekali submit per soal, lalu export &amp; kumpulkan sebelum batas waktu.</div></div>
`;
}
...
export function bannerIntroduction() {
  ...
  const baris = (k) => {
    ...
    return `<tr><td style="padding:5px 6px;border-bottom:1px solid rgba(255,255,255,.06);font-size:11px;color:#e2e8f0;font-weight:800;">${k.p}</td><td style="padding:5px 6px;border-bottom:1px solid rgba(255,255,255,.06);"><span style="display:inline-block;min-width:52px;text-align:center;padding:2px 7px;border-radius:6px;background:${bg};border:1px solid ${bd};color:${fg};font-size:9.5px;font-weight:800;letter-spacing:.6px;">${k.tipe}</span></td><td style="padding:5px 6px;border-bottom:1px solid rgba(255,255,255,.06);font-size:11px;color:#cbd5e1;white-space:nowrap;">${tanggal}</td><td style="padding:5px 6px;border-bottom:1px solid rgba(255,255,255,.06);font-size:11px;color:#94a3b8;">${isi}</td></tr>`;
  };
...
<div style="background:#070b16;font-family:'Segoe UI','Helvetica Neue',Arial,sans-serif;border-radius:22px;overflow:hidden;border:1px solid rgba(96,165,250,.18);color:#e2e8f0;">
  ...
  <div style="padding:34px 38px 28px;position:relative;overflow:hidden;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="position:relative;z-index:1;">
      <tr>
        <td valign="middle" style="padding-right:28px;">
          ...
          <h1 style="font-size:34px;font-weight:900;line-height:1.08;margin:0 0 12px 0;letter-spacing:-1px;color:#f8fafc;">
            Teknik Tenaga
          ...
          <p style="font-size:14px;color:#a5b4c7;line-height:1.7;margin:0 0 17px 0;max-width:610px;">
        ...
        <td width="250" valign="middle" style="text-align:center;">
          <div style="width:220px;height:180px;display:inline-flex;align-items:center;justify-content:center;border-radius:22px;background:linear-gradient(145deg,rgba(37,99,235,.13),rgba(139,92,246,.08));border:1px solid rgba(96,165,250,.16);position:relative;">
            <svg width="195" height="145" viewBox="0 0 195 145" fill="none" xmlns="http://www.w3.org/2000/svg">
              <defs>
  ...
  <div style="height:1px;background:linear-gradient(90deg,transparent,rgba(34,211,238,.3),rgba(139,92,246,.3),rgba(244,114,182,.3),transparent);margin:0 38px;"></div>

  <div style="padding:25px 38px 28px;">
    <div style="font-size:11px;font-weight:800;color:#64748b;letter-spacing:2px;text-transform:uppercase;margin-bottom:12px;font-family:monospace;">Alur Kompetensi</div>
    <table width="100%" cellpadding="0" cellspacing="8" border="0">
      <tr>
        <td width="25%" valign="top" style="background:rgba(34,211,238,.055);border:1px solid rgba(34,211,238,.14);border-radius:13px;padding:14px;">
          <div style="font-size:20px;margin-bottom:7px;">01</div>
        ...
        <td width="25%" valign="top" style="background:rgba(96,165,250,.055);border:1px solid rgba(96,165,250,.14);border-radius:13px;padding:14px;">
          <div style="font-size:20px;margin-bottom:7px;">02</div>
        ...
        <td width="25%" valign="top" style="background:rgba(167,139,250,.055);border:1px solid rgba(167,139,250,.14);border-radius:13px;padding:14px;">
          <div style="font-size:20px;margin-bottom:7px;">03</div>
        ...
        <td width="25%" valign="top" style="background:rgba(244,114,182,.055);border:1px solid rgba(244,114,182,.14);border-radius:13px;padding:14px;">
          <div style="font-size:20px;margin-bottom:7px;">04</div>
    ...
    <div style="margin-top:19px;font-size:11px;font-weight:800;color:#94a3b8;letter-spacing:2px;text-transform:uppercase;margin-bottom:12px;font-family:monospace;">Outline Mata Kuliah</div>
    <table width="100%" cellpadding="0" cellspacing="8" border="0" style="table-layout:fixed;">
      <tr>
        <td width="50%" valign="top" style="background:#0b1d31;border:1px solid #1e4976;border-radius:13px;padding:16px;">
          <div style="font-size:11px;font-weight:900;color:#67e8f9;margin-bottom:9px;">KONSEP, JARINGAN, &amp; TRANSMISI (SEBELUM UTS)</div>
          <div style="font-size:10px;color:#dbeafe;line-height:1.75;">${outline(MODUL.slice(0, 7))}</div>
        </td>
        <td width="50%" valign="top" style="background:#221638;border:1px solid #5b3a88;border-radius:13px;padding:16px;">
          <div style="font-size:11px;font-weight:900;color:#d8b4fe;margin-bottom:9px;">TRANSMISI, DISTRIBUSI, &amp; ANALISIS (SETELAH UTS)</div>
          <div style="font-size:10px;color:#ede9fe;line-height:1.75;">${outline(MODUL.slice(7))}</div>
        </td>
      </tr>
    </table>
    ...
    <div style="background:#0a1730;border:1px solid #27447d;border-radius:15px;padding:17px 18px;box-shadow:inset 0 1px 0 rgba(255,255,255,.025);">
      ...
      <table width="100%" cellpadding="0" cellspacing="7" border="0" style="table-layout:fixed;">
        <tr>
          <td width="25%" valign="top" style="background:#0d2940;border:1px solid #155e75;border-radius:11px;padding:12px 13px;">
          ...
          <td width="25%" valign="top" style="background:#351b27;border:1px solid #9f1239;border-radius:11px;padding:12px 13px;">
    ...
    <div style="margin-top:19px;font-size:11px;font-weight:800;color:#94a3b8;letter-spacing:2px;text-transform:uppercase;margin-bottom:12px;font-family:monospace;">Kalender Pertemuan &mdash; Kode Kelas 2F, Sabtu Reguler 2</div>
    <table width="100%" cellpadding="0" cellspacing="8" border="0" style="table-layout:fixed;">
      <tr>
        <td width="50%" valign="top" style="background:#0b1425;border:1px solid #1f3355;border-radius:13px;padding:10px 12px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="8" border="0" style="table-layout:fixed;">
      <tr>
        <td width="33.33%" valign="top" style="background:#0b2235;border:1px solid #155e75;border-radius:13px;padding:16px;">
          <div style="font-size:12px;font-weight:900;color:#67e8f9;margin-bottom:10px;">STATUS KEHADIRAN</div>
          <div style="font-size:10px;color:#dbeafe;line-height:1.75;">
        ...
        <td width="33.33%" valign="top" style="background:#161b3a;border:1px solid #4338ca;border-radius:13px;padding:16px;">
          <div style="font-size:12px;font-weight:900;color:#c4b5fd;margin-bottom:10px;">ATURAN MODUL</div>
          <div style="font-size:10px;color:#e0e7ff;line-height:1.75;">
        ...
        <td width="33.33%" valign="top" style="background:#32162f;border:1px solid #9d174d;border-radius:13px;padding:16px;">
          <div style="font-size:12px;font-weight:900;color:#f9a8d4;margin-bottom:10px;">ATURAN EXAM</div>
          <div style="font-size:10px;color:#fce7f3;line-height:1.75;">
    ...
    <div style="margin-top:10px;background:#111827;border:1px solid #334155;border-radius:11px;padding:11px 14px;font-size:10px;color:#cbd5e1;line-height:1.6;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:19px;background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.07);border-radius:14px;">
      <tr>
        ...
        <td style="padding:15px 18px;border-left:1px solid rgba(255,255,255,.06);">
        ...
        <td width="190" style="padding:12px 15px;border-left:1px solid rgba(255,255,255,.06);text-align:center;">
          <a href="${BASE}/OBE/Penilaian-OBE.htm" target="_blank" style="display:inline-block;width:165px;box-sizing:border-box;padding:11px 13px;border-radius:10px;text-decoration:none;background:linear-gradient(135deg,#2563eb,#7c3aed);color:#fff;font-size:11px;font-weight:800;box-shadow:0 5px 15px rgba(37,99,235,.25);">Silabus &amp; Penilaian OBE &rarr;</a>
          <a href="https://dedik-romahadi.github.io/Mechanical-Engineering-Courses/Unduhan-Gabungan/RPS-Teknik-Tenaga-Listrik.pdf" target="_blank" rel="noopener noreferrer" style="display:inline-block;width:165px;box-sizing:border-box;margin-top:8px;padding:11px 13px;border-radius:10px;text-decoration:none;background:linear-gradient(135deg,#16a34a,#0d9488);color:#fff;font-size:11px;font-weight:800;box-shadow:0 5px 15px rgba(22,163,74,.24);">RPS (PDF) &rarr;</a>
        </td>
      </tr>
    </table>
  </div>

  <div style="background:rgba(255,255,255,.025);border-top:1px solid rgba(255,255,255,.06);padding:11px 38px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
